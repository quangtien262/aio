    ...
    class CmsModuleSeeder extends Seeder
    {
        public function run(): void
        {
            ...
            if (Schema::hasTable('site_profiles') && Schema::hasTable('cms_menus')) {
                ...
                $primaryMenu = DB::table('cms_menus')->where('location', 'primary-navigation')->first();
                $primaryMenuItems = $primaryMenu ? json_decode((string) $primaryMenu->items, true) : [];
                ...
                if (strtoupper($activeThemeKey) === 'XD0301' && $primaryMenu && $hasForeignDemoLink) {
                    DB::table('cms_menus')->where('id', $primaryMenu->id)->update([
                        'items' => json_encode([
