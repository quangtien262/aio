@php
    ...
    $contactFormTitle = $contactFormTitle !== ''
        ? $contactFormTitle
        : (app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0314', app()->getLocale(), 'legacy_inline.87c2af088eba47e9', 'Gửi yêu cầu liên hệ'));
    $contactNoteTitle = $contactNoteTitle !== ''
        ? $contactNoteTitle
        : (app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0314', app()->getLocale(), 'legacy_inline.e8d82960e1ea853a', 'Chia sẻ nhu cầu, chúng tôi tư vấn đúng giải pháp.'));
    $contactNoteText = $contactNoteText !== ''
        ? $contactNoteText
        : (app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0314', app()->getLocale(), 'legacy_inline.90c7204030ac6333', 'Hãy gửi thêm địa điểm, diện tích, tiến độ mong muốn hoặc yêu cầu kỹ thuật để đội ngũ chuẩn bị phương án phù hợp ngay từ lần phản hồi đầu tiên.'));
    $contactSubmitLabel = $contactSubmitLabel !== ''
        ? $contactSubmitLabel
        : (app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0314', app()->getLocale(), 'legacy_inline.fc4ed14bcef77494', 'Gửi liên hệ'));
@endphp
        ...
        <div class="xd-contact-page">
            ...
            <article class="xd-contact-form-card">
                ...
                <form class="xd-contact-form" method="POST" action="{{ route('site.contact.submit') }}">
                    ...
                    <input type="hidden" name="subject" value="{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0314', app()->getLocale(), 'legacy_inline.86f0eaa8c37c43a7', 'Yêu cầu liên hệ từ landing page') }}">
