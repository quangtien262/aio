    <script>
        (() => {
            const bb14Nav = document.querySelector('[data-bb14-nav]');
            document.querySelector('[data-bb14-menu-toggle]')?.addEventListener('click', (event) => {
                const isOpen = bb14Nav?.classList.toggle('is-open') || false;
                event.currentTarget.setAttribute('aria-expanded', isOpen ? 'true' : 'false');
            });

            const bb14HeroSlides = Array.from(document.querySelectorAll('.bb14-hero__slide'));
            let bb14HeroIndex = 0;
            const showBb14Hero = (next) => {
                if (!bb14HeroSlides.length) return;
                bb14HeroIndex = (next + bb14HeroSlides.length) % bb14HeroSlides.length;
                bb14HeroSlides.forEach((slide, slideIndex) => slide.classList.toggle('is-active', slideIndex === bb14HeroIndex));
            };
            let bb14HeroTimer = window.setInterval(() => showBb14Hero(bb14HeroIndex + 1), 6200);
            const restartBb14Hero = () => {
                window.clearInterval(bb14HeroTimer);
                bb14HeroTimer = window.setInterval(() => showBb14Hero(bb14HeroIndex + 1), 6200);
            };
            document.querySelector('[data-bb14-hero-prev]')?.addEventListener('click', () => { showBb14Hero(bb14HeroIndex - 1); restartBb14Hero(); });
            document.querySelector('[data-bb14-hero-next]')?.addEventListener('click', () => { showBb14Hero(bb14HeroIndex + 1); restartBb14Hero(); });

            document.querySelectorAll('[data-bb14-row]').forEach((track) => {
                let rowTimer = null;
                const step = () => Math.max(280, Math.round(track.clientWidth * 0.34));
                const go = () => {
                ...
                rowTimer = window.setInterval(go, 3600);
                track.addEventListener('pointerenter', () => window.clearInterval(rowTimer));
                track.addEventListener('pointerleave', () => { rowTimer = window.setInterval(go, 3600); });
            });
