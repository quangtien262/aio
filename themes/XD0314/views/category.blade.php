@php
    ...
    if (! $navItems->contains(fn (array $item): bool => in_array(mb_strtolower(trim($item['label'])), ['trang chủ', 'home'], true) || rtrim($item['href'], '/') === rtrim($homeUrl, '/'))) {
        $navItems->prepend([
            'label' => app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0314', app()->getLocale(), 'legacy_inline.4c23dc9bef7f79b4', 'Trang chủ'),
            'href' => $homeUrl,
            'target' => '_self',
    ...
    if (false && ! $hasProductItem && \Illuminate\Support\Facades\Schema::hasTable('catalog_categories') && \Illuminate\Support\Facades\Schema::hasTable('catalog_products')) {
        ...
        if ($productCategories->isNotEmpty()) {
            $productMenuItem = [
                'label' => app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0314', app()->getLocale(), 'legacy_inline.571ef44479d97bfd', 'Sản phẩm'),
    ...
    $footerNewsletterSource = 'theme-footer-xd0314-category';
@endphp
            ...
            <section class="xd-container xd-catalog">
                <aside class="xd-sidebar">
                    ...
                    <div class="xd-panel xd-promo">
                        ...
                        <p>Gửi nhu cầu công trình để đội ngũ XD0314 gợi ý nhóm sản phẩm và phương án triển khai phù hợp.</p>
                    </div>
                </aside>
