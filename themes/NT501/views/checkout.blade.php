@php
    ...
    $footerNewsletterSource = 'theme-footer-nt501-checkout';
@endphp

@extends('theme-nt501::layout')

@section('title'){{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.ffbbcb0e6e846d20', 'Thanh toán') }} | {{ $logoAlt }}@endsection
        ...
        <main class="xd-page-main">
            <div class="xd-container">
                <nav class="xd-breadcrumb" aria-label="Breadcrumb">
                    <a href="{{ route('site.home') }}">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.95a078fb35ea9444', 'Trang chủ') }}</a>
                    <span>/</span>
                    <a href="{{ route('site.cart.index') }}">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.039c20d382b35f6c', 'Giỏ hàng') }}</a>
                    <span>/</span>
                    <strong>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.ffbbcb0e6e846d20', 'Thanh toán') }}</strong>
                </nav>

                <section class="xd-heading">
                    <div>
                        <span class="xd-kicker">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.ffbbcb0e6e846d20', 'Thanh toán') }}</span>
                        <h1>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.ed438e4fa576143c', 'Hoàn tất đơn hàng') }}</h1>
                        <p>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.60041320569bac29', 'Xác nhận thông tin nhận hàng và phương thức thanh toán trước khi gửi đơn.') }}</p>
                    </div>
                </section>
                ...
                @if (empty($cartItems))
                    <section class="xd-empty">
                        <h2>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.aa67286ccf049194', 'Giỏ hàng đang trống') }}</h2>
                        <p>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.f8ad59e6971a4dc0', 'Vui lòng thêm sản phẩm trước khi thanh toán.') }}</p>
                        <a class="xd-button" href="{{ route('site.catalog.search') }}">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.d287e584ad26a5b7', 'Xem sản phẩm') }}</a>
                    </section>
                @else
                    <section class="xd-checkout-grid">
                        <form class="xd-panel xd-form-panel" method="POST" action="{{ route('site.checkout.store') }}">
                            @csrf
                            <h2 class="xd-section-title">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.304050a05e54de05', 'Thông tin nhận hàng') }}</h2>
                            <div class="xd-form-grid">
                                <label class="xd-field">
                                    <span>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.2da0a765563692ba', 'Họ tên') }}</span>
                                    <input name="customer_name" value="{{ old('customer_name', $form['customer_name'] ?? '') }}" required>
                                </label>
                                <label class="xd-field">
                                    <span>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.54aefaac6268d7d4', 'Số điện thoại') }}</span>
                                ...
                                <label class="xd-field is-wide">
                                    <span>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.70e3eae66184f74b', 'Địa chỉ nhận hàng') }}</span>
                                    <textarea name="delivery_address" required>{{ old('delivery_address', $form['delivery_address'] ?? '') }}</textarea>
                                </label>
                                <label class="xd-field is-wide">
                                    <span>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.5f3baddb185a13c2', 'Ghi chú') }}</span>
                                    <textarea name="note">{{ old('note', $form['note'] ?? '') }}</textarea>
                                </label>
                            </div>

                            <div class="xd-payment-list">
                                <h2 class="xd-section-title">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.f84820dd494e0b39', 'Phương thức thanh toán') }}</h2>
                            ...
                            <button class="xd-button" type="submit">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.07a2504dbde9a662', 'Đặt hàng') }}</button>
                        </form>

                        <aside class="xd-panel xd-summary">
                            <h2>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.1e0f38a8c0ea756b', 'Tóm tắt đơn hàng') }}</h2>
                            ...
                            <div class="xd-summary-row">
                                <span>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.7bae6079922c9b39', 'Số sản phẩm') }}</span>
                                <b>{{ (int) ($cartSummary['count'] ?? 0) }}</b>
                            </div>
                            <div class="xd-summary-row">
                                <span>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.e9ceb75b16e156cb', 'Tạm tính') }}</span>
                                <strong>{{ $formatCurrency($cartSummary['subtotal'] ?? 0) }}</strong>
                            </div>
                            <a class="xd-button is-ghost" href="{{ route('site.cart.index') }}">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.a00530860cd44a4c', 'Quay lại giỏ hàng') }}</a>
                        </aside>
                    </section>
                @endif
            </div>
