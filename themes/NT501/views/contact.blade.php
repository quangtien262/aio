...
<main class="xd-page-main">
            <div class="xd-container">
                    <section class="xd-cms-hero">
                        <div>
                            <span class="xd-kicker">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.3320c47802c074f4', 'Liên hệ') }}</span>
                            <h1>{{ $entry->title }}</h1>
                            @if (!empty($entry->excerpt))
                                <p>{{ $entry->excerpt }}</p>
                            @else
                                <p>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.f8e77a37a939f481', 'Gửi nhu cầu tư vấn, thiết kế hoặc thi công. Đội ngũ NT501 sẽ phản hồi trong thời gian sớm nhất.') }}</p>
                            @endif
                        </div>
                        <div class="xd-cms-stats">
                            <strong>24h</strong>
                            <span>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.a57fe22824ad954e', 'Thời gian phản hồi') }}</span>
                        </div>
                    </section>

                    <section class="xd-contact-page">
                        <aside class="xd-contact-panel">
                            <span class="xd-kicker">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.07c97badbd540233', 'Thông tin liên hệ') }}</span>
                            <h2>{{ $branding['company_name'] ?? $logoAlt }}</h2>
                            <p>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.60e0d5f7b82df22a', 'Hãy cho chúng tôi biết nhu cầu, quy mô và thời gian dự kiến. Đội ngũ tư vấn sẽ kiểm tra và đề xuất hướng triển khai phù hợp.') }}</p>
                            <ul class="xd-contact-methods">
                                ...
                                <li class="xd-contact-method">
                                    ...
                                    <div>
                                        <small>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.39f1595b075abbb4', 'Địa chỉ') }}</small>
                                        <span>{{ $address }}</span>
                                    </div>
                                </li>
                            </ul>
                            <div class="xd-contact-note">
                                <strong>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.6b20eb393015ec65', 'Chia sẻ nhu cầu, chúng tôi tư vấn đúng giải pháp.') }}</strong>
                                <span>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.d1a5dfffedb6e0e5', 'Hãy gửi thêm địa điểm, diện tích, tiến độ mong muốn hoặc yêu cầu kỹ thuật để đội ngũ chuẩn bị phương án phù hợp ngay từ lần phản hồi đầu tiên.') }}</span>
                            </div>
                        </aside>

                        <article class="xd-contact-form-card">
                            <h2>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.17a86d6788887a67', 'Gửi yêu cầu liên hệ') }}</h2>
                            @if (session('contact_status'))
                                <div class="xd-contact-alert">{{ session('contact_status') }}</div>
                            @endif
                            @if ($errors->any())
                                <div class="xd-contact-errors">
                                    {{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.ca610cc75d04b584', 'Vui lòng kiểm tra lại thông tin.') }}
                                    <ul>
                            ...
                            <form class="xd-contact-form" method="POST" action="{{ route('site.contact.submit') }}">
                                ...
                                <input type="hidden" name="subject" value="{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.f6ba18550b431f61', 'Yêu cầu liên hệ từ website') }}">
                                ...
                                <label class="xd-contact-field">
                                    <span>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.57629094dd2370ca', 'Số điện thoại') }}</span>
                                ...
                                <label class="xd-contact-field">
                                    <span>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.bfc4253cd375512d', 'Nội dung') }}</span>
                                    <textarea name="message" required>{{ old('message') }}</textarea>
                                </label>
                                <button class="xd-contact-submit" type="submit">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.345403bcaa8eba97', 'Gửi liên hệ') }}</button>
                            </form>
                        </article>
                    </section>
            </div>
