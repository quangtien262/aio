@php
    ...
    $footerNewsletterSource = 'theme-footer-nt501-checkout-success';
@endphp

@extends('theme-nt501::layout')

@section('title'){{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.073da0a1c207a96b', 'Đặt hàng thành công') }} | {{ $logoAlt }}@endsection
        ...
        <main class="xd-page-main">
            <div class="xd-container">
                <nav class="xd-breadcrumb" aria-label="Breadcrumb">
                    ...
                    <span>/</span>
                    <strong>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.073da0a1c207a96b', 'Đặt hàng thành công') }}</strong>
                </nav>

                <section class="xd-success-hero">
                    <span class="xd-kicker">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.3e2d57b436474e82', 'Thành công') }}</span>
                    <h1>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.a7d6a5113d703034', 'Đơn hàng đã được ghi nhận') }}</h1>
                    <p>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.e4babb1d650e5b69', 'Đội ngũ tư vấn sẽ liên hệ lại để xác nhận thông tin sản phẩm, giao nhận và phương thức thanh toán.') }}</p>
                </section>

                <section class="xd-success-grid">
                    <article class="xd-panel xd-order-panel">
                        <h2>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.1448415a47947f7c', 'Thông tin đơn hàng') }}</h2>
                        <div class="xd-order-lines">
                            <div class="xd-order-line">
                                <span class="xd-order-label">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.dbde08e2bee8bf66', 'Mã đơn hàng') }}</span>
                                <span class="xd-order-value">{{ $confirmedOrder->order_code }}</span>
                            </div>
                            <div class="xd-order-line">
                                <span class="xd-order-label">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.bd6fb2ec8f965d6c', 'Thời gian tạo') }}</span>
                                <span class="xd-order-value">{{ optional($confirmedOrder->placed_at ?? $confirmedOrder->created_at)->format('H:i d/m/Y') }}</span>
                            </div>
                            <div class="xd-order-line">
                                <span class="xd-order-label">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.53e5f724ba217e20', 'Khách hàng') }}</span>
                                <span class="xd-order-value">{{ $confirmedOrder->customer_name }}</span>
                            </div>
                            <div class="xd-order-line">
                                <span class="xd-order-label">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.54aefaac6268d7d4', 'Số điện thoại') }}</span>
                            ...
                            <div class="xd-order-line">
                                <span class="xd-order-label">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.70e3eae66184f74b', 'Địa chỉ nhận hàng') }}</span>
                                <span class="xd-order-value">{{ $confirmedOrder->delivery_address }}</span>
                            </div>
                            <div class="xd-order-line">
                                <span class="xd-order-label">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.f84820dd494e0b39', 'Phương thức thanh toán') }}</span>
                                <span class="xd-order-value">{{ $confirmedOrder->payment_label }}</span>
                            </div>
                            <div class="xd-order-line">
                                <span class="xd-order-label">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.e9ceb75b16e156cb', 'Tạm tính') }}</span>
                                <span class="xd-order-value">{{ $formatCurrency($confirmedOrder->subtotal) }}</span>
                            </div>
                        </div>

                        <div class="xd-button-row">
                            <a class="xd-button" href="{{ route('site.home') }}">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.4edc668e9b5ed78d', 'Về trang chủ') }}</a>
                            <a class="xd-button is-ghost" href="{{ route('customer.account') }}">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.2824d699527b8355', 'Tài khoản của tôi') }}</a>
                        </div>
                    </article>

                    <aside class="xd-panel xd-order-panel">
                        <h2>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), 'legacy_inline.76ffae1228bd585e', 'Sản phẩm') }}</h2>
                        <div class="xd-items">
