@php
    ...
    $themeText = fn (string $key): string => app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('NT501', app()->getLocale(), $key);
    ...
    if ($navItems->isEmpty()) {
        $navItems = collect([
            ['label' => $themeText('nt501.nav.home'), 'href' => route('site.home'), 'children' => []],
            ['label' => $themeText('nt501.nav.services'), 'href' => '#dich-vu', 'children' => []],
            ['label' => $themeText('nt501.nav.story'), 'href' => '#gioi-thieu', 'children' => []],
            ['label' => $themeText('nt501.nav.menu'), 'href' => '#du-an', 'children' => []],
            ['label' => $themeText('nt501.nav.news'), 'href' => '#tin-tuc', 'children' => []],
            ['label' => $themeText('nt501.nav.team'), 'href' => '#lien-he', 'children' => []],
        ]);
    }
@endphp
        ...
        <div class="foot-container foot-header__masthead-inner">
            <a class="foot-brand" href="{{ route('site.home') }}" aria-label="{{ $companyName }}">
                ...
                @else
                    ...
                    <span><strong>{{ $companyName }}</strong><small>@themeT('nt501.brand.tagline')</small></span>
                @endif
            </a>
            <div class="foot-header__account">
                @guest('customer')
                    ...
                    <button type="button" data-xd-auth-open="register">@themeT('nt501.header.register')</button>
                @else
