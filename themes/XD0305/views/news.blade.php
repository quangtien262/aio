...
<main class="xd-page-main">
            <div class="xd-container">
                    <section class="xd-cms-hero">
                        <div>
                            <span class="xd-kicker">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0305', app()->getLocale(), 'legacy_inline.6b36109704ab6b5c', 'Tin tức') }}</span>
                            <h1>{{ $pageTitle ?? (app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0305', app()->getLocale(), 'legacy_inline.6b36109704ab6b5c', 'Tin tức')) }}</h1>
                            <p>{{ $pageDescription ?? (app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0305', app()->getLocale(), 'legacy_inline.e9823473fc1a10fa', 'Danh sách bài viết, tin tức và kinh nghiệm xây dựng mới nhất.')) }}</p>
                        </div>
                        <div class="xd-cms-stats">
                            <strong>{{ method_exists($listingItems, 'total') ? $listingItems->total() : collect($listingItems ?? [])->count() }}</strong>
                            <span>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0305', app()->getLocale(), 'legacy_inline.c95b49cab7687600', 'Bài viết đã xuất bản') }}</span>
                        </div>
                    </section>
                        ...
                        @forelse ($listingItems as $post)
                            ...
                            <article class="xd-service-card">
                                <a class="xd-service-image" href="{{ $postUrl }}" aria-label="{{ $post->title }}">
                                    <img src="{{ $image ?: 'https://picsum.photos/seed/xd0305-post-'.($post->id ?? 'default').'/960/720' }}" alt="{{ $post->title }}">
                                </a>
                                <div class="xd-service-body">
                                    <h2><a href="{{ $postUrl }}">{{ $post->title }}</a></h2>
                                    <p>{{ $summary }}</p>
                                    <a class="xd-text-link" href="{{ $postUrl }}">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0305', app()->getLocale(), 'legacy_inline.670799778692b0ce', 'Đọc tiếp') }}</a>
                                </div>
                            </article>
                        @empty
                            <p>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0305', app()->getLocale(), 'legacy_inline.63ca33d5dc39d4d6', 'Chưa có bài viết nào được xuất bản.') }}</p>
                        @endforelse
                    </section>
