...
<footer class="ec9-footer">
    <div class="ec9-container ec9-footer-grid">
        <section><h3>{{ $name }}</h3><p>{{ data_get($branding, 'company_description', __('EC900.footer.about')) }}</p><a href="mailto:{{ $email }}"><i class="fa-regular fa-envelope"></i>{{ $email }}</a><span><i class="fa-solid fa-location-dot"></i>{{ $location }}</span><a href="tel:{{ preg_replace('/\s+/', '', $hotline) }}"><i class="fa-solid fa-phone"></i>{{ $hotline }}</a></section>
