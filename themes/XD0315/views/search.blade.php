@php
    ...
    if (! $navItems->contains(fn (array $item): bool => in_array(mb_strtolower(trim($item['label'])), ['trang chủ', 'home'], true) || rtrim($item['href'], '/') === rtrim($homeUrl, '/'))) {
        $navItems->prepend([
            'label' => app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0315', app()->getLocale(), 'legacy_inline.95a078fb35ea9444', 'Trang chủ'),
            'href' => $homeUrl,
            'target' => '_self',
    ...
    if ($productNavigationItems->isNotEmpty()) {
        if ($hasProductItem) {
            ...
            array_splice($navArray, $homeIndex === false ? 0 : $homeIndex + 1, 0, [[
                'label' => app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0315', app()->getLocale(), 'legacy_inline.76ffae1228bd585e', 'Sản phẩm'),
    ...
    $footerNewsletterSource = 'theme-footer-XD0315-search';
@endphp
            ...
            <section class="xd-container xd-search-shell">
                <aside>
                    ...
                    <div class="xd-panel xd-promo">
                        ...
                        <p>Gửi từ khóa hoặc gọi hotline để đội ngũ XD0315 gợi ý nhóm sản phẩm phù hợp cho công trình.</p>
                    </div>
                </aside>
