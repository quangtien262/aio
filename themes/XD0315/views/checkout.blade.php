@php
    ...
    $footerNewsletterSource = 'theme-footer-XD0315-checkout';
@endphp

@extends('theme-xd0315::layout')

@section('title'){{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0315', app()->getLocale(), 'legacy_inline.e4d10558a6946e7d', 'Thanh toán') }} | {{ $logoAlt }}@endsection
        ...
        <main class="xd-page-main">
            <div class="xd-container">
                <nav class="xd-breadcrumb" aria-label="Breadcrumb">
                    <a href="{{ route('site.home') }}">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0315', app()->getLocale(), 'legacy_inline.c7ca2e1f76eac4d1', 'Trang chủ') }}</a>
                    <span>/</span>
                    <a href="{{ route('site.cart.index') }}">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0315', app()->getLocale(), 'legacy_inline.be92f1b271cbf9e7', 'Giỏ hàng') }}</a>
                    <span>/</span>
                    <strong>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0315', app()->getLocale(), 'legacy_inline.e4d10558a6946e7d', 'Thanh toán') }}</strong>
                </nav>

                <section class="xd-heading">
                    <div>
                        <span class="xd-kicker">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0315', app()->getLocale(), 'legacy_inline.e4d10558a6946e7d', 'Thanh toán') }}</span>
                        <h1>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0315', app()->getLocale(), 'legacy_inline.884062d9595eb222', 'Hoàn tất đơn hàng') }}</h1>
                        <p>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0315', app()->getLocale(), 'legacy_inline.555ac8abe9ea1db0', 'Xác nhận thông tin nhận hàng và phương thức thanh toán trước khi gửi đơn.') }}</p>
                    </div>
                </section>
                ...
                @if (empty($cartItems))
                    <section class="xd-empty">
                        <h2>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0315', app()->getLocale(), 'legacy_inline.c1dac840193b88c6', 'Giỏ hàng đang trống') }}</h2>
                        <p>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0315', app()->getLocale(), 'legacy_inline.a4a47b2511c36b6c', 'Vui lòng thêm sản phẩm trước khi thanh toán.') }}</p>
                        <a class="xd-button" href="{{ route('site.catalog.search') }}">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0315', app()->getLocale(), 'legacy_inline.f39fcbfc4b28b226', 'Xem sản phẩm') }}</a>
                    </section>
                @else
                    <section class="xd-checkout-grid">
                        <form class="xd-panel xd-form-panel" method="POST" action="{{ route('site.checkout.store') }}">
                            @csrf
                            <h2 class="xd-section-title">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0315', app()->getLocale(), 'legacy_inline.2787c8f1d8efe572', 'Thông tin nhận hàng') }}</h2>
                            <div class="xd-form-grid">
                                ...
                                <label class="xd-field">
                                    <span>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0315', app()->getLocale(), 'legacy_inline.fe4637d9642f9400', 'Số điện thoại') }}</span>
                                ...
                                <label class="xd-field is-wide">
                                    <span>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0315', app()->getLocale(), 'legacy_inline.56eabcd63ef1db88', 'Địa chỉ nhận hàng') }}</span>
                                    <textarea name="delivery_address" required>{{ old('delivery_address', $form['delivery_address'] ?? '') }}</textarea>
                                </label>
                                <label class="xd-field is-wide">
                                    <span>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0315', app()->getLocale(), 'legacy_inline.1e6a87fc00217087', 'Ghi chú') }}</span>
                                    <textarea name="note">{{ old('note', $form['note'] ?? '') }}</textarea>
                                </label>
                            </div>

                            <div class="xd-payment-list">
                                <h2 class="xd-section-title">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0315', app()->getLocale(), 'legacy_inline.2c9b97df9eb07c2c', 'Phương thức thanh toán') }}</h2>
                            ...
                            <button class="xd-button" type="submit">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0315', app()->getLocale(), 'legacy_inline.064cafd7b1a19604', 'Đặt hàng') }}</button>
                        </form>

                        <aside class="xd-panel xd-summary">
                            <h2>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0315', app()->getLocale(), 'legacy_inline.52cb65f9e638b3bd', 'Tóm tắt đơn hàng') }}</h2>
                            ...
                            <div class="xd-summary-row">
                                <span>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0315', app()->getLocale(), 'legacy_inline.dd68d4ac64b245b7', 'Số sản phẩm') }}</span>
                                <b>{{ (int) ($cartSummary['count'] ?? 0) }}</b>
                            </div>
                            <div class="xd-summary-row">
                                <span>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0315', app()->getLocale(), 'legacy_inline.a04d9edb9bde8e97', 'Tạm tính') }}</span>
                                <strong>{{ $formatCurrency($cartSummary['subtotal'] ?? 0) }}</strong>
                            </div>
                            <a class="xd-button is-ghost" href="{{ route('site.cart.index') }}">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0315', app()->getLocale(), 'legacy_inline.4536be0a49c9a62c', 'Quay lại giỏ hàng') }}</a>
                        </aside>
                    </section>
                @endif
            </div>
