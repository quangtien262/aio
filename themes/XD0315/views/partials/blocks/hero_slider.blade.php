        ...
        @foreach ($slides as $slide)
            <article class="af15-hero__slide {{ $loop->first ? 'is-active' : '' }}">
                ...
                <div class="af15-hero__content">
                    ...
                    <a class="af15-button af15-button--ghost" href="{{ $slide['link_url'] ?? '#lop-tap' }}">{{ $slide['button_label'] ?? $data['button_label'] ?? 'Dang ky tap thu' }}</a>
                </div>
            </article>
        @endforeach
        <button class="af15-hero__nav prev" type="button" data-af15-hero-prev aria-label="Slide truoc">&lsaquo;</button>
        <button class="af15-hero__nav next" type="button" data-af15-hero-next aria-label="Slide sau">&rsaquo;</button>
        <div class="af15-hero__dots">
            @foreach ($slides as $slide)
                <button class="af15-hero__dot {{ $loop->first ? 'is-active' : '' }}" type="button" data-af15-hero-dot="{{ $loop->index }}" aria-label="Slide {{ $loop->iteration }}"></button>
            @endforeach
        </div>
    </div>
</section>
