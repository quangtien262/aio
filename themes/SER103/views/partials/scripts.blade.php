    <script>
        (() => {
            ...
            const editorItemFields = (blockType) => {
                ...
                if (blockType === 'service_pricing') {
                    return [
                        ['title', 'Tên gói dịch vụ'],
                        ['price', 'Giá hiển thị'],
                        ['features', 'Quyền lợi (ngăn cách bằng dấu |)', 'textarea'],
                        ['icon', 'Class icon Font Awesome'],
                        ['featured', 'Nổi bật (nhập 1 hoặc để trống)'],
                    ];
                }

                if (blockType === 'business_service_grid') {
                    return [
                        ['title', 'Tiêu đề'],
                        ['summary', 'Mô tả', 'textarea'],
                        ['price', 'Giá hiển thị'],
                        ['type', 'Loại (product hoặc service)'],
                        ['image', 'Ảnh'],
                        ['url', 'Link'],
                    ];
                }

                if (blockType === 'collection_gallery') {
                    return [
                        ['title', 'Tiêu đề quảng cáo'],
                        ['summary', 'Thông điệp ngắn'],
                        ['badge', 'Nhãn nhỏ'],
                        ['image', 'Ảnh'],
                        ['url', 'Link'],
                    ];
...
<script>
(() => {
    const menuToggle = document.querySelector('[data-ser103-menu-toggle]');
    const menu = document.querySelector('[data-ser103-menu]');
    const header = document.querySelector('[data-ser103-header]');
    const modal = document.querySelector('[data-ser103-booking]');
    const form = document.querySelector('[data-ser103-booking-form]');
    const feedback = document.querySelector('[data-ser103-booking-feedback]');
    const serviceField = document.querySelector('[data-ser103-service]');
    const dateField = document.querySelector('[data-ser103-date]');
    const timeField = document.querySelector('[data-ser103-time]');
    const noteField = document.querySelector('[data-ser103-note]');
    const messageField = document.querySelector('[data-ser103-booking-message]');
    const csrf = document.querySelector('meta[name="csrf-token"]')?.content || '';
    ...
    const openModal = (service = '') => {
        ...
        if (service && serviceField) {
            const knownOption = [...serviceField.options].some((option) => option.value === service);
            if (!knownOption) serviceField.add(new Option(service, service));
            serviceField.value = service;
        }
        modal.hidden = false;
        document.body.classList.add('ser103-modal-open');
        window.setTimeout(() => serviceField?.focus(), 80);
    };
    const closeModal = () => {
        if (!modal) return;
        modal.hidden = true;
        document.body.classList.remove('ser103-modal-open');
    };
    document.addEventListener('click', (event) => {
        const trigger = event.target.closest('[data-ser103-booking-open]');
        if (trigger) {
            event.preventDefault();
            openModal(trigger.dataset.service || '');
            return;
        }
        if (event.target.closest('[data-ser103-booking-close]')) closeModal();
    });
    ...
    form?.addEventListener('submit', async (event) => {
        ...
        const service = String(serviceField?.value || '').trim();
        ...
        const errors = {};
        if (!service) errors.service = 'Vui lòng chọn dịch vụ.';
        ...
        if (!/^\S+@\S+\.\S+$/.test(email)) errors.email = 'Email chưa hợp lệ.';
        ...
        messageField.value = `Dịch vụ: ${service}\nNgày hẹn: ${date}\nGiờ hẹn: ${time}\nGhi chú: ${String(noteField?.value || 'Không có')}`;
        const submit = form.querySelector('button[type="submit"]');
        submit.disabled = true;
        try {
            const payload = Object.fromEntries(new FormData(form).entries());
            const response = await fetch(@json(route('site.contact.submit')), {
                method: 'POST',
            ...
            form.reset();
            if (dateField) dateField.min = new Date().toISOString().slice(0, 10);
            setFeedback(@json(app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('SER103', app()->getLocale(), 'SER103.booking.success')));
        } catch (error) {
            Object.entries(error?.errors || {}).forEach(([key, messages]) => {
                const node = form.querySelector(`[data-error="${key}"]`);
                if (node) node.textContent = Array.isArray(messages) ? messages[0] : messages;
            });
            setFeedback(error?.message || @json(app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('SER103', app()->getLocale(), 'SER103.booking.error')), true);
        } finally {
...
(() => {
    ...
    const revealNodes = [...document.querySelectorAll('[data-ser103-reveal]')];
    ...
    const hero = document.querySelector('[data-ser103-hero]');
    if (!hero) return;
    const slides = [...hero.querySelectorAll('[data-ser103-slide]')];
    const dots = [...hero.querySelectorAll('[data-ser103-dot]')];
    const content = @json($heroSlides ?? []);
    let current = 0;
    const show = (next) => {
        ...
        set('[data-ser103-kicker]', item.kicker);
        set('[data-ser103-title]', item.title);
        set('[data-ser103-summary]', item.summary);
        const link = hero.querySelector('[data-ser103-link]');
        if (link) {
            if (item.link_url) link.href = item.link_url;
            if (item.button_label) link.childNodes[0].textContent = item.button_label + ' ';
        }
    };
    dots.forEach((dot, index) => dot.addEventListener('click', () => show(index)));
    if (slides.length > 1) window.setInterval(() => show(current + 1), Math.max(3000, Number(hero.dataset.autoplay || 6000)));
})();
</script>
