...
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box}body{margin:0;color:var(--ink);background:var(--bg);font-family:var(--font);font-size:16px;line-height:1.7}a{text-decoration:none;color:inherit}img{display:block;max-width:100%}button,input,select{font:inherit}.xd-container{width:min(1540px,calc(100% - 56px));margin:0 auto}
        ...
        .xd-product-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:24px}.xd-product-card{overflow:hidden;transition:transform .18s ease,box-shadow .18s ease}.xd-product-card:hover{transform:translateY(-4px);box-shadow:var(--shadow)}.xd-product-image{position:relative;aspect-ratio:4/3;background:#eef2ef;overflow:hidden}.xd-product-image img{width:100%;height:100%;object-fit:cover;transition:transform .35s ease}.xd-product-card:hover img{transform:scale(1.04)}.xd-product-tag{position:absolute;left:14px;bottom:14px;padding:6px 10px;background:rgba(13,28,38,.82);color:#fff;font-size:12px;font-weight:900;text-transform:uppercase}.xd-product-body{padding:22px}.xd-product-body h3{margin:0 0 12px;font-size:21px;line-height:1.25;letter-spacing:-.025em}.xd-product-body p{margin:0 0 18px;color:var(--muted);font-weight:650}.xd-price-row{display:flex;align-items:center;gap:10px;flex-wrap:wrap}.xd-price{color:#9a6a3e;font-size:24px;font-weight:950;letter-spacing:-.04em}.xd-old-price{color:#9aa3a9;text-decoration:line-through}.xd-discount{display:inline-flex;align-items:center;height:26px;padding:0 8px;border-radius:999px;background:var(--ink);color:#fff;font-size:12px;font-weight:900}.xd-empty{padding:42px;background:#fff;border:1px solid var(--line);color:var(--muted);font-weight:750}
                            ...
                            @foreach ($productItems as $product)
                                <a class="xd-product-card" href="{{ $product['url'] ?? '#' }}">
                                    ...
                                    <span class="xd-product-body">
                                        ...
                                        @if (filled($product['summary'] ?? null))
                                            <p>{{ \Illuminate\Support\Str::limit(strip_tags((string) $product['summary']), 112) }}</p>
                                        @endif
