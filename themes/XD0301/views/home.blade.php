@php
    ...
    if ($blocks->isEmpty()) {
        $blocks = collect([
            ...
            ['id' => null, 'block_type' => 'about_experience', 'anchor_id' => 'gioi-thieu', 'is_visible' => true, 'settings' => ['years' => 10], 'media' => ['image' => 'https://images.unsplash.com/photo-1518005020951-eccb494ad742?auto=format&fit=crop&w=1000&q=85'], 'data' => ['subtitle' => 'Giới thiệu', 'title' => 'Thiết kế và thi công Nhà ở, Tòa nhà văn phòng.', 'description' => 'ARKIT là công ty chuyên về thiết kế và thi công. Được thành lập và phát triển bởi các kiến trúc sư, kỹ sư nhiều năm kinh nghiệm.', 'button_label' => 'Tìm hiểu thêm', 'content' => ['body' => 'Chúng tôi phối hợp nhịp nhàng giữa chủ đầu tư, kiến trúc, kết cấu và nhà thầu để đưa ra giải pháp tối ưu, bền vững và phù hợp ngân sách.']]],
        ]);
    }
    ...
    $normalizeNavItem = function (array $item, int $index = 0) use (&$normalizeNavItem): array {
...
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box} html{scroll-behavior:smooth} body{margin:0;color:var(--ink);background:#fff;font-family:var(--font);font-size:16px;line-height:1.75} a{text-decoration:none;color:inherit} img{display:block;max-width:100%}
        ...
        .xd-projects{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px}.xd-project-card{position:relative;min-height:540px;overflow:hidden;color:#fff;background:#101d28}.xd-project-card img{width:100%;height:100%;object-fit:cover;transition:.5s}.xd-project-card:after{content:"";position:absolute;inset:0;background:linear-gradient(180deg,transparent 45%,rgba(6,13,18,.88))}.xd-project-card:hover img{transform:scale(1.06)}.xd-project-caption{position:absolute;z-index:2;left:36px;right:28px;bottom:34px}.xd-project-caption small{display:block;margin-bottom:12px;font-size:15px;font-weight:900;text-transform:uppercase}.xd-project-caption h3{margin:0;font-size:28px;line-height:1.2}
...
<body>
    <div id="top" class="xd-page">
                ...
                @switch($block['block_type'])
                    ...
                    @case('about_experience')
                        <section id="{{ $anchor }}" class="xd-section xd-landing-block" data-landing-block-id="{{ $block['id'] }}" data-block-type="{{ $block['block_type'] }}">
                            {!! $editButton !!}
                            <div class="xd-container xd-intro"><div class="xd-intro-copy"><h2>{!! nl2br(e($data['title'] ?? '')) !!}</h2><p>{{ $data['description'] ?? '' }}</p>@if(filled($content['body'] ?? null))<p>{{ $content['body'] }}</p>@endif<a class="xd-button" href="#dich-vu">{{ $data['button_label'] ?? 'Tìm hiểu thêm' }}</a></div><div class="xd-intro-visual"><div class="xd-years"><strong>{{ $settings['years'] ?? 10 }}</strong><span>Năm kinh nghiệm</span></div><img src="{{ $media['image'] ?? '' }}" alt="{{ $data['title'] ?? 'Giới thiệu' }}"></div></div>
                        </section>
                        @break
        ...
        <footer id="{{ data_get($footerBlock, 'anchor_id', 'lien-he') }}" class="xd-footer xd-landing-block" data-landing-block-id="{{ data_get($footerBlock, 'id') }}" data-block-type="footer_contact">
            ...
            <div class="xd-container xd-footer-grid">
                ...
                <div><h3>Thông tin</h3><nav class="xd-footer-links" aria-label="Thông tin">@foreach ($navItems as $item)<a href="{{ $item['href'] }}">{{ $item['label'] }}</a>@endforeach</nav></div>
    ...
    <script>
        (() => {
            ...
            const renderEditorItem = (item = {}, index = 0, blockType = '') => {
                ...
                row.innerHTML = `
                    ...
                    <div class="xd-editor-item-actions">
            ...
            const renderItemsEditor = (block, contentOverride = null) => {
                ...
                const canEditList = ['hero_slider', 'featured_services', 'project_gallery', 'team_members', 'testimonials', 'partner_logos'].includes(blockType);
                ...
                itemHelp.textContent = blockHasDynamicItems(block)
                    ? 'Khối này đang lấy danh sách từ dữ liệu CMS. Nếu muốn đổi từng mục, nên sửa trong module CMS tương ứng; form này chủ yếu dùng cho block tĩnh.'
                    : 'Chỉnh từng mục bằng form, hệ thống sẽ tự chuyển về cấu trúc lưu phù hợp.';

                items.forEach((item, index) => itemList.appendChild(renderEditorItem(item, index, blockType)));
                if (!items.length) itemList.appendChild(renderEditorItem({}, 0, blockType));
            ...
            const previewSourceItems = async () => {
                ...
                try {
                    ...
                    const nextContent = {...(parseJson(field('content').value, {}) || {})};
                    nextContent[activeItemKey || editorItemKey(activeBlock.block_type)] = items;
                    field('content').value = pretty(nextContent);
                    renderItemsEditor(activeBlock, nextContent);
                } catch (error) {
            ...
            itemForm?.addEventListener('submit', (event) => {
                ...
                const blockType = activeBlock.block_type || addItemButton?.dataset.xdBlockType || '';
                if (indexValue === '') {
                    itemList.appendChild(renderEditorItem(item, rows.length, blockType));
                } else {
                    const index = Number(indexValue);
                    rows[index]?.replaceWith(renderEditorItem(item, index, blockType));
                }
                syncItemNumbers();
                closeItemModal();
            });
