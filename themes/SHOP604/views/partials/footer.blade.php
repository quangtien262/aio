...
$phone=$branding['support_hotline']??'';$email=$branding['support_email']??'';$address=$branding['support_location']??'';$name=trim((string)($branding['company_name']??''))?:$t('SHOP604.brand.default');$logo=trim((string)($branding['logo_url']??''));
...
<footer class="s604-footer"><div class="s604-footer-grid">
    <section class="s604-footer-brand"><a class="s604-logo s604-logo-light" href="{{ route('site.home') }}">@if($logo)<img src="{{ $logo }}" alt="{{ $name }}">@endif</a><p>{{ $t('SHOP604.footer.tagline') }}</p><p><i class="fa-solid fa-location-dot"></i> {{ $address }}</p><p><i class="fa-solid fa-phone"></i> <b>{{ $phone }}</b></p><p><i class="fa-regular fa-envelope"></i> <b>{{ $email }}</b></p></section>
