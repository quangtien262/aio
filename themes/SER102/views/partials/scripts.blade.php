...
<script>
(() => {
    const menuToggle = document.querySelector('[data-ser102-menu-toggle]');
    const menu = document.querySelector('[data-ser102-menu]');
    const header = document.querySelector('[data-ser102-header]');
    const modal = document.querySelector('[data-ser102-booking]');
    const form = document.querySelector('[data-ser102-booking-form]');
    const feedback = document.querySelector('[data-ser102-booking-feedback]');
    const serviceField = document.querySelector('[data-ser102-service]');
    const dateField = document.querySelector('[data-ser102-date]');
    const timeField = document.querySelector('[data-ser102-time]');
    const noteField = document.querySelector('[data-ser102-note]');
    const messageField = document.querySelector('[data-ser102-booking-message]');
    const csrf = document.querySelector('meta[name="csrf-token"]')?.content || '';
    ...
    const openModal = (service = '') => {
        ...
        modal.hidden = false;
        document.body.classList.add('ser102-modal-open');
        window.setTimeout(() => serviceField?.focus(), 80);
    };
    const closeModal = () => {
        if (!modal) return;
        modal.hidden = true;
        document.body.classList.remove('ser102-modal-open');
    };
    document.addEventListener('click', (event) => {
        const trigger = event.target.closest('[data-ser102-booking-open]');
        if (trigger) {
            event.preventDefault();
            openModal(trigger.dataset.service || '');
            return;
        }
        if (event.target.closest('[data-ser102-booking-close]')) closeModal();
    });
    ...
    form?.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            form.reset();
            if (dateField) dateField.min = new Date().toISOString().slice(0, 10);
            setFeedback(@json(app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('SER102', app()->getLocale(), 'SER102.booking.success')));
        } catch (error) {
            Object.entries(error?.errors || {}).forEach(([key, messages]) => {
                const node = form.querySelector(`[data-error="${key}"]`);
                if (node) node.textContent = Array.isArray(messages) ? messages[0] : messages;
            });
            setFeedback(error?.message || @json(app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('SER102', app()->getLocale(), 'SER102.booking.error')), true);
        } finally {
