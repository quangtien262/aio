...
<div class="sf-language-mobile-slot">@include('partials.storefront-language-switcher')</div>
    <div class="ec93-utility"><div class="ec93-container"><span><i class="fa-solid fa-location-dot"></i> {{ data_get($branding, 'support_location', '') }} <i class="fa-solid fa-caret-down"></i></span><a href="#newsletter"><i class="fa-regular fa-envelope"></i> Đăng ký bản tin</a><div><a href="tel:{{ preg_replace('/\s+/', '', $hotline) }}"><i class="fa-solid fa-phone"></i> Hotline: <b>{{ $hotline }}</b></a><a href="mailto:{{ $email }}"><i class="fa-solid fa-envelope"></i> Email: {{ $email }}</a><button type="button" data-auth-open="register"><i class="fa-regular fa-user"></i> Đăng ký</button><button type="button" data-auth-open="login">Đăng nhập</button></div>
            @include('partials.storefront-language-switcher')
        </div></div>
    <div class="ec93-head-main"><div class="ec93-container"><a class="ec93-logo" href="{{ route('site.home') }}">@if($logo)<img src="{{ $logo }}" alt="{{ data_get($siteProfile ?? [], 'site_name') }}">@endif</a><form action="{{ route('site.catalog.search') }}" method="get"><select aria-label="Danh mục"><option>Tất cả danh mục</option><option>Ẩm thực</option><option>Spa & Làm đẹp</option><option>Du lịch</option></select><input name="q" placeholder="Tìm kiếm sản phẩm / khuyến mãi"><button aria-label="Tìm kiếm"><i class="fa-solid fa-magnifying-glass"></i></button></form><a class="ec93-cart" href="{{ route('site.cart.index') }}"><i class="fa-solid fa-cart-shopping"></i><b>{{ collect($cartLines ?? [])->sum('quantity') }}</b><span>GIỎ HÀNG</span></a></div></div>
