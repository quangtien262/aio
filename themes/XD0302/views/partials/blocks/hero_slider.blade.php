@php
    ...
    $hasSlideText = $slides->contains(function (array $slide): bool {
        $hasCopy = collect([
            $slide['kicker'] ?? null,
            $slide['title'] ?? null,
            $slide['summary'] ?? null,
        ])->contains(fn ($value) => filled($value));
        $hasButton = filled($slide['button_label'] ?? null) && filled($slide['link_url'] ?? null);

        return $hasCopy || $hasButton;
    });
@endphp
<section id="{{ $anchor }}" class="xd2-hero xd-landing-block {{ $hasSlideText ? 'xd2-hero--has-text' : '' }}" data-landing-block-id="{{ $block['id'] }}" data-block-type="hero_slider" data-xd2-hero data-autoplay="{{ $settings['autoplay_ms'] ?? 6000 }}">
    {!! $editButton !!}
    @foreach ($slides as $slide)
        @php
            $slideButtonLabel = trim((string) ($slide['button_label'] ?? ''));
            $slideButtonUrl = trim((string) ($slide['link_url'] ?? ''));
        @endphp
        <article class="xd2-hero__slide {{ $loop->first ? 'is-active' : '' }}"><img src="{{ $slide['image'] }}" alt="{{ $slide['alt'] ?? $slide['title'] ?? 'Banner' }}"><div class="xd2-container xd2-hero__copy"><div><p>{{ $slide['kicker'] ?? '' }}</p><h1>{{ $slide['title'] ?? '' }}</h1><p>{{ $slide['summary'] ?? '' }}</p>@if (filled($slideButtonLabel) && filled($slideButtonUrl))<a class="xd2-button" href="{{ $slideButtonUrl }}">{{ $slideButtonLabel }} <span>→</span></a>@endif</div></div></article>
    @endforeach
