@php
    ...
    if ($blocks->isEmpty()) {
        $blocks = collect([
            ['id' => null, 'block_type' => 'hero_slider', 'anchor_id' => 'top', 'is_visible' => true, 'settings' => ['autoplay_ms' => 5200], 'media' => [], 'data' => ['subtitle' => 'Trang chủ', 'title' => 'Xây dựng ngôi nhà mơ ước', 'description' => 'Từ bản vẽ, vật liệu đến thi công hoàn thiện, XD0302 giúp chủ đầu tư kiểm soát chất lượng và tiến độ rõ ràng.', 'button_label' => 'Xem dự án →', 'content' => ['slides' => [
                ['kicker' => 'Residential', 'title' => 'Xây dựng ngôi nhà mơ ước', 'summary' => 'Từ bản vẽ, vật liệu đến thi công hoàn thiện, XD0302 giúp chủ đầu tư kiểm soát chất lượng và tiến độ rõ ràng.', 'image' => 'https://images.unsplash.com/photo-1504917595217-d4dc5ebe6122?auto=format&fit=crop&w=1920&q=85'],
    ...
    $heroSlides = collect(data_get($hero, 'data.content.slides', []))
        ...
        ->values();
    $heroSlides = $heroSlides->isNotEmpty() ? $heroSlides : collect([['kicker' => 'Residential', 'title' => 'Xây dựng ngôi nhà mơ ước', 'summary' => 'XD0302 Construction Landing', 'image' => 'https://images.unsplash.com/photo-1504917595217-d4dc5ebe6122?auto=format&fit=crop&w=1920&q=85']]);
    ...
    $homeLabel = app(\App\Core\Themes\ThemeTranslationService::class)
        ->bladeText((string) data_get($activeTheme ?? [], 'key', 'XD0302'), app()->getLocale(), 'common.home', 'Trang chủ');
...
@section('title', data_get($landingPage ?? [], 'meta_title') ?: data_get($landingPage ?? [], 'title', 'XD0302 Construction Landing'))
...
<main>
    @foreach ($blocks->where('block_type', '!=', 'footer_contact') as $block)
        ...
        @includeIf('theme-xd0302::partials.blocks.'.($block['block_type'] ?? ''))
    @endforeach
</main>
@endsection
