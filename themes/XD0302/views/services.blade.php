...
<main class="xd-page-main">
            <div class="xd-container">
                    <section class="xd-cms-hero">
                        <div>
                            <span class="xd-kicker">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0302', app()->getLocale(), 'legacy_inline.f7d5c6216287dbca', 'Dịch vụ') }}</span>
                            <h1>{{ $pageTitle ?? (app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0302', app()->getLocale(), 'legacy_inline.f7d5c6216287dbca', 'Dịch vụ')) }}</h1>
                            <p>{{ $pageDescription ?? (app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0302', app()->getLocale(), 'legacy_inline.cc4da477e0430213', 'Danh sách dịch vụ thiết kế và thi công nổi bật.')) }}</p>
                        </div>
                        <div class="xd-cms-stats">
                            <strong>{{ collect($listingItems ?? [])->count() }}</strong>
                            <span>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0302', app()->getLocale(), 'legacy_inline.82fb2e9e0ebce8c4', 'Dịch vụ đang hiển thị') }}</span>
                        </div>
                    </section>
                        ...
                        @forelse ($listingItems as $service)
                            ...
                            <article class="xd-service-card">
                                ...
                                <div class="xd-service-body">
                                    ...
                                    <a class="xd-text-link" href="{{ $serviceUrl }}">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0302', app()->getLocale(), 'legacy_inline.32919a31e26bbb86', 'Tìm hiểu ngay') }}</a>
                                </div>
                            </article>
                        @empty
                            <p>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0302', app()->getLocale(), 'legacy_inline.f2320a5706a9550e', 'Chưa có dịch vụ nào được xuất bản.') }}</p>
                        @endforelse
                    </section>
            </div>
</main>
