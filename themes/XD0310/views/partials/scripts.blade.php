    ...
    (() => {
        ...
        revealGroups.forEach((group) => {
            ...
            targets.forEach((target, index) => {
                target.classList.add('xd10-reveal');
                target.style.setProperty('--xd10-delay', `${Math.min(index * 90, 360)}ms`);
                revealItems.push(target);
            });
        });

        document.querySelectorAll('.xd5-section > .xd5-container > .xd5-eyebrow, .xd5-team-head, .xd-section-title')
            .forEach((target) => {
                target.classList.add('xd10-reveal');
                revealItems.push(target);
            });
        ...
        document.documentElement.classList.add('xd10-motion-ready');
