...
@foreach($blocks as $block)
        ...
        <section class="bds-hero xd-landing-block" data-block-type="{{ $type }}" style="background-image:url('{{ data_get($block, 'media.image') }}')">
            <div class="bds-container bds-hero-content">
                ...
                <form class="bds-search" method="GET" action="{{ route('site.real-estate.index', ['locale' => app()->getLocale()]) }}">
                    <select name="transaction_type"><option value="sale">@themeT('sale', 'Bán')</option><option value="rent">@themeT('rent', 'Cho thuê')</option></select>
                    <input name="q" placeholder="@themeT('search_placeholder', 'Tìm dự án, biệt thự, căn hộ...')">
                    <span class="bds-search-more"><i class="fa-solid fa-sliders"></i>&nbsp; @themeT('advanced', 'Nâng cao')</span>
                    <button><i class="fa-solid fa-magnifying-glass"></i> {{ data_get($block, 'data.button_label', app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('BDS701', app()->getLocale(), 'search', 'Tìm kiếm nhanh')) }}</button>
                </form>
                <p class="bds-quick-label">@themeT('home.quick_type', 'Tìm nhanh theo kiểu dáng')</p>
    ...
    @elseif(in_array($type, ['bds701_latest_listings','bds701_rental_listings']))
        <section id="{{ data_get($block, 'anchor_id') }}" class="bds-section soft xd-landing-block" data-block-type="{{ $type }}">
            <div class="bds-container">
                <header class="bds-heading"><div><h2><em>@themeT('home.project', 'Dự án')</em> {{ $type === 'bds701_rental_listings' ? app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('BDS701', app()->getLocale(), 'home.for_rent', 'cho thuê') : app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('BDS701', app()->getLocale(), 'home.latest', 'mới nhất') }}</h2><p>{{ data_get($block, 'data.subtitle') }}</p></div>
                </header>
    ...
    @elseif($type === 'bds701_property_types')
        <section id="{{ data_get($block, 'anchor_id') }}" class="bds-section xd-landing-block" data-block-type="{{ $type }}"><div class="bds-container">
            <header class="bds-heading" style="justify-content:center;text-align:center"><div><h2><em>@themeT('home.featured_model', 'Mẫu dự án')</em> @themeT('home.featured', 'tiêu biểu')</h2><p>{{ data_get($block, 'data.subtitle') }}</p></div></header>
    ...
    @elseif($type === 'bds701_market_news')
        ...
        <section id="{{ data_get($block, 'anchor_id') }}" class="bds-section xd-landing-block" data-block-type="{{ $type }}"><div class="bds-container">
            <header class="bds-heading"><div><h2><em>@themeT('home.market_news', 'Tin tức')</em> @themeT('home.market', 'thị trường')</h2><p>{{ data_get($block, 'data.subtitle') }}</p></div></header>
    ...
    @elseif($type === 'bds701_latest_news')
        <section id="{{ data_get($block, 'anchor_id') }}" class="bds-section xd-landing-block" data-block-type="{{ $type }}"><div class="bds-container">
            <header class="bds-heading"><div><h2><em>@themeT('home.real_estate_news', 'Tin bất động sản mới')</em></h2><p>{{ data_get($block, 'data.subtitle') }}</p></div></header>
    ...
    @elseif($type === 'bds701_newsletter')
        <section id="{{ data_get($block, 'anchor_id') }}" class="bds-newsletter xd-landing-block" data-block-type="{{ $type }}" style="background-image:url('{{ data_get($block, 'media.image') }}')"><div class="bds-container"><div class="bds-newsletter-card">
            ...
            <form method="POST" action="{{ route('site.newsletter.subscribe') }}">@csrf<input type="hidden" name="source" value="bds701-home"><input type="email" name="email" required placeholder="@themeT('home.email_placeholder', 'Nhập địa chỉ email của bạn...')"><button>{{ data_get($block, 'data.button_label', app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('BDS701', app()->getLocale(), 'home.newsletter_button', 'Nhận tin miễn phí')) }}</button></form>
        </div></div></section>
    @endif
@endforeach
</main>
...
(() => {
    ...
    document.querySelectorAll('.xd-landing-block[data-block-type]').forEach((section) => {
        ...
        const button = document.createElement('button'); button.type='button'; button.className='xd-edit-block'; button.dataset.xdEditBlock=id; button.textContent={{ \Illuminate\Support\Js::from(app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('BDS701', app()->getLocale(), 'home.edit_block', 'Sửa khối')) }}; section.appendChild(button);
    });
})();
</script>
