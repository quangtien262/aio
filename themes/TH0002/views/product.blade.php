@php
    ...
    $primaryImage = $gallery[0]['url'] ?? ($product['image'] ?? 'https://picsum.photos/seed/th0002-product-fallback/960/720');
    ...
    $orderGuideSteps = [
        [
            'step' => '01',
            'title' => $t('product.guide_step_1_title', 'Chọn mẫu và form dáng'),
            'body' => $t('product.guide_step_1_body', 'Xác định kiểu áo, chất liệu, màu sắc và số lượng để xưởng tư vấn cấu hình phù hợp cho đơn sỉ hoặc lẻ.'),
        ],
        [
            'step' => '02',
            'title' => $t('product.guide_step_2_title', 'Duyệt thông số và báo giá'),
            'body' => $t('product.guide_step_2_body', 'Xưởng xác nhận bảng size, chi tiết in thêu, thời gian mẫu và gửi báo giá theo đúng cấu hình đã chốt.'),
        ],
        [
            'step' => '03',
            'title' => $t('product.guide_step_3_title', 'Đặt cọc và vào chuyền'),
            'body' => $t('product.guide_step_3_body', 'Sau khi duyệt mẫu hoặc xác nhận mockup, đơn sẽ được lên lịch sản xuất và cập nhật tiến độ theo từng giai đoạn.'),
        ],
        [
            'step' => '04',
            'title' => $t('product.guide_step_4_title', 'Kiểm hàng và giao đơn'),
            'body' => $t('product.guide_step_4_body', 'Xưởng kiểm hoàn thiện, đóng gói theo size hoặc theo điểm nhận rồi bàn giao cho khách lẻ, đại lý hoặc doanh nghiệp.'),
        ],
    ];
    $orderGuideNotes = [
        $t('product.guide_note_1', 'Nên thống nhất bảng size, logo in thêu và số lượng từng màu trước khi duyệt báo giá cuối cùng.'),
        $t('product.guide_note_2', 'Với đơn sỉ, nên chốt trước quy cách đóng gói, tem nhãn và lịch giao nhiều đợt nếu cần.'),
        $t('product.guide_note_3', 'Nếu cần làm mẫu nhanh hoặc chỉnh rập, hãy ghi rõ trong phần ghi chú để đội ngũ kỹ thuật ưu tiên xử lý.'),
    ];
    $productionSnapshot = [
        ['label' => $t('product.snapshot_category', 'Nhóm line'), 'value' => $product['tag'] ?? $t('product.snapshot_category_fallback', 'Atelier ready')],
        ['label' => $t('product.snapshot_stock', 'Số lượng khả dụng'), 'value' => number_format((int) ($product['meta'] ?? 0), 0, ',', '.').' pcs'],
        ['label' => $t('product.snapshot_sales', 'Mẫu đã chốt'), 'value' => number_format($soldCount, 0, ',', '.').' đơn'],
        ['label' => $t('product.snapshot_leadtime', 'Thời gian hoàn thiện mẫu'), 'value' => $deadline ? $t('product.snapshot_leadtime_value', '7-12 ngày') : $t('product.snapshot_leadtime_open', 'Theo cấu hình')],
    ];
    ...
    if ($detailParagraphsList === []) {
        $detailParagraphsList = [
            $productModel->short_description ?: $t('product.detail_default_1', 'Sản phẩm này đang dùng mô tả mặc định. Sếp có thể cập nhật chi tiết chất liệu, form dáng và thông số ngay trong admin Catalog.'),
            $t('product.detail_default_2', 'Trang chi tiết TH0002 hỗ trợ gallery nhiều ảnh, mô tả quy cách may và luồng đặt hàng để phù hợp cả bán sỉ lẫn bán lẻ.'),
        ];
    }
@endphp
<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ $product['title'] }} | {{ data_get($branding, 'company_name', 'TH0002') }}</title>
        ...
        <style>
            ...
            * { box-sizing: border-box; }
            ...
            .flash-banner { margin: 0 0 18px; padding: 14px 16px; border: 1px solid #f0d2a3; background: #fff7e8; color: #925715; font-size: 14px; }
            ...
            .nav-category-panel .th-sidebar-entry:hover .th-sidebar-item { color: var(--th-red); background: #fff5ec; }
            ...
            .btn-secondary { border: 1px solid #d9a46f; color: var(--th-green-dark); background: #fff; min-width: 184px; }
            .btn-favorite { display: inline-flex; align-items: center; justify-content: center; min-height: 48px; padding: 0 18px; border: 1px solid #efcfad; background: #fff; color: #a85618; font-weight: 700; cursor: pointer; }
            .btn-favorite.is-active { background: #fff1e4; border-color: #d67a2c; color: #8b4a18; }
            ...
            .guide-hero { display: grid; grid-template-columns: minmax(0, 1.15fr) minmax(240px, .85fr); gap: 18px; padding: 18px; border: 1px solid #f1dcc0; background: linear-gradient(135deg, #fffaf3 0%, #fff2e1 100%); }
            .guide-hero-copy h3 { margin: 0 0 10px; font-size: 24px; color: #8a4e16; }
            .guide-hero-copy p { margin: 0; color: #9a6737; line-height: 1.8; }
            ...
            .hero-atelier .panel, .section-panel, .sidebar-card { border-radius: 28px; border-color: #e7d7ca; box-shadow: 0 22px 50px rgba(93, 74, 58, 0.08); }
            .hero-atelier { align-items: start; }
            .gallery-panel { background: linear-gradient(180deg, #fffaf5 0%, #f3e6db 100%); }
            .gallery-stage { border-radius: 22px; border-color: #e3d2c3; background: linear-gradient(135deg, #f7ece4 0%, #fffaf5 100%); }
            .gallery-thumb { border-radius: 16px; border-color: #e0d3c8; }
            .gallery-thumb.is-active { border-color: #a35035; box-shadow: inset 0 0 0 1px #a35035; }
            .info-panel { background: linear-gradient(160deg, rgba(255,248,241,.98) 0%, rgba(247,237,229,.96) 100%); }
            .atelier-kicker { display: inline-flex; align-items: center; gap: 8px; min-height: 34px; padding: 0 14px; border-radius: 999px; background: rgba(163,80,53,.1); color: #8f442c; font-size: 12px; font-weight: 800; letter-spacing: .14em; text-transform: uppercase; }
            .atelier-kicker::before { content: ''; width: 8px; height: 8px; border-radius: 50%; background: #a35035; }
            .title { margin-top: 14px; color: #2f231e; }
            .share { margin-top: 10px; color: #8d6f5b; text-transform: uppercase; letter-spacing: .12em; font-size: 11px; }
            .summary { color: #5f4a3f; font-size: 16px; }
            .price-box { border-color: #eadbcf; }
            .deal-price { color: #8f442c; }
            .discount-badge { background: #7f3a25; border-radius: 999px; min-width: 62px; }
            .offer-row { grid-template-columns: minmax(0, 1fr) 220px; gap: 18px; }
            .benefit-box { gap: 14px; }
            .moneyback { border-radius: 999px; background: rgba(110,138,98,.14); border-color: rgba(64,107,82,.18); color: #35533f; }
            .purchase-line select { border-radius: 12px; border-color: #dbc9bc; }
            .qr-box.atelier-brief { display: grid; gap: 14px; padding: 16px; border-radius: 22px; border-color: #e0cdbf; background: rgba(255,255,255,.82); text-align: left; }
            .atelier-brief-label { display: inline-flex; align-items: center; gap: 8px; color: #8f442c; font-size: 12px; font-weight: 800; text-transform: uppercase; letter-spacing: .14em; }
            .atelier-brief-grid { display: grid; gap: 10px; }
            .atelier-brief-item { padding: 10px 12px; border-radius: 16px; background: #fff8f1; border: 1px solid #ecddd2; }
            .atelier-brief-item strong { display: block; margin-bottom: 5px; font-size: 12px; color: #9c7a65; text-transform: uppercase; letter-spacing: .08em; }
            .atelier-brief-item span { color: #3f3028; font-weight: 700; font-size: 14px; }
            .btn-primary, .btn-secondary, .btn-favorite { border-radius: 999px; }
            .btn-primary { background: linear-gradient(135deg, #c96d24 0%, #a85618 100%); }
            .btn-secondary { border-color: #b8875e; color: #8f442c; }
            .stats { margin-top: 16px; padding: 16px 18px 0; border-top: 1px solid #eadbcf; color: #6f5849; }
            .section-title { padding-top: 18px; font-size: 13px; letter-spacing: .18em; color: #8f442c; }
            .section-body { padding-top: 18px; }
            .tabs { padding: 0 18px; gap: 10px; border-bottom: 0; background: transparent; }
            .tab-button { margin-top: 16px; border-radius: 999px; background: #f2e6dc; color: #8a6e5d; padding: 12px 18px; }
            .tab-button.is-active { background: #7f3a25; color: #fff8f1; border-top: 0; }
            .tab-panel { padding-top: 18px; }
            .guide-hero { border: 1px solid #ead8ca; border-radius: 24px; background: linear-gradient(135deg, #fffaf5 0%, #f2e6da 100%); }
            .guide-hero-copy h3, .guide-note-card h4, .guide-support-card h4, .sidebar-card h3 { color: #2f231e; }
            .guide-badge { background: rgba(163,80,53,.1); color: #8f442c; }
            .guide-step-card { border-radius: 20px; border-color: #eadbcf; background: #fffaf5; }
            .guide-step-badge { background: #7f3a25; border-radius: 12px; }
            .guide-note-card, .guide-support-card { border-radius: 24px; border-color: #eadbcf; }
            .sidebar-card { position: sticky; top: 18px; background: linear-gradient(180deg, #fff8f1 0%, #f7ece3 100%); }
            .sidebar-related { gap: 12px; }
            .sidebar-related-card { padding: 12px; border-radius: 18px; border: 1px solid #eadbcf; background: rgba(255,255,255,.9); }
            .sidebar-related-card img { border-radius: 16px; border-color: #e8d6c8; }
        ...
        <header class="header">
            <div class="wrap header-main">
                <a class="brand" href="{{ route('site.home') }}">
                    <img src="{{ data_get($branding, 'logo_url', 'https://htvietnam.vn/images/logo/logo_vn_noslogan.png') }}" alt="{{ data_get($branding, 'company_name', 'TH0002') }}">
                </a>
        ...
        <main class="wrap">
            ...
            <section class="hero hero-atelier">
                ...
                <div class="panel info-panel" id="deal-purchase">
                    <div class="atelier-kicker">{{ $product['tag'] ?? $t('product.snapshot_category_fallback', 'Atelier ready') }}</div>
                    <h1 class="title">{{ $product['title'] }}</h1>
                    <div class="share">@themeT('product.share', 'Chia sẻ bộ sưu tập')</div>
                    ...
                    <form method="POST" action="{{ route('site.cart.add', ['slug' => $productModel->slug]) }}">
                        ...
                        <div class="offer-row">
                            ...
                            <div class="qr-box atelier-brief">
                                <span class="atelier-brief-label">{{ $t('product.snapshot_title', 'Thông số chốt đơn') }}</span>
                                <div class="atelier-brief-grid">
                                    @foreach ($productionSnapshot as $entry)
                                        <div class="atelier-brief-item">
                            ...
                            <div class="guide-intro">
                                <div class="guide-hero">
                                    <div class="guide-hero-copy">
                                        ...
                                        <p>{{ $t('product.guide_intro', 'Quy trình đặt may trên TH0002 được tối ưu để khách hàng chốt mẫu, duyệt thông số và theo dõi tiến độ sản xuất một cách rõ ràng.') }}</p>
                                    ...
                                    <div class="guide-side-card">
                                        ...
                                        <p>{{ $t('product.guide_checkout_notice_body', 'Hãy kiểm tra kỹ chất liệu, bảng size, số lượng, deadline và phương án hoàn thiện để tránh phát sinh thay đổi sau khi đơn đã được xác nhận.') }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="guide-grid">
                                    ...
                                    <aside class="guide-support-card">
                                        ...
                                        <div class="guide-support-block">
                                            ...
                                            <span>{{ $t('product.guide_support_payment_body', 'Gửi yêu cầu tư vấn hoặc làm theo hướng dẫn trên website để hệ thống ghi nhận đơn và liên hệ xác nhận.') }}</span>
                                        </div>
                                        <div class="guide-support-block">
                                            <strong>{{ $t('product.guide_support_voucher_title', 'Nhận xác nhận đơn') }}</strong>
                                            <span>{{ $t('product.guide_support_voucher_body', 'Thông tin đơn, báo giá hoặc lịch duyệt mẫu sẽ được gửi về email / tài khoản ngay sau khi xử lý thành công.') }}</span>
                                        </div>
                                        <div class="guide-support-block">
