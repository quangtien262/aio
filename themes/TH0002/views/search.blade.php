@php
    ...
    $searchInsightCards = [
        ...
        ['value' => $t('search.insight_flow_value', 'Sỉ lẻ / OEM'), 'label' => $t('search.insight_flow', 'luồng tư vấn mặc định')],
    ];
@endphp
<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ $searchQuery !== '' ? str_replace(':query', $searchQuery, $t('search.title_query', 'Kết quả cho ":query"')) : $t('search.title_default', 'Tìm kiếm sản phẩm') }} | {{ data_get($branding, 'company_name', 'TH0002') }}</title>
        ...
        <style>
            ...
            * { box-sizing: border-box; }
            ...
            .search-hero { border-radius: 30px; border-color: #eadacc; border-top: 0; background: linear-gradient(135deg, #fff8f1 0%, #f1e2d5 100%); padding: 24px; gap: 18px; }
            .search-hero-shell { display: grid; grid-template-columns: minmax(0, 1.15fr) minmax(280px, .85fr); gap: 18px; }
            .search-hero-copy span { display: inline-flex; align-items: center; gap: 8px; min-height: 32px; padding: 0 14px; border-radius: 999px; background: rgba(163,80,53,.1); color: #8f442c; font-size: 12px; font-weight: 800; text-transform: uppercase; letter-spacing: .14em; }
            .search-hero h1 { font-size: 42px; line-height: 1.05; color: #2f231e; }
            .search-hero p, .search-summary { color: #5f4a3f; }
            .search-hero-aside { display: grid; gap: 12px; }
            .search-insight-card { padding: 14px 16px; border-radius: 20px; background: rgba(255,255,255,.76); border: 1px solid #eadacc; }
            .search-insight-card strong { display: block; margin-bottom: 4px; font-size: 28px; color: #8f442c; }
            .search-field input, .search-field select { border-radius: 16px; border-color: #e4d4c7; background: rgba(255,255,255,.92); }
            .search-button, .search-reset, .search-filter-chip { border-radius: 999px; }
            .search-button { background: linear-gradient(135deg, #c96d24 0%, #a85618 100%); }
            .product-card { border-radius: 26px; border-color: #eadacc; background: linear-gradient(180deg, #fff8f1 0%, #fff 100%); box-shadow: 0 22px 50px rgba(93, 74, 58, 0.08); }
            .product-media { aspect-ratio: 4 / 5; }
            .product-badge { left: 12px; right: auto; top: 12px; bottom: auto; background: rgba(127,58,37,.9); }
            .price { color: #8f442c; }
            .discount { background: #c96d24; border-radius: 999px; }
            .old-price-row { margin-top: 10px; padding-top: 10px; border-top: 1px solid #efdfd3; }
            .empty-state { border-radius: 26px; border-color: #eadacc; background: linear-gradient(180deg, #fff8f1 0%, #f4ebe4 100%); }
    ...
    <body>
            ...
            <main>
                <div class="th-container">
                    ...
                    <section class="search-hero">
                        <div class="search-hero-shell">
                            <div class="search-hero-copy">
                                <span>{{ $t('search.hero_badge', 'Atelier search') }}</span>
                                <h1>{{ $searchQuery !== '' ? __('search.title_query', ['query' => $searchQuery]) : __('search.title_default') }}</h1>
                                <p>@themeT('search.intro', 'Tìm theo tên sản phẩm, SKU, chất liệu hoặc dòng mô tả để đi nhanh tới mẫu may, đồng phục hay bộ sưu tập phù hợp.')</p>
                                <div class="search-summary">{{ __('search.result_count', ['count' => $resultCount ?? $productCollection->count()]) }}</div>
                            </div>
                            <div class="search-hero-aside">
                                @foreach ($searchInsightCards as $item)
        ...
        @include('theme-th0002::partials.engagement-modals')
    </body>
</html>
