...
@section('title', $title)
@section('content')
<main><section class="ec12-content"><div class="ec12-container ec12-product-detail"><div><img src="{{ $image ?: '/theme-demo/ec912/phone-graphite.webp' }}" alt="{{ $title }}"></div><div><p>SẢN PHẨM CHÍNH HÃNG</p><h1>{{ $title }}</h1><p>@if($original && $original > $price)<del>{{ number_format((float) $original, 0, ',', '.') }}đ</del>@endif</p><strong>{{ number_format((float) $price, 0, ',', '.') }}đ</strong><p>Giao hàng nhanh · Trả góp 0% · Bảo hành minh bạch</p><form action="{{ route('site.cart.add', ['slug' => data_get($productModel ?? null, 'slug')]) }}" method="post">@csrf<input type="number" name="quantity" value="1" min="1"><button class="ec12-button">Thêm vào giỏ</button></form><div class="ec12-prose">{!! $body !!}</div></div></div></section></main>
@endsection
