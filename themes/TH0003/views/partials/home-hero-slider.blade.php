<section class="th-hero-card" data-th-hero-slider>
    @foreach ($heroSlides as $slide)
        <article class="th-hero-slide {{ $loop->first ? 'is-active' : '' }}" data-th-hero-slide>
            <img src="{{ $slide['image'] ?? 'https://picsum.photos/seed/TH0003-fallback-hero/960/520' }}" alt="{{ $slide['title'] ?? 'Bộ sưu tập nổi bật hôm nay' }}">
            <div class="th-hero-overlay">
                <div class="th-inline-edit-head">
                    <span class="th-eyebrow" data-translation-display="{{ $slide['translation_keys']['eyebrow'] ?? ($loop->first ? ($heroBannerEditKeyMap['eyebrow']['key'] ?? 'theme.fallback.hero_eyebrow') : ($heroSlideDefaultKeyMap['eyebrow'] ?? 'theme_block.TH0003.hero_slide.eyebrow')) }}">{{ $slide['eyebrow'] ?? $slide['kicker'] ?? 'Flash sale' }}</span>
                    @if (!empty($canQuickEditThemeBlocks))
                        <button
                            type="button"
                            class="sf-inline-edit-btn"
                            data-sf-inline-edit-trigger
                            data-edit-title="Sửa hero TH0003"
                            data-edit-fields='@json($heroQuickEditFields ?? [])'
                        >Sửa hero</button>
                    @endif
                </div>
                <h1 class="th-hero-title" data-translation-display="{{ $slide['translation_keys']['title'] ?? ($loop->first ? ($heroBannerEditKeyMap['title']['key'] ?? 'theme.fallback.hero_title') : '') }}">{{ $slide['title'] ?? 'Bộ sưu tập nổi bật hôm nay' }}</h1>
    ...
    @if ($heroSlides->count() > 1)
        ...
        <div class="th-hero-dots">
            @foreach ($heroSlides as $slide)
                <button type="button" class="th-hero-dot {{ $loop->first ? 'is-active' : '' }}" data-th-hero-dot aria-label="Chọn slide"></button>
            @endforeach
        </div>
    @endif
</section>
