@php
    ...
    $sortOptions = [
        ...
        ['label' => $t('search.sort_price_asc', 'Giá thấp trước'), 'value' => 'price_asc'],
    ...
    <head>
        ...
        <style>
            ...
            * { box-sizing: border-box; }
            ...
            .fashion-category-page { background: #f7f5f2; }
            .fashion-category-page .th-topbar { border-top-color: #1f1a1d; background: #fdfbf8; }
            .fashion-category-page .th-header { background: #fffaf6; border-bottom: 1px solid #eadfda; }
            .fashion-category-page .th-search { border: 1px solid #1f1a1d; border-radius: 0; }
            .fashion-category-page .th-search button, .fashion-category-page .th-main-nav { background: #1f1a1d; }
            .fashion-category-page .th-main-nav-categories { background: #b20f3a; letter-spacing: .08em; }
            .fashion-category-page .th-main-nav-menu a { letter-spacing: .08em; font-size: 12px; }
            .fashion-collection-hero { display: grid; grid-template-columns: minmax(0, 1fr) minmax(280px, .38fr); gap: 18px; margin: 18px 0 22px; background: #1f1a1d; color: #fff; }
            .fashion-collection-copy { padding: clamp(28px, 5vw, 52px); display: grid; align-content: center; gap: 14px; }
            .fashion-collection-kicker { color: #f2c94c; font-size: 12px; font-weight: 900; letter-spacing: .18em; text-transform: uppercase; }
            .fashion-collection-hero h1 { margin: 0; font-family: Georgia, 'Times New Roman', serif; font-size: clamp(36px, 6vw, 72px); line-height: .95; font-weight: 500; }
            .fashion-collection-hero p { margin: 0; max-width: 560px; color: rgba(255,255,255,.72); line-height: 1.8; }
            .fashion-collection-stats { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); border-left: 1px solid rgba(255,255,255,.12); }
            .fashion-stat { min-height: 150px; display: grid; align-content: center; padding: 28px; border-bottom: 1px solid rgba(255,255,255,.12); }
            .fashion-stat strong { color: #f2c94c; font-size: 34px; font-family: Georgia, 'Times New Roman', serif; font-weight: 500; }
            .fashion-stat span { margin-top: 8px; color: rgba(255,255,255,.68); font-size: 12px; letter-spacing: .12em; text-transform: uppercase; }
            .fashion-category-page .breadcrumb { color: #756a70; }
            .fashion-category-page .catalog-layout { grid-template-columns: 280px minmax(0, 1fr); gap: 26px; align-items: start; }
            .fashion-category-page .filter-stack { position: sticky; top: 18px; }
            .fashion-category-page .filter-card { border: 0; background: #fffaf6; box-shadow: 0 14px 38px rgba(31,26,29,.07); }
            .fashion-category-page .filter-card-header { border-bottom-color: #eadfda; letter-spacing: .1em; font-size: 12px; color: #1f1a1d; }
            .fashion-category-page .filter-title-icon { border-radius: 0; background: #1f1a1d; color: #f2c94c; }
            .fashion-category-page .filter-link, .fashion-category-page .sort-pill { min-height: 34px; }
            .fashion-category-page .filter-link.active, .fashion-category-page .sort-pill.is-active { color: #b20f3a; }
            .fashion-category-page .catalog-toolbar { border: 0; background: transparent; padding: 0; min-height: auto; }
            .fashion-category-page .catalog-heading { padding: 0 0 8px; }
            .fashion-category-page .catalog-heading-icon { display: none; }
            .fashion-category-page .catalog-heading h1 { font-family: Georgia, 'Times New Roman', serif; font-size: clamp(34px, 5vw, 56px); text-transform: none; font-weight: 500; color: #1f1a1d; }
            .fashion-category-page .catalog-heading p { color: #756a70; letter-spacing: .08em; text-transform: uppercase; }
            .fashion-category-page .quick-chip { border: 1px solid #1f1a1d; background: transparent; color: #1f1a1d; letter-spacing: .08em; text-transform: uppercase; font-size: 12px; }
            .fashion-category-page .product-grid { gap: 26px 22px; }
            .fashion-category-page .product-card { border: 0; background: transparent; overflow: visible; box-shadow: none; }
            .fashion-category-page .product-card:hover { transform: translateY(-4px); box-shadow: none; }
            .fashion-category-page .product-media { aspect-ratio: 3 / 4; background: #ded4cf; }
            .fashion-category-page .product-media img { transition: transform .32s ease; }
            .fashion-category-page .product-card:hover img { transform: scale(1.045); }
            .fashion-category-page .product-badge { right: 0; bottom: 0; border-radius: 0; background: #1f1a1d; letter-spacing: .08em; text-transform: uppercase; }
            .fashion-category-page .product-body { padding: 14px 0 0; }
            .fashion-category-page .product-title { font-size: 16px; min-height: 48px; color: #1f1a1d; }
            .fashion-category-page .product-price { color: #b20f3a; letter-spacing: 0; }
            .fashion-category-page .product-discount { border-radius: 0; background: #0f8a8a; }
            .fashion-category-page .th-footer { background: #1f1a1d; color: #fff; border: 0; }
            .fashion-category-page .th-footer-card h4, .fashion-category-page .th-company strong { color: #f2c94c; letter-spacing: .12em; }
            .fashion-category-page .th-footer-links { color: rgba(255,255,255,.68); }
            .fashion-category-page .th-company { background: rgba(255,255,255,.06); border: 1px solid rgba(255,255,255,.12); border-radius: 0; }
            @media (max-width: 1100px) {
                .fashion-collection-hero, .fashion-category-page .catalog-layout { grid-template-columns: 1fr; }
                .fashion-category-page .filter-stack { position: static; }
            }
            @media (max-width: 760px) {
    ...
    <body>
        <div class="th-page fashion-category-page">
            ...
            <main class="th-container">
                ...
                <section class="fashion-collection-hero">
                    <div class="fashion-collection-copy">
                        <div class="fashion-collection-kicker">@themeT('category.collection_kicker', 'Fashion collection')</div>
                        <h1>{{ $category->name }}</h1>
                        <p>{{ $category->description ?: $t('category.collection_summary', 'Các sản phẩm được sắp theo tinh thần lookbook: dễ quét, rõ giá, ảnh lớn và ưu tiên cảm giác bộ sưu tập thay vì danh sách sản phẩm thời trang dày đặc.') }}</p>
                    </div>
                    <div class="fashion-collection-stats">
                        <div class="fashion-stat">
                            <strong>{{ $productCollection->count() }}</strong>
                            <span>@themeT('category.collection_items', 'sản phẩm hiển thị')</span>
                        </div>
                        <div class="fashion-stat">
                        ...
                        <section class="filter-card">
                            ...
                            <form method="GET" action="{{ request()->url() }}" class="filter-card-body is-stacked">
                                ...
                                <div class="price-range" data-price-range data-min="{{ $minPrice }}" data-max="{{ $maxPrice }}">
                                    ...
                                    <input type="range" min="{{ $minPrice }}" max="{{ $maxPrice }}" value="{{ $selectedMinPrice }}" step="1000" data-price-range-min name="min_price" aria-label="{{ $t('search.sort_price_asc', 'Giá thấp trước') }}">
                                    <input type="range" min="{{ $minPrice }}" max="{{ $maxPrice }}" value="{{ $selectedMaxPrice }}" step="1000" data-price-range-max name="max_price" aria-label="{{ $t('search.sort_price_desc', 'Giá cao trước') }}">
                                </div>
