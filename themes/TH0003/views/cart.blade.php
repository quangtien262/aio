...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Giỏ hàng{{ $companyTitle ? ' | '.$companyTitle : '' }}</title>
    ...
    <body class="th-fashion-page">
        ...
        <main class="wrap">
                            ...
                            @foreach ($cartItems as $item)
                                <article class="cart-row">
                                    <a href="{{ $item['url'] ?? route('site.catalog.search') }}">
                                        <img src="{{ $item['image'] ?: 'https://picsum.photos/seed/TH0003-product-fallback/640/420' }}" alt="{{ $item['title'] }}">
                                    </a>
            ...
            @else
                <div class="empty-state">@themeT('cart.empty', 'Giỏ hàng hiện đang trống. Hãy quay lại danh mục hoặc trang sản phẩm để thêm sản phẩm thời trang vào giỏ trước khi mua.')</div>
            @endif
        </main>
