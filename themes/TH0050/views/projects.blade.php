@php
    $title = $pageTitle ?? (app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('TH0050', app()->getLocale(), 'legacy_inline.6980f6dccf6e96cb', 'Dự án'));
    $description = $pageDescription ?? (app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('TH0050', app()->getLocale(), 'legacy_inline.01f5fc41dc67335e', 'Danh sách dự án đã xuất bản.'));
...
<main class="xd-page-main">
    <div class="xd-container">
        <section class="xd-cms-hero">
            <div>
                <span class="xd-kicker">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('TH0050', app()->getLocale(), 'legacy_inline.6980f6dccf6e96cb', 'Dự án') }}</span>
                <h1>{{ $title }}</h1>
                <p>{{ $description }}</p>
            </div>
            <div class="xd-cms-stats">
                <strong>{{ collect($listingItems ?? [])->count() }}</strong>
                <span>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('TH0050', app()->getLocale(), 'legacy_inline.d9f11545f501258d', 'Dự án đang hiển thị') }}</span>
            </div>
        </section>
        ...
        <section class="xd-projects-list">
            ...
            @empty
                <p>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('TH0050', app()->getLocale(), 'legacy_inline.cb746a768450cd69', 'Chưa có dự án nào được xuất bản.') }}</p>
            @endforelse
        </section>
    </div>
</main>
