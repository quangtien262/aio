@php
    ...
    $footerLogo = data_get($branding, 'logo_url');
    $footerLogoAlt = data_get($siteProfile ?? [], 'site_name', 'EGA Mini Mart');
...
<footer class="ec96-footer" id="footer"><div class="ec96-container ec96-footer-grid">
    <section class="ec96-footer-brand"><a class="ec96-logo" href="{{ route('site.home') }}" aria-label="{{ $footerLogoAlt }}">@if($footerLogo)<img src="{{ $footerLogo }}" alt="{{ $footerLogoAlt }}" loading="lazy">@endif</a><h3>Siêu thị Mini EGA</h3><p>Thương hiệu siêu thị uy tín và chất lượng, mang đến trải nghiệm mua sắm tiện lợi, hiện đại và phong phú.</p><p>Mã số thuế: 12345678999</p><p><i class="fa-solid fa-location-dot"></i> {{ $address }}</p><div><a href="tel:{{ preg_replace('/\s+/', '', $phone) }}"><i class="fa-solid fa-phone"></i> Hotline<br><b>{{ $phone }}</b></a><a href="mailto:{{ $email }}"><i class="fa-regular fa-envelope"></i> Email<br><b>{{ $email }}</b></a></div><h4>Mạng xã hội</h4><p class="ec96-social"><a href="#"><i class="fa-brands fa-facebook-f"></i></a><a href="#"><i class="fa-brands fa-youtube"></i></a><a href="#"><i class="fa-brands fa-tiktok"></i></a><a href="#"><i class="fa-brands fa-instagram"></i></a></p></section>
