@php
    $t = fn ($key) => app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0320', app()->getLocale(), 'catalog_listing.'.$key, $key);
    ...
    $count = $isSearch ? ($resultCount ?? $items->count()) : $items->count();
@endphp
<main class="xd-catalog-page">
    <section class="xdc-hero">
        <div class="xdc-container">
            ...
            <h1>{{ $title }}</h1>
            <p>{{ !$isSearch && filled($category->description) ? $category->description : $t('intro') }}</p>
        </div>
    </section>
            ...
            <details class="xdc-filters" open>
                ...
                <nav class="xdc-categories" aria-label="{{ $t('categories') }}">
                    ...
                    @if($isSearch)
                        ...
                        @endforeach
                    @else
                        @include('theme-xd0320::partials.catalog-categories', ['nodes' => $catalogTreeCategories ?? $sidebarCategories ?? []])
                    @endif
                </nav>
            </details>
