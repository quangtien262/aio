...
$phone=$branding['support_hotline']??'1900 6750';$name=trim((string)($branding['company_name']??''))?:'Sudes Aquarium';$logo=trim((string)($branding['logo_url']??''));$cartCount=(int)data_get($cartSummary??[],'count',0);$nav=collect(data_get($shell,'top_menu',[]))->filter(fn($item)=>is_array($item)&&filled(data_get($item,'label')))->values();
@endphp
