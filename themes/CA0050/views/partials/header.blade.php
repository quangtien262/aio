...
$phone=$branding['support_hotline']??'';$name=trim((string)($branding['company_name']??''))?:'Sudes Aquarium';$logo=trim((string)($branding['logo_url']??''));$cartCount=(int)data_get($cartSummary??[],'count',0);$nav=collect(data_get($shell,'top_menu',[]))->filter(fn($item)=>is_array($item)&&filled(data_get($item,'label')))->values();
@endphp
...
<div class="sf-language-mobile-slot">@include('partials.storefront-language-switcher')</div>
 <div class="ca50-header-inner">
  <a class="ca50-logo" href="{{ route('site.home') }}">@if($logo)<img src="{{ $logo }}" alt="{{ $name }}">@endif</a>
  ...
  <div class="ca50-tools"><a href="{{ route('customer.account') }}"><i class="fa-regular fa-heart"></i><em>0</em></a><a href="{{ route('site.cart.index') }}"><i class="fa-solid fa-basket-shopping"></i><em>{{ $cartCount }}</em></a>
