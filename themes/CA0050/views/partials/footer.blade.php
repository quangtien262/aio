...
<footer id="lien-he" class="ca50-footer xd-landing-block" data-landing-block-id="{{ data_get($footerCta??[],'id') }}" data-block-type="ca0050_footer">
 ...
 <div class="ca50-footer-wave"></div><div class="ca50-footer-grid">
  <section><a class="ca50-logo" href="{{ route('site.home') }}">@if($logo)<img src="{{ $logo }}" alt="{{ $name }}">@else<span><i class="fa-solid fa-fish-fins"></i></span><b>SUDES<br><strong>AQUARIUM</strong></b>@endif</a><p>Sudes Aquarium là cửa hàng chuyên cung cấp cá cảnh, phụ kiện và dịch vụ setup bể thủy sinh trọn gói.</p><p><b>📍 Địa chỉ:</b> {{ $address }}</p><p><b>📞 Điện thoại:</b> {{ $phone }}</p><p><b>✉ Email:</b> {{ $email }}</p></section>
