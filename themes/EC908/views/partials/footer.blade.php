...
<footer class="ec98-footer"><div class="ec98-container ec98-footer-brand"><a class="ec98-logo is-footer" href="{{ route('site.home') }}" aria-label="{{ $siteName }}">@if($logo)<img src="{{ $logo }}" alt="{{ $siteName }}">@endif</a></div><div class="ec98-container ec98-footer-grid">
    ...
    <section><h3>VỀ CHÚNG TÔI</h3><p>Nếu bạn cần hỗ trợ hoặc có thắc mắc gì, vui lòng liên hệ ngay với chúng tôi</p><p><i class="fa-solid fa-location-dot"></i> {{ data_get($branding,'support_location','') }}</p><p><i class="fa-solid fa-phone"></i> {{ data_get($branding,'support_hotline','') }}</p><p><i class="fa-solid fa-envelope"></i> {{ data_get($branding,'support_email','') }}</p></section>
