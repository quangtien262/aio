...
<section id="{{ $anchor }}" class="xd4-hero" data-xd4-hero data-autoplay="{{ (int) ($settings['autoplay_ms'] ?? 6000) }}">
    @forelse($slides as $index => $slide)
        <article class="xd4-hero__slide {{ $index === 0 ? 'is-active' : '' }}" data-xd4-slide>
            ...
            <div class="xd4-container xd4-hero__content"><div class="xd4-hero__copy"><p class="xd4-eyebrow">{{ $slide['kicker'] ?? $data['subtitle'] ?? '' }}</p><h1>{{ $slide['title'] ?? $data['title'] ?? '' }}</h1><p>{{ $slide['summary'] ?? $slide['description'] ?? $data['description'] ?? '' }}</p><div class="xd4-actions"><a class="xd4-button xd4-button--outline" href="#footer">Nháº­n bÃ¡o giÃ¡</a><a class="xd4-button" href="{{ $slide['link_url'] ?? '#dich-vu' }}">{{ $slide['button_label'] ?? $data['button_label'] ?? 'Xem thÃªm' }} â†’</a></div></div></div>
        </article>
    @empty
    ...
    @endforelse
    @if($slides->count() > 1)<button class="xd4-hero__arrow xd4-hero__arrow--prev" type="button" data-xd4-prev>â€¹</button><button class="xd4-hero__arrow xd4-hero__arrow--next" type="button" data-xd4-next>â€º</button>@endif
</section>
