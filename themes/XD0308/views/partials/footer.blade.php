<footer id="footer" class="xd4-footer">
    <div class="xd4-container xd4-footer__grid">
        <div><a class="xd4-brand" href="#top">@if(filled($logoUrl ?? null))<img src="{{ $logoUrl }}" alt="{{ $companyName }}">@else<span class="xd4-brand__mark">L</span><strong>{{ $companyName }}</strong>@endif</a><p>{{ $companyDescription ?? 'Giải pháp vận tải và hậu cần linh hoạt cho doanh nghiệp.' }}</p><p>{{ $supportAddress ?? '' }}</p><p><a href="tel:{{ $phoneHref ?? '' }}">{{ $hotline ?? '' }}</a></p><p><a href="mailto:{{ $supportEmail ?? '' }}">{{ $supportEmail ?? '' }}</a></p></div>
