...
<main class="xd-page-main">
            <div class="xd-container">
                    <section class="xd-cms-hero">
                        <div>
                            <span class="xd-kicker">{{ app()->getLocale() === 'en' ? 'News' : 'Tin tÃƒÂ¡Ã‚Â»Ã‚Â©c' }}</span>
                            <h1>{{ $pageTitle ?? (app()->getLocale() === 'en' ? 'News' : 'Tin tÃƒÂ¡Ã‚Â»Ã‚Â©c') }}</h1>
                            <p>{{ $pageDescription ?? (app()->getLocale() === 'en' ? 'Latest company updates and construction insights.' : 'Danh sÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡ch bÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â i viÃƒÆ’Ã‚Â¡Ãƒâ€šÃ‚ÂºÃƒâ€šÃ‚Â¿t, tin tÃƒÆ’Ã‚Â¡Ãƒâ€šÃ‚Â»Ãƒâ€šÃ‚Â©c vÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â  kinh nghiÃƒÆ’Ã‚Â¡Ãƒâ€šÃ‚Â»ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¡m xÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢y dÃƒÆ’Ã‚Â¡Ãƒâ€šÃ‚Â»Ãƒâ€šÃ‚Â±ng mÃƒÆ’Ã‚Â¡Ãƒâ€šÃ‚Â»ÃƒÂ¢Ã¢â€šÂ¬Ã‚Âºi nhÃƒÆ’Ã‚Â¡Ãƒâ€šÃ‚ÂºÃƒâ€šÃ‚Â¥t.') }}</p>
                        </div>
                        <div class="xd-cms-stats">
                            <strong>{{ method_exists($listingItems, 'total') ? $listingItems->total() : collect($listingItems ?? [])->count() }}</strong>
                            <span>{{ app()->getLocale() === 'en' ? 'Published posts' : 'BÃƒÆ’Ã‚Â i viÃƒÂ¡Ã‚ÂºÃ‚Â¿t Ãƒâ€žÃ¢â‚¬ËœÃƒÆ’Ã‚Â£ xuÃƒÂ¡Ã‚ÂºÃ‚Â¥t bÃƒÂ¡Ã‚ÂºÃ‚Â£n' }}</span>
                        </div>
                    </section>
                        ...
                        @forelse ($listingItems as $post)
                            ...
                            <article class="xd-service-card">
                                ...
                                <div class="xd-service-body">
                                    ...
                                    <p>{{ $summary }}</p>
                                    <a class="xd-text-link" href="{{ $postUrl }}">{{ app()->getLocale() === 'en' ? 'Read more' : 'Ãƒâ€žÃ‚ÂÃƒÂ¡Ã‚Â»Ã‚Âc tiÃƒÂ¡Ã‚ÂºÃ‚Â¿p' }}</a>
                                </div>
                            </article>
                        @empty
                            <p>{{ app()->getLocale() === 'en' ? 'No posts are available yet.' : 'ChÃƒâ€ Ã‚Â°a cÃƒÆ’Ã‚Â³ bÃƒÆ’Ã‚Â i viÃƒÂ¡Ã‚ÂºÃ‚Â¿t nÃƒÆ’Ã‚Â o Ãƒâ€žÃ¢â‚¬ËœÃƒâ€ Ã‚Â°ÃƒÂ¡Ã‚Â»Ã‚Â£c xuÃƒÂ¡Ã‚ÂºÃ‚Â¥t bÃƒÂ¡Ã‚ÂºÃ‚Â£n.' }}</p>
                        @endforelse
                    </section>
