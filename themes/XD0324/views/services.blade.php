...
<main class="xd-page-main">
            <div class="xd-container">
                    <section class="xd-cms-hero">
                        <div>
                            <span class="xd-kicker">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0324', app()->getLocale(), 'legacy_inline.57402b53c32281d2', 'Dịch vụ') }}</span>
                            <h1>{{ $pageTitle ?? (app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0324', app()->getLocale(), 'legacy_inline.57402b53c32281d2', 'Dịch vụ')) }}</h1>
                            <p>{{ $pageDescription ?? (app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0324', app()->getLocale(), 'legacy_inline.4c06edf681791e0b', 'Danh sách dịch vụ thiết kế và thi công nổi bật.')) }}</p>
                        </div>
                        <div class="xd-cms-stats">
                            <strong>{{ collect($listingItems ?? [])->count() }}</strong>
                            <span>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0324', app()->getLocale(), 'legacy_inline.4f764a9013ef2ac2', 'Dịch vụ đang hiển thị') }}</span>
                        </div>
                    </section>
                        ...
                        @forelse ($listingItems as $service)
                            ...
                            <article class="xd-service-card">
                                ...
                                <div class="xd-service-body">
                                    ...
                                    <a class="xd-text-link" href="{{ $serviceUrl }}">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0324', app()->getLocale(), 'legacy_inline.e856bed2f7c52556', 'Tìm hiểu ngay') }}</a>
                                </div>
                            </article>
                        @empty
                            <p>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0324', app()->getLocale(), 'legacy_inline.8490359c9289323c', 'Chưa có dịch vụ nào được xuất bản.') }}</p>
                        @endforelse
                    </section>
            </div>
</main>
