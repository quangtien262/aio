...
@php($phone=data_get($branding,'support_hotline',''))
@php($email=data_get($branding,'support_email',''))
@php($address=data_get($branding,'support_location',''))
<footer class="ec97-footer"><div class="ec97-container ec97-footer-grid">
<section><a class="ec97-logo is-footer" href="#top" aria-label="{{ $siteName }}">@if($logo)<img src="{{ $logo }}" alt="{{ $siteName }}">@endif</a><p>Chuyên cung cấp các thiết bị điện máy, điện tử và Gaming Gear.</p><p>Mã số thuế: 12345678910</p><p><i class="fa-solid fa-location-dot"></i> {{ $address }}</p><div class="ec97-contact"><a href="tel:{{ preg_replace('/\s+/','',$phone) }}">Hotline<b>{{ $phone }}</b></a><a href="mailto:{{ $email }}">Email<b>{{ $email }}</b></a></div><h4>Mạng xã hội</h4><div class="ec97-social"><i class="fa-brands fa-facebook-f"></i><i class="fa-brands fa-youtube"></i><i class="fa-brands fa-tiktok"></i><i class="fa-brands fa-instagram"></i></div></section>
