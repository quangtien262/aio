@php
    ...
    if ($navItems->isEmpty()) {
        $navItems = collect([
            ['label' => $themeText('xd321.nav.home'), 'href' => route('site.home'), 'children' => []],
            ['label' => $themeText('xd321.nav.services'), 'href' => '#dich-vu', 'children' => []],
            ['label' => $themeText('xd321.nav.story'), 'href' => '#gioi-thieu', 'children' => []],
            ['label' => $themeText('xd321.nav.menu'), 'href' => '#du-an', 'children' => []],
            ['label' => $themeText('xd321.nav.news'), 'href' => '#du-an', 'children' => []],
            ['label' => $themeText('xd321.nav.team'), 'href' => '#doi-ngu', 'children' => []],
        ]);
    }
@endphp
        ...
        <div class="foot-container foot-header__masthead-inner">
            <a class="foot-brand" href="{{ route('site.home') }}" aria-label="{{ $companyName }}">
                ...
                @else
                    ...
                    <span><strong>{{ $companyName }}</strong><small>@themeT('xd321.brand.tagline')</small></span>
                @endif
            </a>
            <div class="foot-header__account">
                @guest('customer')
                    ...
                    <button type="button" data-xd-auth-open="register">@themeT('xd321.header.register')</button>
                @else
