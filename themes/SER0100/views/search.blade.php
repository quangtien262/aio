...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ $searchQuery !== '' ? str_replace(':query', $searchQuery, $t('search.title_query', 'Kết quả cho ":query"')) : $t('search.title_default', 'Tìm kiếm dịch vụ') }} | {{ data_get($branding, 'company_name', 'SER0100') }}</title>
        ...
        <style>
            ...
            * { box-sizing: border-box; }
            body {
                ...
                background: radial-gradient(circle at top right, rgba(245, 158, 11, 0.14), transparent 28%), #f7fbfd;
                color: #243b53;
            }
            ...
            .hero {
                ...
                color: #fff;
                box-shadow: 0 26px 56px rgba(16, 42, 67, 0.14);
            }
            ...
            .hero button { border: 0; background: linear-gradient(135deg, var(--o), #ea580c); color: #fff; font-weight: 800; cursor: pointer; }
            ...
            .card { padding: 18px; border: 1px solid rgba(217, 226, 236, 0.92); border-radius: 26px; background: rgba(255, 255, 255, 0.94); box-shadow: 0 18px 40px rgba(16, 42, 67, 0.08); }
            ...
            .card-tag { display: inline-flex; margin-bottom: 12px; padding: 8px 12px; border-radius: 999px; background: rgba(31, 111, 120, 0.08); color: var(--p); font-size: 12px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.04em; }
            .card h3 { margin: 0 0 10px; color: var(--night); }
                ...
                @forelse ($productCollection as $product)
                    <article class="card">
                        <a class="card-media" href="{{ $product['url'] ?? '#' }}" aria-label="{{ $product['title'] ?? '' }}">
                            <img src="{{ $product['image'] ?? 'https://picsum.photos/seed/ser0100-search/960/720' }}" alt="{{ $product['title'] ?? '' }}">
                        </a>
