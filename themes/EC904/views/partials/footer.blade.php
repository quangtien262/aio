...
<footer class="ec94-footer" id="newsletter"><div class="ec94-container ec94-footer-main">
    <section><a class="ec94-logo is-footer" href="{{ route('site.home') }}" aria-label="{{ $siteName }}">@if($logo)<img src="{{ $logo }}" alt="{{ $siteName }}">@endif</a><h3>{{ $siteName }}</h3><p>{{ data_get($brand, 'company_description', 'Siêu thị đa ngành với sản phẩm chọn lọc và dịch vụ tận tâm.') }}</p><p>Địa chỉ: {{ data_get($brand, 'support_location', '') }}<br>Điện thoại: {{ data_get($brand, 'support_hotline', '') }} · Email: {{ data_get($brand, 'support_email', '') }}</p></section>
