...
@section('content')
    <section class="spa111-page-head">
        <div class="spa111-container">
            <nav class="spa111-breadcrumb"><a href="{{ route('site.home') }}">Trang chủ</a><span>/</span><span>{{ $pageTitle }}</span></nav>
            <h1>{{ $pageTitle }}</h1>
            ...
            @if($entries->isNotEmpty())
                <div class="spa111-list-grid">
                    @foreach($entries as $item)
                        ...
                        @endphp
                        <a class="spa111-list-card" href="{{ $url }}">
                            <img src="{{ $image }}" alt="{{ $title }}">
                            <h2>{{ $title }}</h2>
                            @if(filled($price))<span class="spa111-price">{{ number_format((float) $price, 0, ',', '.') }}đ</span>@endif
                            @if(filled($summary))<p>{{ \Illuminate\Support\Str::limit(strip_tags($summary), 150) }}</p>@endif
                        </a>
                    @endforeach
                </div>
            @else
                <div class="spa111-empty">Chưa có sản phẩm để hiển thị.</div>
            @endif
        </div>
    </section>
@endsection
