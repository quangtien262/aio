...
@section('content')
    <section class="spa111-page-head">
        <div class="spa111-container">
            <nav class="spa111-breadcrumb"><a href="{{ route('site.home') }}">Trang chủ</a><span>/</span><span>Giỏ hàng</span></nav>
            <h1>Giỏ hàng</h1>
            ...
            @if($entries->isNotEmpty())
                <div class="spa111-list-grid">
                    @foreach($entries as $item)
                        ...
                        @endphp
                        <article class="spa111-list-card">
                            <img src="{{ $image }}" alt="{{ $title }}">
                            <h2>{{ $title }}</h2>
                            <p>Số lượng: {{ $qty }}</p>
                            <span class="spa111-price">{{ number_format((float) $price, 0, ',', '.') }}đ</span>
                        </article>
                    @endforeach
                </div>
                <p class="spa111-price" style="margin-top:30px">Tạm tính: {{ number_format((float) $total, 0, ',', '.') }}đ</p>
            @else
                <div class="spa111-empty">Giỏ hàng đang trống.</div>
            @endif
        </div>
    </section>
@endsection
