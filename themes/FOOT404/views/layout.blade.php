...
<head>
    ...
    <title>@yield('title', data_get($siteProfile ?? [], 'site_name', 'FOOT404'))</title>
    ...
    @include('theme-foot404::partials.styles')
    @stack('head')
...
@php
    ...
    $editorLocales = collect(\App\Support\FrontendLocalization::localeOptions())
        ...
        ->map(fn (array $locale): array => ['code' => $locale['code'] ?? '', 'label' => $locale['label'] ?? strtoupper($locale['code'] ?? '')])
        ->filter(fn (array $locale): bool => filled($locale['code']))->values()->all();
@endphp
<div class="f404-page" id="top">
    @include('theme-foot404::partials.header')
    @yield('content')
    @include('theme-foot404::partials.footer')
</div>
...
@include('theme-foot404::partials.scripts')
@if($canEditLanding)
