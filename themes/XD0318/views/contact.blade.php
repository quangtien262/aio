@php
    ...
    $footerNewsletterSource = 'theme-footer-XD0318-cms';
@endphp
...
<main class="xd-page-main">
            <div class="xd-container">
                    <section class="xd-cms-hero">
                        <div>
                            <span class="xd-kicker">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0318', app()->getLocale(), 'legacy_inline.416dc399394e8648', 'Liên hệ') }}</span>
                            <h1>{{ $entry->title }}</h1>
                            @if (!empty($entry->excerpt))
                                <p>{{ $entry->excerpt }}</p>
                            @else
                                <p>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0318', app()->getLocale(), 'legacy_inline.853bb0d2ac4347c4', 'Gửi nhu cầu tư vấn, thiết kế hoặc thi công. Đội ngũ XD0318 sẽ phản hồi trong thời gian sớm nhất.') }}</p>
                            @endif
                        </div>
                        <div class="xd-cms-stats">
                            <strong>24h</strong>
                            <span>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0318', app()->getLocale(), 'legacy_inline.1e963876e7fe12c9', 'Thời gian phản hồi') }}</span>
                        </div>
                    </section>

                    <section class="xd-contact-page">
                        <aside class="xd-contact-panel">
                            <span class="xd-kicker">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0318', app()->getLocale(), 'legacy_inline.6e174ab7cf77aa95', 'Thông tin liên hệ') }}</span>
                            <h2>{{ $branding['company_name'] ?? $logoAlt }}</h2>
                            <p>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0318', app()->getLocale(), 'legacy_inline.4ef88df2d78887d4', 'Hãy cho chúng tôi biết nhu cầu, quy mô và thời gian dự kiến. Đội ngũ tư vấn sẽ kiểm tra và đề xuất hướng triển khai phù hợp.') }}</p>
                            <ul class="xd-contact-methods">
                                ...
                                <li class="xd-contact-method">
                                    ...
                                    <div>
                                        <small>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0318', app()->getLocale(), 'legacy_inline.5c983b845ed1f9a8', 'Địa chỉ') }}</small>
                                        <span>{{ $address }}</span>
                                    </div>
                                </li>
                            </ul>
                            <div class="xd-contact-note">
                                <strong>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0318', app()->getLocale(), 'legacy_inline.09fee4c7fe5eff4e', 'Chia sẻ nhu cầu, chúng tôi tư vấn đúng giải pháp.') }}</strong>
                                <span>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0318', app()->getLocale(), 'legacy_inline.6cedf167e76834b6', 'Hãy gửi thêm địa điểm, diện tích, tiến độ mong muốn hoặc yêu cầu kỹ thuật để đội ngũ chuẩn bị phương án phù hợp ngay từ lần phản hồi đầu tiên.') }}</span>
                            </div>
                        </aside>

                        <article class="xd-contact-form-card">
                            <h2>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0318', app()->getLocale(), 'legacy_inline.1e77514570be5cdf', 'Gửi yêu cầu liên hệ') }}</h2>
                            @if (session('contact_status'))
                                <div class="xd-contact-alert">{{ session('contact_status') }}</div>
                            @endif
                            @if ($errors->any())
                                <div class="xd-contact-errors">
                                    {{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0318', app()->getLocale(), 'legacy_inline.be3354f6dcd97e9a', 'Vui lòng kiểm tra lại thông tin.') }}
                                    <ul>
                            ...
                            <form class="xd-contact-form" method="POST" action="{{ route('site.contact.submit') }}">
                                ...
                                <input type="hidden" name="subject" value="{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0318', app()->getLocale(), 'legacy_inline.a00b0d7367f96375', 'Yêu cầu liên hệ từ website') }}">
                                ...
                                <label class="xd-contact-field">
                                    <span>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0318', app()->getLocale(), 'legacy_inline.84b329c821a5b27e', 'Số điện thoại') }}</span>
                                ...
                                <label class="xd-contact-field">
                                    <span>{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0318', app()->getLocale(), 'legacy_inline.d8109b33388f7ada', 'Nội dung') }}</span>
                                    <textarea name="message" required>{{ old('message') }}</textarea>
                                </label>
                                <button class="xd-contact-submit" type="submit">{{ app(\App\Core\Themes\ThemeTranslationService::class)->bladeText('XD0318', app()->getLocale(), 'legacy_inline.72ecdde11b76357d', 'Gửi liên hệ') }}</button>
                            </form>
                        </article>
                    </section>
            </div>
