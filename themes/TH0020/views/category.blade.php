@php
    ...
    $sortOptions = [
        ['label' => $t('search.sort_default', 'M?c d?nh'), 'value' => 'default'],
        ['label' => $t('search.sort_bestseller', 'Bán ch?y'), 'value' => 'bestseller'],
        ['label' => $t('search.sort_price_asc', 'Giá th?p tru?c'), 'value' => 'price_asc'],
        ['label' => $t('search.sort_price_desc', 'Giá cao tru?c'), 'value' => 'price_desc'],
        ['label' => $t('search.sort_newest', 'M?i nh?t'), 'value' => 'newest'],
    ];
    ...
    <head>
        ...
        <style>
            ...
            * { box-sizing: border-box; }
            ...
            .interior-category-page { background: #f7f5f2; }
            .interior-category-page .th-topbar { border-top-color: #1f1a1d; background: #fdfbf8; }
            .interior-category-page .th-header { background: #fffaf6; border-bottom: 1px solid #eadfda; }
            .interior-category-page .th-search { border: 1px solid #1f1a1d; border-radius: 0; }
            .interior-category-page .th-search button, .interior-category-page .th-main-nav { background: #1f1a1d; }
            .interior-category-page .th-main-nav-categories { background: #b20f3a; letter-spacing: .08em; }
            .interior-category-page .th-main-nav-menu a { letter-spacing: .08em; font-size: 12px; }
            .interior-collection-hero { display: grid; grid-template-columns: minmax(0, 1fr) minmax(280px, .38fr); gap: 18px; margin: 18px 0 22px; background: #1f1a1d; color: #fff; }
            .interior-collection-copy { padding: clamp(28px, 5vw, 52px); display: grid; align-content: center; gap: 14px; }
            .interior-collection-kicker { color: #f2c94c; font-size: 12px; font-weight: 900; letter-spacing: .18em; text-transform: uppercase; }
            .interior-collection-hero h1 { margin: 0; font-family: Georgia, 'Times New Roman', serif; font-size: clamp(36px, 6vw, 72px); line-height: .95; font-weight: 500; }
            .interior-collection-hero p { margin: 0; max-width: 560px; color: rgba(255,255,255,.72); line-height: 1.8; }
            .interior-collection-stats { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); border-left: 1px solid rgba(255,255,255,.12); }
            .interior-stat { min-height: 150px; display: grid; align-content: center; padding: 28px; border-bottom: 1px solid rgba(255,255,255,.12); }
            .interior-stat strong { color: #f2c94c; font-size: 34px; font-family: Georgia, 'Times New Roman', serif; font-weight: 500; }
            .interior-stat span { margin-top: 8px; color: rgba(255,255,255,.68); font-size: 12px; letter-spacing: .12em; text-transform: uppercase; }
            .interior-category-page .breadcrumb { color: #756a70; }
            .interior-category-page .catalog-layout { grid-template-columns: 280px minmax(0, 1fr); gap: 26px; align-items: start; }
            .interior-category-page .filter-stack { position: sticky; top: 18px; }
            .interior-category-page .filter-card { border: 0; background: #fffaf6; box-shadow: 0 14px 38px rgba(31,26,29,.07); }
            .interior-category-page .filter-card-header { border-bottom-color: #eadfda; letter-spacing: .1em; font-size: 12px; color: #1f1a1d; }
            .interior-category-page .filter-title-icon { border-radius: 0; background: #1f1a1d; color: #f2c94c; }
            .interior-category-page .filter-link, .interior-category-page .sort-pill { min-height: 34px; }
            .interior-category-page .filter-link.active, .interior-category-page .sort-pill.is-active { color: #b20f3a; }
            .interior-category-page .catalog-toolbar { border: 0; background: transparent; padding: 0; min-height: auto; }
            .interior-category-page .catalog-heading { padding: 0 0 8px; }
            .interior-category-page .catalog-heading-icon { display: none; }
            .interior-category-page .catalog-heading h1 { font-family: Georgia, 'Times New Roman', serif; font-size: clamp(34px, 5vw, 56px); text-transform: none; font-weight: 500; color: #1f1a1d; }
            .interior-category-page .catalog-heading p { color: #756a70; letter-spacing: .08em; text-transform: uppercase; }
            .interior-category-page .quick-chip { border: 1px solid #1f1a1d; background: transparent; color: #1f1a1d; letter-spacing: .08em; text-transform: uppercase; font-size: 12px; }
            .interior-category-page .product-grid { gap: 26px 22px; }
            .interior-category-page .product-card { border: 0; background: transparent; overflow: visible; box-shadow: none; }
            .interior-category-page .product-card:hover { transform: translateY(-4px); box-shadow: none; }
            .interior-category-page .product-media { aspect-ratio: 3 / 4; background: #ded4cf; }
            .interior-category-page .product-media img { transition: transform .32s ease; }
            .interior-category-page .product-card:hover img { transform: scale(1.045); }
            .interior-category-page .product-badge { right: 0; bottom: 0; border-radius: 0; background: #1f1a1d; letter-spacing: .08em; text-transform: uppercase; }
            .interior-category-page .product-body { padding: 14px 0 0; }
            .interior-category-page .product-title { font-size: 16px; min-height: 48px; color: #1f1a1d; }
            .interior-category-page .product-price { color: #b20f3a; letter-spacing: 0; }
            .interior-category-page .product-discount { border-radius: 0; background: #0f8a8a; }
            .interior-category-page .th-footer { background: #1f1a1d; color: #fff; border: 0; }
            .interior-category-page .th-footer-card h4, .interior-category-page .th-company strong { color: #f2c94c; letter-spacing: .12em; }
            .interior-category-page .th-footer-links { color: rgba(255,255,255,.68); }
            .interior-category-page .th-company { background: rgba(255,255,255,.06); border: 1px solid rgba(255,255,255,.12); border-radius: 0; }
            @media (max-width: 1100px) {
                .interior-collection-hero, .interior-category-page .catalog-layout { grid-template-columns: 1fr; }
                .interior-category-page .filter-stack { position: static; }
            }
            @media (max-width: 760px) {
                .interior-collection-stats { grid-template-columns: 1fr; border-left: 0; }
                .interior-stat { min-height: 100px; }
            }
            @include('theme-th0020::partials.interior-shell-styles')
        </style>
    </head>
    <body>
        <div class="th-page th-interior-page interior-category-page">
            ...
            <main class="th-container">
                ...
                <section class="interior-collection-hero">
                    <div class="interior-collection-copy">
                        <div class="interior-collection-kicker">@themeT('category.collection_kicker', 'Interior collection')</div>
                        <h1>{{ $category->name }}</h1>
                        <p>{{ $category->description ?: $t('category.collection_summary', 'Các s?n ph?m du?c s?p theo tinh th?n lookbook: d? quét, rõ giá, ?nh l?n và uu tiên c?m giác b? suu t?p thay vì danh sách s?n ph?m th?i trang dày d?c.') }}</p>
                    </div>
                    <div class="interior-collection-stats">
                        <div class="interior-stat">
                            <strong>{{ $productCollection->count() }}</strong>
                            <span>@themeT('category.collection_items', 's?n ph?m hi?n th?')</span>
                        </div>
                        <div class="interior-stat">
                            <strong>{{ count($childCategories ?? []) }}</strong>
                            <span>@themeT('category.collection_edits', 'nhóm phong cách')</span>
                        </div>
                    </div>
                </section>
                ...
                <section class="catalog-layout">
                    <aside class="filter-stack">
                        <section class="filter-card">
                            <div class="filter-card-header">
                                <strong><span class="filter-title-icon">?</span> @themeT('category.filter_category', 'Danh m?c')</strong>
                            </div>
                        ...
                        <section class="filter-card">
                            <div class="filter-card-header">
                                <strong><span class="filter-title-icon">?</span> @themeT('category.sort_by', 'S?p x?p theo')</strong>
                                <span>^</span>
                            </div>
                        ...
                        <section class="filter-card">
                            <div class="filter-card-header">
                                <strong><span class="filter-title-icon">$</span> @themeT('category.price_range', 'Kho?ng giá')</strong>
                                <span>^</span>
                            </div>
                            <form method="GET" action="{{ request()->url() }}" class="filter-card-body is-stacked">
                                ...
                                <div class="price-range" data-price-range data-min="{{ $minPrice }}" data-max="{{ $maxPrice }}">
                                    ...
                                    <input type="range" min="{{ $minPrice }}" max="{{ $maxPrice }}" value="{{ $selectedMinPrice }}" step="1000" data-price-range-min name="min_price" aria-label="{{ $t('search.sort_price_asc', 'Giá th?p tru?c') }}">
                                    <input type="range" min="{{ $minPrice }}" max="{{ $maxPrice }}" value="{{ $selectedMaxPrice }}" step="1000" data-price-range-max name="max_price" aria-label="{{ $t('search.sort_price_desc', 'Giá cao tru?c') }}">
                                </div>
                    ...
                    <div class="catalog-main">
                        ...
                        @else
                            <section class="empty-state">@themeT('category.empty', 'Chua có s?n ph?m nào trong danh m?c này.')</section>
                        @endif
                    </div>
                </section>
            </main>

            @include('theme-th0020::partials.footer', ['footerContainerClass' => 'th-container'])
        </div>
