...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Ð?t hàng thành công{{ $companyTitle ? ' | '.$companyTitle : '' }}</title>
    ...
    <body class="th-interior-page">
        ...
        <main class="wrap">
            <section class="success-card">
                <h1>@themeT('checkout_success.title', 'Ð?t hàng thành công')</h1>
                <p>@themeT('checkout_success.summary', 'Ðon hàng c?a b?n dã du?c ghi nh?n. B? ph?n cham sóc khách hàng s? liên h? theo s? di?n tho?i b?n cung c?p d? xác nh?n th?i gian giao nh?n ho?c g?i mã voucher.')</p>

                <div class="order-grid">
                    <div class="order-line">
                        <span class="order-label">@themeT('checkout_success.order_code', 'Mã don hàng')</span>
                        <span class="order-value">{{ $confirmedOrder->order_code }}</span>
                    </div>
                    <div class="order-line">
                        <span class="order-label">@themeT('checkout_success.placed_at', 'Th?i gian t?o don')</span>
                    ...
                    <div class="order-line">
                        <span class="order-label">@themeT('checkout_success.phone', 'Ði?n tho?i')</span>
                        <span class="order-value">{{ $confirmedOrder->customer_phone }}</span>
                    </div>
                    <div class="order-line">
                        <span class="order-label">@themeT('checkout_success.address', 'Ð?a ch? nh?n hàng / nh?n mã')</span>
                        <span class="order-value">{{ $confirmedOrder->delivery_address }}</span>
                    </div>
                    <div class="order-line">
                        <span class="order-label">@themeT('checkout_success.payment_method', 'Phuong th?c thanh toán')</span>
                        <span class="order-value">{{ $confirmedOrder->payment_label }}</span>
                    </div>
                    <div class="order-line">
                        <span class="order-label">@themeT('checkout_success.total', 'T?ng thanh toán')</span>
                        <span class="order-value">{{ $formatCurrency($confirmedOrder->subtotal) }}</span>
                    </div>
                    <div class="order-line">
                        <span class="order-label">@themeT('checkout_success.email_status', 'Email xác nh?n')</span>
                    ...
                    <div class="order-line">
                        <span class="order-label">@themeT('checkout_success.sms_status', 'SMS xác nh?n')</span>
                        <span class="order-value">@themeT('checkout_success.sms_paused', 'T?m d?ng')</span>
                    </div>
                </div>
                ...
                <div class="cta-row">
                    <a href="{{ route('site.home') }}" class="cta-link primary">@themeT('checkout_success.continue_shopping', 'Ti?p t?c mua s?m')</a>
                    <a href="tel:{{ preg_replace('/\D+/', '', $contactHotline) }}" class="cta-link secondary">@themeT('checkout_success.hotline_support', 'G?i hotline h? tr?')</a>
                </div>
            </section>
        </main>
