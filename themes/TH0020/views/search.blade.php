...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ $searchQuery !== '' ? str_replace(':query', $searchQuery, $t('search.title_query', 'K?t qu? cho " :query "')) : $t('search.title_default', 'Tìm ki?m s?n ph?m') }}{{ $companyTitle ? ' | '.$companyTitle : '' }}</title>
            ...
            <header class="th-header">
                <div class="th-container th-header-inner">
                    ...
                    <form class="th-search" method="GET" action="{{ route('site.catalog.search') }}" role="search">
                        <input type="search" name="q" value="{{ $searchQuery }}" placeholder="@themeT('common.search_placeholder', 'Tìm ki?m s?n ph?m / khuy?n mãi')" aria-label="@themeT('common.search_aria', 'Tìm ki?m s?n ph?m')" data-th-product-search data-suggest-url="{{ route('site.catalog.search.suggestions') }}">
            ...
            <main>
                <div class="th-container">
                    <div class="breadcrumb">
                        ...
                        <span>/</span>
                        <span>@themeT('search.breadcrumb', 'Tìm ki?m s?n ph?m')</span>
                    </div>

                    <section class="search-hero">
                        <h1>{{ $searchQuery !== '' ? __('search.title_query', ['query' => $searchQuery]) : __('search.title_default') }}</h1>
                        <p>@themeT('search.intro', 'Tìm theo tên s?n ph?m, SKU ho?c n?i dung mô t? d? di?u hu?ng nhanh t?i s?n ph?m th?i trang phù h?p.')</p>
                        ...
                        <form method="GET" action="{{ route('site.catalog.search') }}" class="search-toolbar">
                            <label class="search-field">
                                <span>@themeT('search.keyword', 'T? khóa')</span>
                                <input type="search" name="q" value="{{ $searchFilters['q'] ?? '' }}" placeholder="@themeT('search.keyword_placeholder', 'Tên s?n ph?m, SKU, mô t?')" data-th-product-search data-suggest-url="{{ route('site.catalog.search.suggestions') }}">
                            </label>
                            <label class="search-field">
                            ...
                            <label class="search-field">
                                <span>@themeT('search.sort', 'S?p x?p')</span>
                                <select name="sort">
                            ...
                            <label class="search-field">
                                <span>@themeT('search.price_from', 'Giá t?')</span>
                                <input type="number" min="0" step="1000" name="min_price" value="{{ $searchFilters['min_price'] ?? 0 }}" placeholder="0">
                            </label>
                            <label class="search-field">
                                <span>@themeT('search.price_to', 'Giá d?n')</span>
                                <input type="number" min="0" step="1000" name="max_price" value="{{ $searchFilters['max_price'] ?? 0 }}" placeholder="0">
                            </label>
                            <div class="search-actions">
                                <button type="submit" class="search-button">@themeT('search.apply', 'Áp d?ng')</button>
                                @if ($hasActiveFilters)
                                    <a href="{{ route('site.catalog.search') }}" class="search-reset">@themeT('search.clear', 'Xóa l?c')</a>
                                @endif
                            </div>
                        </form>
                    ...
                    @if ($productCollection->isNotEmpty())
                        ...
                        @if ($pagination && (method_exists($pagination, 'previousPageUrl') || method_exists($pagination, 'nextPageUrl')))
                            <div class="search-pagination">
                                <a href="{{ $pagination->previousPageUrl() ?: '#' }}" class="search-page-link {{ $pagination->previousPageUrl() ? '' : 'is-disabled' }}">@themeT('search.prev_page', 'Trang tru?c')</a>
                    ...
                    @else
                        <section class="empty-state">
                            <h3>@themeT('search.empty_title', 'Chua có s?n ph?m phù h?p')</h3>
                            <p>@themeT('search.empty_summary', 'Hãy th? t? khóa khác nhu tên s?n ph?m, SKU ho?c danh m?c.')</p>
                        </section>
                    @endif
                </div>
            </main>
