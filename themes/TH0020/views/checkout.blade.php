    ...
    <body class="th-interior-page">
        ...
        <main class="wrap">
            <div class="breadcrumb">
                ...
                <span>›</span>
                <a href="{{ route('site.cart.index') }}">@themeT('cart.breadcrumb', 'Gi? hàng')</a>
                <span>›</span>
            ...
            <section class="checkout-layout">
                <form method="POST" action="{{ route('site.checkout.store') }}" class="checkout-panel">
                    @csrf
                    <h1 class="checkout-title">@themeT('checkout.title', 'Thanh toán don hàng')</h1>

                    <h2 class="section-title">@themeT('checkout.shipping_info', 'Thông tin nh?n hàng')</h2>
                    <div class="form-grid">
                        <div class="field">
                            <label for="customer_name">@themeT('checkout.name', 'H? và tên')</label>
                            <input id="customer_name" name="customer_name" value="{{ $form['customer_name'] ?? '' }}" required>
                        </div>
                        <div class="field">
                            <label for="customer_phone">@themeT('checkout.phone', 'S? di?n tho?i')</label>
                        ...
                        <div class="field full">
                            <label for="delivery_address">@themeT('checkout.address', 'Ð?a ch? nh?n hàng / nh?n mã')</label>
                            <textarea id="delivery_address" name="delivery_address" required>{{ $form['delivery_address'] ?? '' }}</textarea>
                        </div>
                        <div class="field full">
                            <label for="note">@themeT('checkout.note', 'Ghi chú cho don hàng')</label>
                            <textarea id="note" name="note">{{ $form['note'] ?? '' }}</textarea>
                        </div>
                    </div>

                    <h2 class="section-title">@themeT('checkout.payment_method', 'Phuong th?c thanh toán')</h2>
                    ...
                    <div style="margin-top: 20px; display: flex; gap: 12px; flex-wrap: wrap;">
                        <button type="submit" class="submit-button">@themeT('checkout.submit', 'Xác nh?n d?t hàng')</button>
                        <a href="{{ route('site.cart.index') }}" style="display:inline-flex;align-items:center;justify-content:center;min-height:48px;padding:0 22px;border:1px solid #b9d99c;color:#4e9620;font-weight:700;">@themeT('checkout.back_to_cart', 'Quay l?i gi? hàng')</a>
                    </div>
                </form>

                <aside class="summary-panel">
                    <h2>@themeT('checkout.summary_title', 'Ðon hàng c?a b?n')</h2>
                    ...
                    <div class="summary-line">
                        <span>@themeT('checkout.total_products', 'T?ng s? s?n ph?m')</span>
