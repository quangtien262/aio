    ...
    return (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 16 }}>
            {themes.map((theme) => (
                ...
                >
                    <div className="theme-thumbnail" style={{ width: '100%', aspectRatio: '1 / 1', overflow: 'hidden', borderRadius: 8, background: '#fff', boxShadow: theme.key === selectedThemeKey ? '0 6px 22px rgba(0,0,0,0.12)' : '0 4px 12px rgba(0,0,0,0.06)' }}>
                        {theme.is_active ? (
                            <Tag color="green" style={{ position: 'absolute', top: 8, right: 8, zIndex: 3 }}>Đang kích hoạt</Tag>
                        ) : null}
                        <img src={theme.preview_urls?.thumbnail || theme.avatar_url || undefined} alt={theme.name} loading="lazy" />
                    </div>
