...
class Ser0101ThemeBlockDefinition implements ThemeBlockDefinition
{
    ...
    private function metricEntries(string $websiteKey, ThemeBlockRegistry $registry): array
    {
        ...
        foreach ($defaults as $index => $metric) {
            $entries[] = new ThemeBlockEntry(
                ...
                sprintf('Khối theme / SER0101 / Chỉ số trang chủ / Mục %d / Giá trị', $index + 1),
                $metric['value'],
            );
            $entries[] = new ThemeBlockEntry(
                $registry->contentKey($this->themeKey(), sprintf('service_metrics.%d.suffix', $index)),
                sprintf('Khối theme / SER0101 / Chỉ số trang chủ / Mục %d / Hậu tố', $index + 1),
                $metric['suffix'],
            );
            $entries[] = new ThemeBlockEntry(
                $registry->contentKey($this->themeKey(), sprintf('service_metrics.%d.label', $index)),
                sprintf('Khối theme / SER0101 / Chỉ số trang chủ / Mục %d / Mô tả', $index + 1),
                $metric['label'],
            );
        }
    ...
    private function quotePanelEntries(ThemeBlockRegistry $registry): array
    {
        ...
        $entries = [new ThemeBlockEntry(
            ...
            'Khối theme / SER0101 / Báo giá trong ngày / Nhãn nổi bật',
            'Báo giá trong ngày',
        )];
        ...
        foreach ($defaults as $index => $item) {
            $entries[] = new ThemeBlockEntry(
                ...
                sprintf('Khối theme / SER0101 / Báo giá trong ngày / Mục %d / Tiêu đề', $index + 1),
                $item['title'],
            );
            $entries[] = new ThemeBlockEntry(
                $registry->contentKey($this->themeKey(), sprintf('quote_panel.items.%d.summary', $index)),
                sprintf('Khối theme / SER0101 / Báo giá trong ngày / Mục %d / Mô tả', $index + 1),
                $item['summary'],
            );
        }
    ...
    {
        return [
            new ThemeBlockEntry($registry->contentKey($this->themeKey(), 'latest_posts.kicker'), 'Khối theme / SER0101 / Tin mới / Nhãn phụ', 'Tin mới'),
            new ThemeBlockEntry($registry->contentKey($this->themeKey(), 'latest_posts.title'), 'Khối theme / SER0101 / Tin mới / Tiêu đề', 'Tin mới'),
            new ThemeBlockEntry(
                $registry->contentKey($this->themeKey(), 'latest_posts.summary'),
                'Khối theme / SER0101 / Tin mới / Mô tả',
