...
class LandingPageBuilder
{
    ...
    {
        /** @var Builder $query */
        $query = CmsPost::query()->with(['featuredMedia', 'category'])->where('status', 'published')
    ...
    private function defaultBlocksForTheme(string $themeKey): array
    {
        return match (strtoupper($themeKey)) {
            'E801' => $this->e801DefaultBlocks(),
            'E802' => $this->e802DefaultBlocks(),
            'E803' => $this->e803DefaultBlocks(),
            'E804' => $this->e804DefaultBlocks(),
            'E806' => $this->e806DefaultBlocks(),
            'E807' => $this->e807DefaultBlocks(),
            'E805' => $this->e805DefaultBlocks(),
            'E800' => $this->e800DefaultBlocks(),
    ...
    private function e806DefaultBlocks(): array
    {
        ...
        $schema = fn (int $limit): array => ['source' => ['type' => 'select', 'label' => 'Nguồn dữ liệu', 'options' => ['cms_products' => 'Sản phẩm', 'custom' => 'Tự nhập']], 'limit' => ['type' => 'number', 'label' => 'Số mục', 'default' => $limit], 'category_id' => ['type' => 'select', 'label' => 'Danh mục'], 'featured_only' => ['type' => 'boolean', 'label' => 'Chỉ lấy mục nổi bật']];
        $names = ['Tivi Vision 55 inch 4K', 'Tivi Lumi 49 inch', 'Máy hút bụi SteamGo', 'Ống kính Creator 24mm', 'Máy tính bảng Mini 8', 'Laptop Air 14 Plus', 'Tai nghe SoundFit Pro', 'Máy chiếu Cinema Home', 'Điện thoại Nova S', 'Máy ảnh Mirror X', 'Nồi chiên AirChef 6L', 'Máy xay FreshMix', 'Áo sơ mi Linen Blue', 'Balo Urban Trail', 'Túi thổ cẩm Heritage', 'Đồng hồ Active Round'];
        $products = collect($names)->map(fn ($name, $index) => ['title' => $name, 'image' => $index % 2 ? $hero : $tech, 'price' => 339000 + $index * 935000, 'original_price' => $index % 3 ? 590000 + $index * 1120000 : null, 'url' => '#'])->all();
        $stores = collect(['Hai Bà Trưng', 'Hà Đông', 'Đội Cấn', 'Cầu Giấy', 'Long Biên', 'Thanh Xuân', 'Nam Từ Liêm', 'Hoàn Kiếm'])->map(fn ($district) => ['title' => 'E806 Market '.$district, 'address' => 'Trung tâm '.$district.', Hà Nội', 'phone' => '1900 6750'])->all();
        $brands = collect(['NOVA', 'LUMI', 'HOMEA', 'AURORA', 'VISION', 'E806 SELECT'])->map(fn ($name) => ['title' => $name])->all();
        $block = fn (string $type, string $label, int $limit, array $items): array => ['block_type' => $type, 'label' => $label, 'description' => $label.' của theme điện máy E806.', 'preview_image' => $preview, 'anchor_id' => str_replace('_', '-', $type), 'dynamic' => true, 'settings' => ['source' => 'cms_products', 'limit' => $limit], 'settings_schema' => $schema($limit), 'data' => ['vi' => array_merge($heading($label), $withItems($items)), 'en' => $heading($label)]];

        return [
            ['block_type' => 'hero_market', 'label' => 'Hero điện máy', 'description' => 'Mega menu, hero gia dụng và ba lợi ích.', 'preview_image' => $preview, 'anchor_id' => 'top', 'dynamic' => true, 'settings' => ['placement' => 'e806-hero-market', 'limit' => 1], 'settings_schema' => ['placement' => ['type' => 'text', 'label' => 'Placement']], 'media' => ['image_url' => $hero], 'data' => ['vi' => $heading('Bếp đẹp, nhà vui', 'Bộ sưu tập gia dụng mới', 'Thiết kế tinh tế · tiện nghi mỗi ngày'), 'en' => $heading('A happier modern kitchen', 'New appliance collection')]],
            $block('e806_featured_tabs', 'Sản phẩm nổi bật', 4, array_slice($products, 0, 4)),
            $block('e806_electronics', 'Điện tử và gia dụng', 10, array_slice($products, 0, 10)),
            ['block_type' => 'e806_promo_duo', 'label' => 'Banner công nghệ đôi', 'description' => 'Hai banner sản phẩm công nghệ.', 'preview_image' => $preview, 'anchor_id' => 'khuyen-mai', 'settings' => [], 'media' => ['image_url' => $tech], 'data' => ['vi' => $heading('Ưu đãi công nghệ'), 'en' => $heading('Technology offers')]],
            $block('e806_gifts', 'Quà tặng và phong cách', 4, array_slice($products, 12, 4)),
            ['block_type' => 'e806_brands', 'label' => 'Thương hiệu đồng hành', 'description' => 'Dải thương hiệu dạng chữ.', 'preview_image' => $preview, 'anchor_id' => 'thuong-hieu', 'settings' => [], 'data' => ['vi' => array_merge($heading('Thương hiệu đồng hành'), $withItems($brands)), 'en' => $heading('Partner brands')]],
            ['block_type' => 'e806_stores', 'label' => 'Hệ thống cửa hàng', 'description' => 'Danh sách tám điểm bán.', 'preview_image' => $preview, 'anchor_id' => 'cua-hang', 'settings' => [], 'data' => ['vi' => array_merge($heading('Hệ thống cửa hàng'), $withItems($stores)), 'en' => $heading('Store network')]],
        ];
    }
    ...
    private function e805DefaultBlocks(): array
    {
        ...
        return [
            ['block_type'=>'hero_slider','label'=>'Hero siêu sale','description'=>'Banner sale lớn đầu trang.','preview_image'=>$preview,'anchor_id'=>'top','dynamic'=>true,'settings'=>['placement'=>'e805-hero-slider','limit'=>3],'settings_schema'=>['placement'=>['type'=>'text','label'=>'Placement'],'limit'=>['type'=>'number','label'=>'Số slide']],'media'=>['image'=>$hero],'data'=>['vi'=>array_merge($heading('Siêu sale làm bạn xiêu lòng'),['content'=>['slides'=>[['title'=>'Siêu sale','image'=>$hero]]]]),'en'=>$heading('Super sale')]],
            ['block_type'=>'e805_categories','label'=>'Danh mục sản phẩm','description'=>'Mười danh mục tròn dưới hero.','preview_image'=>$preview,'anchor_id'=>'danh-muc','settings'=>[],'data'=>['vi'=>array_merge($heading('Danh mục sản phẩm'),$with($cats)),'en'=>$heading('Categories')]],
            $block('e805_sale_tabs','Sale theo ngành hàng',5,array_slice($products,0,5)),$block('e805_technology','Đồ công nghệ',8,array_slice($products,0,8)),$block('e805_picks','Gợi ý hôm nay',5,array_slice($products,2,5)),$block('e805_news','Tin tức mới nhất',4,$news,true),
        ];
    }
    ...
    private function e804DefaultBlocks(): array
    {
        ...
        $catNames=[['Đồ Chơi - Mẹ & Bé','fa-solid fa-baby'],['Điện Thoại - Máy Tính Bảng','fa-solid fa-mobile-screen'],['Làm Đẹp - Sức Khỏe','fa-solid fa-spa'],['Điện Gia Dụng','fa-solid fa-blender'],['Phụ kiện thời trang','fa-solid fa-bag-shopping'],['Đồng hồ và Trang sức','fa-solid fa-clock'],['Laptop - Máy Vi Tính','fa-solid fa-laptop'],['Nhà cửa & Đời sống','fa-solid fa-house'],['Bách Hóa Online','fa-solid fa-basket-shopping'],['Thiết Bị Số','fa-solid fa-headphones'],['Điện Tử - Điện Lạnh','fa-solid fa-tv']];$cats=collect($catNames)->map(fn($x)=>['title'=>$x[0],'icon'=>$x[1],'url'=>'#san-pham'])->all();
        $featured=collect([['Tivi','fa-solid fa-tv'],['Điện thoại','fa-solid fa-mobile-screen'],['Máy ảnh','fa-solid fa-camera'],['Tủ lạnh','fa-solid fa-box'],['Điều hòa','fa-solid fa-wind'],['Máy giặt','fa-solid fa-shirt'],['Laptop','fa-solid fa-laptop'],['Quạt','fa-solid fa-fan'],['Đồ nhà bếp','fa-solid fa-kitchen-set'],['Thời trang nam','fa-solid fa-shirt'],['Thời trang nữ','fa-solid fa-person-dress'],['Đồ chơi','fa-solid fa-puzzle-piece'],['Sắc đẹp','fa-solid fa-spa'],['Thể thao','fa-solid fa-futbol'],['Đồng hồ','fa-solid fa-clock'],['Balo & túi','fa-solid fa-bag-shopping'],['Sách truyện','fa-solid fa-book'],['Giày dép','fa-solid fa-shoe-prints']])->map(fn($x)=>['title'=>$x[0],'icon'=>$x[1],'url'=>'#san-pham'])->all();
        $brands=collect(['NOVA TECH','BABY SOFT','PURE LAB','AQUA CARE','HOMEA','LUMINA'])->map(fn($x,$i)=>['title'=>$x,'image'=>$i%2?$hero:$campaign])->all();$news=collect(['Bí quyết săn ưu đãi đúng lúc và tiết kiệm','Cách chọn thiết bị gia dụng phù hợp căn bếp','Gợi ý quà tặng thiết thực cho gia đình','Chăm sóc da tối giản cho người bận rộn','Mẹo sắp xếp góc làm việc gọn gàng','Xu hướng tiêu dùng thông minh năm nay'])->map(fn($x,$i)=>['title'=>$x,'summary'=>'Những gợi ý hữu ích giúp bạn lựa chọn sản phẩm phù hợp và mua sắm hiệu quả.','image'=>$i%2?$hero:$campaign,'url'=>'#'])->all();
        $block=fn($type,$label,$count,$data,$dynamic=true,$posts=false)=>['block_type'=>$type,'label'=>$label,'description'=>$label.' của giao diện marketplace E804.','preview_image'=>$preview,'anchor_id'=>str_replace('_','-',$type),'dynamic'=>$dynamic,'settings'=>$dynamic?['source'=>$posts?'cms_posts':'cms_products','limit'=>$count]:[],'settings_schema'=>$dynamic?$source($count,$posts?['cms_posts'=>'Tin tức','custom'=>'Tự nhập']:['cms_products'=>'Sản phẩm','custom'=>'Tự nhập']):[],'data'=>['vi'=>array_merge($heading($label),$withItems($data)),'en'=>$heading($label)]];
        return [
            ['block_type'=>'hero_slider','label'=>'Hero marketplace','description'=>'Hero chính và mega menu danh mục.','preview_image'=>$preview,'anchor_id'=>'top','dynamic'=>true,'settings'=>['placement'=>'e804-hero-slider','limit'=>3],'settings_schema'=>['placement'=>['type'=>'text','label'=>'Placement banner'],'limit'=>['type'=>'number','label'=>'Số slide']],'media'=>['image'=>$hero],'data'=>['vi'=>array_merge($heading('Hàng hay giá tốt mỗi ngày','Khám phá hàng ngàn sản phẩm tuyển chọn.'),['content'=>['slides'=>[['title'=>'Hàng hay giá tốt mỗi ngày','image'=>$hero]]]]),'en'=>$heading('Great products, better prices')]],
            $block('e804_categories','Danh mục marketplace',11,$cats,false),$block('e804_daily_deals','Giá tốt mỗi ngày',5,array_slice($products,0,5)),$block('e804_featured_categories','Danh mục nổi bật',18,$featured,false),$block('e804_official_brands','Thương hiệu chính hãng',6,$brands,false),$block('e804_best_brands','Thương hiệu bán chạy',6,array_slice($products,4,6)),$block('e804_weekly_deals','Deal hot trong tuần',7,array_slice($products,6,7)),$block('e804_recommendations','Gợi ý hôm nay',10,array_slice($products,0,10)),$block('e804_news','Tin tức mới nhất',6,$news,true,true),
        ];
    }
    ...
    private function e803DefaultBlocks(): array
    {
        ...
        $catNames=[['Laptop - Máy tính xách tay','fa-solid fa-laptop'],['PC Gaming','fa-solid fa-computer'],['Laptop Gaming - Đồ họa','fa-solid fa-laptop-code'],['Phụ kiện Laptop, PC','fa-solid fa-plug'],['PC Đồ họa, Thiết kế, AI','fa-solid fa-wand-magic-sparkles'],['PC Văn phòng - Làm việc','fa-solid fa-briefcase'],['Bàn phím, Chuột - Gaming Gear','fa-solid fa-keyboard'],['Linh kiện máy tính','fa-solid fa-microchip'],['Màn hình máy tính','fa-solid fa-display'],['Máy chủ - Máy đồng bộ','fa-solid fa-server'],['Thiết bị văn phòng','fa-solid fa-print'],['Thiết bị lưu trữ','fa-solid fa-hard-drive'],['Loa, Tai nghe, Webcam','fa-solid fa-headphones'],['Cooling, Tản nhiệt','fa-solid fa-fan'],['Thiết bị mạng, Camera','fa-solid fa-wifi']];
        $cats=collect($catNames)->map(fn($x)=>['title'=>$x[0],'icon'=>$x[1],'url'=>'#san-pham'])->all();
        $promos=collect(['Laptop học tập – Mua 1 tặng 5','Laptop văn phòng – Nâng cấp thông minh','Build PC – Quà tặng đến 5 triệu','Gaming Gear – Deal đến 30%'])->map(fn($title,$i)=>['title'=>$title,'summary'=>'Ưu đãi giới hạn dành cho khách hàng E803.','image'=>$i%2?$hero:$campaign,'url'=>'#'])->all();
        $news=collect(['Khắc phục sự cố Wi-Fi trên Windows hiệu quả','Chọn laptop OLED cho công việc sáng tạo','Tối ưu nhiệt độ khi chơi game dài giờ','Cách chọn màn hình QHD đúng nhu cầu','Nâng cấp SSD: điều cần biết trước khi mua','Bàn phím cơ nào phù hợp cho văn phòng?','Xây dựng PC làm đồ họa theo ngân sách','Bảo dưỡng máy tính để bàn đúng cách'])->map(fn($title,$i)=>['title'=>$title,'summary'=>'Kiến thức công nghệ thực tế, dễ áp dụng cho công việc, học tập và giải trí.','image'=>$i%2?$hero:$campaign,'url'=>'#'])->all();
        return [
            ['block_type'=>'hero_slider','label'=>'Hero PC Gaming','description'=>'Banner chiến dịch lớn đầu trang.','preview_image'=>$preview,'anchor_id'=>'top','dynamic'=>true,'settings'=>['placement'=>'e803-hero-slider','limit'=>3],'settings_schema'=>['placement'=>['type'=>'text','label'=>'Placement banner'],'limit'=>['type'=>'number','label'=>'Số slide']],'media'=>['image'=>$hero],'data'=>['vi'=>array_merge($heading('Cỗ máy cho','mọi cuộc chơi','Khám phá hệ sinh thái PC và gaming gear được tuyển chọn.'),['content'=>['slides'=>[['title'=>'Cỗ máy cho mọi cuộc chơi','image'=>$hero]]]]),'en'=>$heading('Power for','every play','Curated PCs and gaming gear.')]],
            ['block_type'=>'e803_categories','label'=>'Danh mục nổi bật','description'=>'Danh mục PC và thiết bị công nghệ.','preview_image'=>$preview,'anchor_id'=>'danh-muc','settings'=>[],'data'=>['vi'=>array_merge($heading('Danh mục nổi bật'),$withItems($cats)),'en'=>$heading('Featured categories')]],
            ['block_type'=>'e803_deals','label'=>'Giá tốt mỗi ngày','description'=>'Deal theo giờ với năm sản phẩm.','preview_image'=>$preview,'anchor_id'=>'deals','dynamic'=>true,'settings'=>['source'=>'cms_products','limit'=>5,'featured_only'=>true],'settings_schema'=>$source(5),'data'=>['vi'=>array_merge($heading('Giá tốt mỗi ngày'),$withItems(array_slice($products,0,5))),'en'=>$heading('Daily deals')]],
            ['block_type'=>'e803_campaigns','label'=>'Banner chiến dịch kép','description'=>'Hai chiến dịch laptop và phụ kiện.','preview_image'=>$preview,'anchor_id'=>'chien-dich','settings'=>[],'data'=>['vi'=>array_merge($heading(),$withItems(array_slice($promos,0,2))),'en'=>$heading()]],
            ['block_type'=>'e803_mouse_feature','label'=>'Chuột gaming bán chạy','description'=>'Ba sản phẩm và banner dọc.','preview_image'=>$preview,'anchor_id'=>'chuot-ban-chay','dynamic'=>true,'settings'=>['source'=>'cms_products','limit'=>3],'settings_schema'=>$source(3),'data'=>['vi'=>array_merge($heading('Chuột gaming bán chạy nhất'),$withItems(array_slice($products,3,3))),'en'=>$heading('Best-selling gaming mice')]],
            ['block_type'=>'e803_catalog','label'=>'Lưới sản phẩm theo hãng','description'=>'Năm sản phẩm với bộ lọc nhanh.','preview_image'=>$preview,'anchor_id'=>'san-pham','dynamic'=>true,'settings'=>['source'=>'cms_products','limit'=>5],'settings_schema'=>$source(5),'data'=>['vi'=>array_merge($heading('Chọn danh mục'),$withItems(array_slice($products,4,5))),'en'=>$heading('Browse products')]],
            ['block_type'=>'e803_gpu','label'=>'Card đồ họa nổi bật','description'=>'Ba sản phẩm GPU và banner chiến dịch.','preview_image'=>$preview,'anchor_id'=>'card-do-hoa','dynamic'=>true,'settings'=>['source'=>'cms_products','limit'=>3],'settings_schema'=>$source(3),'data'=>['vi'=>array_merge($heading('Card đồ họa'),$withItems(array_slice($products,7,3))),'en'=>$heading('Graphics cards')]],
            ['block_type'=>'e803_build','label'=>'Banner Build PC','description'=>'Banner ngang kêu gọi xây dựng cấu hình.','preview_image'=>$preview,'anchor_id'=>'build-pc','media'=>['image'=>$campaign],'settings'=>[],'data'=>['vi'=>$heading('Cấu hình chất, quà siêu khủng',null,'Đội ngũ kỹ thuật đồng hành từ lựa chọn linh kiện đến tối ưu hiệu năng.'),'en'=>$heading('Build boldly',null,'Expert support from component selection to performance tuning.')]],
            ['block_type'=>'e803_peripherals','label'=>'Phụ kiện theo tab','description'=>'Bàn phím, màn hình, mainboard và RAM.','preview_image'=>$preview,'anchor_id'=>'phu-kien','dynamic'=>true,'settings'=>['source'=>'cms_products','limit'=>4],'settings_schema'=>$source(4),'data'=>['vi'=>array_merge($heading('Gaming Gear'),$withItems(array_slice($products,5,4))),'en'=>$heading('Gaming gear')]],
            ['block_type'=>'e803_promotions','label'=>'Chuyên trang khuyến mãi','description'=>'Bốn thẻ chiến dịch khuyến mãi.','preview_image'=>$preview,'anchor_id'=>'khuyen-mai','settings'=>[],'data'=>['vi'=>array_merge($heading('Chuyên trang khuyến mãi'),$withItems($promos)),'en'=>$heading('Promotion hubs')]],
            ['block_type'=>'e803_news','label'=>'Tin tức - Review','description'=>'Tám bài viết công nghệ mới.','preview_image'=>$preview,'anchor_id'=>'tin-tuc','dynamic'=>true,'settings'=>['source'=>'cms_posts','limit'=>8],'settings_schema'=>$source(8,['cms_posts'=>'Tin tức','custom'=>'Tự nhập']),'data'=>['vi'=>array_merge($heading('Tin tức - Review'),$withItems($news)),'en'=>$heading('News and reviews')]],
        ];
    }
    ...
    private function e802DefaultBlocks(): array
    {
        ...
        $cats=collect(['Xe tay ga','Xe số','Xe côn tay','Xe phân khối lớn'])->map(fn($title,$i)=>['title'=>$title,'image'=>$i%2?$sport:$hero,'url'=>'#san-pham'])->all();
        $campaigns=[['title'=>'Đặt tiền liền tay','subtitle'=>'Nhận ngay ưu đãi','image'=>$sport],['title'=>'Phong cách mới','subtitle'=>'Chính thức trình làng','image'=>$sport]];
        $partsGroups=collect(['Đô thị','Thể thao','Đường dài'])->map(fn($group,$g)=>['title'=>$group,'items'=>collect(range(1,3))->map(fn($i)=>['title'=>['Bộ nhông, xích','Bộ bảo dưỡng chính hãng','Lốp xe bền bỉ'][($i+$g)%3],'price'=>number_format(95000+($g*85000)+($i*53000),0,',','.').'đ'])->all()])->all();
        $news=collect(['Kinh nghiệm chọn xe đô thị phù hợp','Khám phá xe thể thao thế hệ mới','Bí quyết bảo dưỡng xe bền đẹp','Lịch lái thử cuối tuần','Phụ kiện an toàn cho mọi hành trình','Ưu đãi thành viên tháng này'])->map(fn($title,$i)=>['title'=>$title,'summary'=>'Thông tin mới nhất dành cho người yêu xe và hành trình đô thị hiện đại.','image'=>$i%2?$sport:$hero,'url'=>'#'])->all();
        return [
            ['block_type'=>'hero_slider','label'=>'Hero xe máy','description'=>'Banner lớn đầu trang.','preview_image'=>$preview,'anchor_id'=>'top','dynamic'=>true,'settings'=>['placement'=>'e802-hero-slider','limit'=>3],'settings_schema'=>['placement'=>['type'=>'text','label'=>'Placement banner'],'limit'=>['type'=>'number','label'=>'Số slide']],'media'=>['image'=>$hero],'data'=>['vi'=>array_merge($heading('Một lựa chọn','chuẩn phong cách','Khám phá hành trình đô thị theo cách của bạn.'),['content'=>['slides'=>[['title'=>'Một lựa chọn chuẩn phong cách','image'=>$hero]]]]),'en'=>$heading('One choice','distinctive style','Discover your urban journey.')]],
            ['block_type'=>'e802_categories','label'=>'Danh mục nổi bật','description'=>'Bốn dòng xe chính.','preview_image'=>$preview,'anchor_id'=>'danh-muc','settings'=>[],'data'=>['vi'=>array_merge($heading('Danh mục nổi bật'),$withItems($cats)),'en'=>$heading('Featured categories')]],
            ['block_type'=>'e802_deals','label'=>'Deal nổi bật','description'=>'Năm sản phẩm ưu đãi.','preview_image'=>$preview,'anchor_id'=>'deal','dynamic'=>true,'settings'=>['source'=>'cms_products','limit'=>5],'settings_schema'=>$source(5),'data'=>['vi'=>array_merge($heading('Deal nổi bật',null,'Ưu đãi giới hạn dành cho khách hàng đặt xe sớm!'),$withItems(array_slice($products,0,5))),'en'=>$heading('Hot deals')]],
            ['block_type'=>'e802_campaigns','label'=>'Banner chiến dịch kép','description'=>'Hai banner quảng cáo ngang.','preview_image'=>$preview,'anchor_id'=>'chien-dich','settings'=>[],'data'=>['vi'=>array_merge($heading(),$withItems($campaigns)),'en'=>$heading()]],
            ['block_type'=>'e802_featured','label'=>'Sản phẩm nổi bật','description'=>'Banner lifestyle và hai sản phẩm.','preview_image'=>$preview,'anchor_id'=>'noi-bat','dynamic'=>true,'settings'=>['source'=>'cms_products','limit'=>2],'settings_schema'=>$source(2),'data'=>['vi'=>array_merge($heading('Sản phẩm nổi bật'),$withItems(array_slice($products,5,2))),'en'=>$heading('Featured products')]],
            ['block_type'=>'e802_catalog','label'=>'Tất cả sản phẩm','description'=>'Lưới sản phẩm lớn theo dòng xe.','preview_image'=>$preview,'anchor_id'=>'san-pham','dynamic'=>true,'settings'=>['source'=>'cms_products','limit'=>9],'settings_schema'=>$source(9),'data'=>['vi'=>array_merge($heading('Tất cả sản phẩm'),$withItems(array_slice($products,0,9))),'en'=>$heading('All products')]],
            ['block_type'=>'e802_best','label'=>'Sản phẩm bán chạy','description'=>'Năm sản phẩm bán chạy.','preview_image'=>$preview,'anchor_id'=>'ban-chay','dynamic'=>true,'settings'=>['source'=>'cms_products','limit'=>5,'featured_only'=>true],'settings_schema'=>$source(5),'data'=>['vi'=>array_merge($heading('Sản phẩm bán chạy'),$withItems(array_slice($products,3,5))),'en'=>$heading('Best sellers')]],
            ['block_type'=>'e802_parts','label'=>'Phụ tùng theo dòng xe','description'=>'Ba cột phụ tùng.','preview_image'=>$preview,'anchor_id'=>'phu-tung','settings'=>[],'data'=>['vi'=>array_merge($heading('Phụ tùng chính hãng'),['content'=>['groups'=>$partsGroups]]),'en'=>$heading('Genuine parts')]],
            ['block_type'=>'e802_brands','label'=>'Thương hiệu nổi bật','description'=>'Các nhãn dòng xe phân phối.','preview_image'=>$preview,'anchor_id'=>'thuong-hieu','settings'=>[],'data'=>['vi'=>array_merge($heading('Thương hiệu nổi bật'),$withItems(collect(['URBAN','MOTION','E802'])->map(fn($x)=>['title'=>$x])->all())),'en'=>$heading('Featured brands')]],
            ['block_type'=>'e802_testimonials','label'=>'Đánh giá khách hàng','description'=>'Hai phản hồi thành viên.','preview_image'=>$preview,'anchor_id'=>'danh-gia','settings'=>[],'data'=>['vi'=>array_merge($heading('Đánh giá khách hàng'),$withItems([['title'=>'Hoàng Dung','summary'=>'Tôi rất ưng ý với chất lượng xe, phụ tùng và thái độ tư vấn nhiệt tình.'],['title'=>'Sở Bình','summary'=>'Showroom hỗ trợ nhanh chóng, rõ ràng và luôn đồng hành sau khi mua xe.']])),'en'=>$heading('Customer reviews')]],
            ['block_type'=>'e802_news','label'=>'Tin tức mới nhất','description'=>'Tin tức và sự kiện xe máy.','preview_image'=>$preview,'anchor_id'=>'tin-tuc','dynamic'=>true,'settings'=>['source'=>'cms_posts','limit'=>6],'settings_schema'=>$source(6,['cms_posts'=>'Tin tức','custom'=>'Tự nhập']),'data'=>['vi'=>array_merge($heading('Tin tức mới nhất'),$withItems($news)),'en'=>$heading('Latest news')]],
            ['block_type'=>'e802_benefits','label'=>'Quyền lợi dịch vụ','description'=>'Bốn chính sách nổi bật.','preview_image'=>$preview,'anchor_id'=>'quyen-loi','settings'=>[],'data'=>['vi'=>array_merge($heading(),$withItems([['title'=>'Miễn phí vận chuyển','summary'=>'Cho đơn hàng nội thành','icon'=>'fa-solid fa-truck-fast'],['title'=>'Miễn phí đổi – trả','summary'=>'Với lỗi sản xuất hoặc vận chuyển','icon'=>'fa-solid fa-rotate'],['title'=>'Hỗ trợ nhanh chóng','summary'=>'Tư vấn xuyên suốt hành trình','icon'=>'fa-solid fa-headset'],['title'=>'Ưu đãi thành viên','summary'=>'Nhiều quyền lợi dành riêng','icon'=>'fa-solid fa-gift']])),'en'=>$heading()]],
        ];
    }
    ...
    private function e801DefaultBlocks(): array
    {
        ...
        $bikeImages=['https://images.unsplash.com/photo-1571068316344-75bc76f77890?auto=format&fit=crop&w=900&q=85','https://images.unsplash.com/photo-1502744688674-c619d1586c9e?auto=format&fit=crop&w=900&q=85','https://images.unsplash.com/photo-1541625602330-2277a4c46182?auto=format&fit=crop&w=900&q=85','https://images.unsplash.com/photo-1558981806-ec527fa84c39?auto=format&fit=crop&w=900&q=85','https://images.unsplash.com/photo-1529422643029-d4585747aaf2?auto=format&fit=crop&w=900&q=85','https://images.unsplash.com/photo-1507035895480-2b3156c31fc8?auto=format&fit=crop&w=900&q=85'];
        $products=collect(range(0,11))->map(fn(int $i):array=>['title'=>['Xe Đạp Đường Phố Velo City 26 inch','Xe Đạp Thể Thao Velo GN Pro','Xe Đạp Touring Urban Flow','Xe Đạp City Basket 24 inch','Xe Đạp Trẻ Em Happy Fish','Xe Đạp Trẻ Em Mini Rider'][$i%6].' '.($i+1),'summary'=>$i<6?'E801 URBAN':'E801 KIDS','image'=>$bikeImages[$i%6],'price'=>1390000+$i*170000,'original_price'=>2990000+$i*210000,'url'=>'#'])->all();
        $trends=[['title'=>'Xe đạp địa hình','price_label'=>'Từ 990.000đ','image'=>'/themes/E801/images/road-banner.png','url'=>'#san-pham'],['title'=>'Xe đạp trẻ em','price_label'=>'Từ 1.590.000đ','image'=>'/themes/E801/images/hero-summer.png','url'=>'#san-pham'],['title'=>'Xe đạp đường phố','price_label'=>'Từ 1.190.000đ','image'=>'/themes/E801/images/road-banner.png','url'=>'#san-pham']];
        $guideImages=['/themes/E801/images/road-banner.png','/themes/E801/images/hero-summer.png','/themes/E801/images/road-banner.png','/themes/E801/images/hero-summer.png','/themes/E801/images/road-banner.png'];
        $guides=collect(['Hướng dẫn chọn mua và cách lắp chắn bùn xe đạp chi tiết','Cách đọc thông số lốp xe đạp đúng chuẩn, bạn đã biết chưa?','Đồng hồ đo tốc độ xe đạp là gì? Phân loại và cách sử dụng','Khung xe đạp làm bằng gì? 5 vật liệu được sử dụng phổ biến','Nguyên nhân khiến phanh xe đạp bị kêu và cách khắc phục'])->map(fn($title,$i)=>['title'=>$title,'summary'=>'Kinh nghiệm thực tế giúp bạn lựa chọn, sử dụng và bảo dưỡng xe đạp an toàn, hiệu quả.','image'=>$guideImages[$i],'url'=>'#'])->all();
        return [
            ['block_type'=>'hero_slider','label'=>'Hero khuyến mãi mùa hè','description'=>'Banner lớn đầu trang lấy từ SiteBanner.','preview_image'=>$preview,'anchor_id'=>'top','dynamic'=>true,'settings'=>['placement'=>'e801-hero-slider','limit'=>3,'autoplay_ms'=>6500],'settings_schema'=>['placement'=>['type'=>'text','label'=>'Placement banner'],'limit'=>['type'=>'number','label'=>'Số slide'],'autoplay_ms'=>['type'=>'number','label'=>'Tốc độ tự chạy (ms)']],'media'=>['image'=>$hero],'data'=>['vi'=>array_merge($heading('Đón hè năng động – Rộn ràng ưu đãi',null,'Ưu đãi mùa hè dành cho mọi hành trình'),['content'=>['deals'=>[['title'=>'Mua xe đạp tặng phụ kiện','value'=>'Đến 700K'],['title'=>'Xe đạp trẻ em','value'=>'Giảm đến 50%'],['title'=>'MTB · Touring · Road','value'=>'Giảm đến 40%'],['title'=>'Tour du lịch bằng xe đạp','value'=>'Giảm đến 15%']],'slides'=>[['title'=>'Đón hè năng động – Rộn ràng ưu đãi','summary'=>'Ưu đãi mùa hè dành cho mọi hành trình','image'=>$hero,'link_url'=>'#san-pham']]]]),'en'=>$heading('Ride into summer',null,'Bright deals for every journey')]],
            ['block_type'=>'e801_categories','label'=>'Danh mục nhanh','description'=>'Bảy danh mục icon ngay dưới hero.','preview_image'=>$preview,'anchor_id'=>'danh-muc','settings'=>[],'data'=>['vi'=>array_merge($heading(),$withItems([['title'=>'Xe đạp','icon'=>'fa-solid fa-bicycle'],['title'=>'Phụ tùng','icon'=>'fa-solid fa-compact-disc'],['title'=>'Phụ kiện','icon'=>'fa-solid fa-bottle-water'],['title'=>'Xe đạp đua','icon'=>'fa-solid fa-person-biking'],['title'=>'Xe đạp trẻ em','icon'=>'fa-solid fa-child-reaching'],['title'=>'Xe đạp gấp - Folding','icon'=>'fa-solid fa-route'],['title'=>'Xe đạp đường phố','icon'=>'fa-solid fa-person-biking']])), 'en'=>$heading()]],
            ['block_type'=>'e801_trends','label'=>'Sản phẩm xu hướng','description'=>'Ba banner xu hướng theo dòng xe.','preview_image'=>$preview,'anchor_id'=>'xu-huong','settings'=>[],'data'=>['vi'=>array_merge($heading('Sản phẩm xu hướng'),$withItems($trends)),'en'=>$heading('Trending products')]],
            ['block_type'=>'e801_street_bikes','label'=>'Xe đạp đường phố','description'=>'Banner thương hiệu và sản phẩm đường phố.','preview_image'=>$preview,'anchor_id'=>'san-pham','dynamic'=>true,'settings'=>['source'=>'cms_products','limit'=>6],'settings_schema'=>$source(6),'data'=>['vi'=>array_merge($heading('Xe đạp đường phố','Cả tháng tư săn deal bự'),['content'=>['brands'=>['GIANT','TWITTER','TOTEM','FOREVER','JAVA','MERIDA','PACIFIC','PHOENIX'],'items'=>array_slice($products,0,6)]]),'en'=>$heading('City bikes','Big monthly deals')]],
            ['block_type'=>'e801_kids_bikes','label'=>'Xe đạp trẻ em','description'=>'Sản phẩm trẻ em và banner thương hiệu.','preview_image'=>$preview,'anchor_id'=>'xe-tre-em','dynamic'=>true,'settings'=>['source'=>'cms_products','limit'=>6],'settings_schema'=>$source(6),'data'=>['vi'=>array_merge($heading('Xe đạp trẻ em','Ưu đãi xe đạp trẻ em'),['content'=>['brands'=>['KENT','CHIPMUNK','FORNIX','THỐNG NHẤT','ROYAL BABY','AVACYCLE','XDS','GAMMAX'],'items'=>array_slice($products,6,6)]]),'en'=>$heading('Kids bikes','Kids bike offers')]],
            ['block_type'=>'e801_road_banner','label'=>'Banner xe đạp đua','description'=>'Banner ngang chiến dịch ROAD.','preview_image'=>$preview,'anchor_id'=>'road','settings'=>['cta_url'=>'#san-pham'],'media'=>['image'=>$road],'data'=>['vi'=>$heading('ROAD','Xe đạp đua','Từ 2.900.000đ','Mua ngay'),'en'=>$heading('ROAD','Race bikes','From 2,900,000 VND','Buy now')]],
            ['block_type'=>'e801_guides','label'=>'Kinh nghiệm chọn mua xe','description'=>'Bài viết cẩm nang và khuyến mãi nổi bật.','preview_image'=>$preview,'anchor_id'=>'kinh-nghiem','dynamic'=>true,'settings'=>['source'=>'cms_posts','limit'=>5],'settings_schema'=>$source(5,['cms_posts'=>'Tin tức','custom'=>'Tự nhập']),'data'=>['vi'=>array_merge($heading('Kinh nghiệm chọn mua xe','Khuyến mãi nổi bật'),$withItems($guides)),'en'=>$heading('Bicycle buying guides','Featured promotions')]],
        ];
    }
    ...
    private function e800DefaultBlocks(): array
    {
        ...
        $heading = fn (?string $title = null, ?string $subtitle = null, ?string $description = null, ?string $button = null): array => array_filter(['title' => $title, 'subtitle' => $subtitle, 'description' => $description, 'button_label' => $button], fn ($value) => $value !== null);
        $withItems = fn (array $items): array => ['content' => ['items' => $items]];
        $source = fn (int $limit, array $options = ['cms_products' => 'Sản phẩm', 'custom' => 'Tự nhập']): array => [
        ...
        $products = collect(range(0, 9))->map(fn (int $index): array => [
            'title' => ['E800 Velocity Pro', 'E800 Urban Pace', 'E800 Court Motion', 'E800 Trail Core', 'E800 Flex Runner'][$index % 5].' '.($index + 1),
            'summary' => $index % 2 ? 'E800 ACTIVE' : 'E800 PERFORMANCE',
            'image' => $productImages[$index % count($productImages)],
            'price' => 1290000 + ($index * 85000),
            'original_price' => 2490000 + ($index * 110000),
            'variant' => ['Đỏ / Size 40', 'Trắng / Size 39', 'Đen / Size 41'][$index % 3],
            'url' => '#',
        ])->all();
        $collections = [
            ['title' => 'Giày chạy bộ', 'image' => $productImages[0], 'url' => '#san-pham'],
            ['title' => 'Giày đi bộ', 'image' => $productImages[1], 'url' => '#san-pham'],
            ['title' => 'Giày Tennis', 'image' => $productImages[2], 'url' => '#san-pham'],
            ['title' => 'Giày thời trang', 'image' => $productImages[3], 'url' => '#san-pham'],
            ['title' => 'Giày Trekking', 'image' => $productImages[4], 'url' => '#san-pham'],
            ['title' => 'Giày luyện tập', 'image' => $productImages[0], 'url' => '#san-pham'],
        ];
        $categoryImages = [
            ...
            'https://images.unsplash.com/photo-1530137073520-4ea6e2f10a48?auto=format&fit=crop&w=800&q=85',
            'https://images.unsplash.com/photo-1579952363873-27f3bade9f55?auto=format&fit=crop&w=800&q=85',
        ];

        return [
            ['block_type' => 'hero_slider', 'label' => 'Hero sneaker', 'description' => 'Slider banner lớn đầu trang.', 'preview_image' => $preview, 'anchor_id' => 'top', 'dynamic' => true, 'settings' => ['placement' => 'e800-hero-slider', 'limit' => 3, 'autoplay_ms' => 6500], 'settings_schema' => ['placement' => ['type' => 'text', 'label' => 'Placement banner'], 'limit' => ['type' => 'number', 'label' => 'Số slide'], 'autoplay_ms' => ['type' => 'number', 'label' => 'Tốc độ tự chạy (ms)']], 'media' => ['image' => $hero], 'data' => ['vi' => array_merge($heading('Nơi hiệu suất gặp gỡ phong cách hằng ngày', 'Tự tin thể hiện phong cách', 'Bứt phá từng chuyển động với thiết kế sneaker thế hệ mới.', 'Khám phá ngay'), ['content' => ['slides' => [['title' => 'Nơi hiệu suất gặp gỡ phong cách hằng ngày', 'kicker' => 'Tự tin thể hiện phong cách', 'summary' => 'Bứt phá từng chuyển động với thiết kế sneaker thế hệ mới.', 'button_label' => 'Khám phá ngay', 'image' => $hero, 'link_url' => '#san-pham']]]]), 'en' => $heading('Where performance meets everyday style', 'Move with confidence', 'Next-generation sneakers for every motion.', 'Explore now')]],
            ['block_type' => 'e800_collections', 'label' => 'Các bộ sưu tập', 'description' => 'Danh mục sneaker nổi bật dạng thẻ ngang.', 'preview_image' => $preview, 'anchor_id' => 'bo-suu-tap', 'dynamic' => true, 'settings' => ['source' => 'custom', 'limit' => 6], 'settings_schema' => $source(6, ['catalog_categories' => 'Danh mục sản phẩm', 'custom' => 'Tự nhập']), 'data' => ['vi' => array_merge($heading('Khám phá các', 'bộ sưu tập'), $withItems($collections)), 'en' => $heading('Explore our', 'collections')]],
            ['block_type' => 'e800_deals', 'label' => 'Deal sản phẩm lớn', 'description' => 'Ba banner sản phẩm và giá khuyến mãi.', 'preview_image' => $preview, 'anchor_id' => 'deal-noi-bat', 'dynamic' => true, 'settings' => ['source' => 'cms_products', 'limit' => 3], 'settings_schema' => $source(3), 'data' => ['vi' => array_merge($heading(), $withItems(array_slice($products, 0, 3))), 'en' => $heading()]],
            ['block_type' => 'e800_shop_look', 'label' => 'Mua trọn bộ', 'description' => 'Ảnh chiến dịch và ba sản phẩm phối cùng nhau.', 'preview_image' => $preview, 'anchor_id' => 'mua-tron-bo', 'dynamic' => true, 'settings' => ['source' => 'cms_products', 'limit' => 3], 'settings_schema' => $source(3), 'media' => ['image' => $look], 'data' => ['vi' => array_merge($heading('Mua trọn bộ trang phục', 'Ưu đãi đặc biệt cuối năm', 'Phối trọn phong cách và nhận thêm ưu đãi trên toàn bộ đơn hàng.', 'Thêm tất cả vào giỏ hàng'), $withItems(array_slice($products, 3, 3))), 'en' => $heading('Shop the complete look', 'Year-end special', 'Build your look and save more.', 'Add all to cart')]],
            ['block_type' => 'e800_needs', 'label' => 'Sản phẩm theo nhu cầu', 'description' => 'Lưới sản phẩm lớn có tab nhu cầu.', 'preview_image' => $preview, 'anchor_id' => 'san-pham', 'dynamic' => true, 'settings' => ['source' => 'cms_products', 'limit' => 10], 'settings_schema' => $source(10), 'data' => ['vi' => array_merge($heading('Mua sắm', 'theo nhu cầu'), ['content' => ['tabs' => ['Thời trang', 'Luyện tập', 'Chạy bộ'], 'items' => $products]]), 'en' => array_merge($heading('Shop by', 'your needs'), ['content' => ['tabs' => ['Lifestyle', 'Training', 'Running']]])]],
            ['block_type' => 'e800_categories', 'label' => 'Danh mục nổi bật', 'description' => 'Năm thẻ lifestyle theo môn thể thao.', 'preview_image' => $preview, 'anchor_id' => 'danh-muc', 'settings' => [], 'data' => ['vi' => array_merge($heading('Danh mục', 'Nổi bật'), $withItems(collect(['Chạy bộ', 'Thời trang thể thao', 'Luyện tập', 'Thể thao ngoài trời', 'Bóng đá'])->map(fn ($title, $index) => ['title' => $title, 'image' => $categoryImages[$index], 'url' => '#san-pham'])->all())), 'en' => $heading('Featured', 'categories')]],
            ['block_type' => 'e800_brands', 'label' => 'Dải thương hiệu', 'description' => 'Tên các thương hiệu phân phối.', 'preview_image' => $preview, 'anchor_id' => 'thuong-hieu', 'settings' => [], 'data' => ['vi' => array_merge($heading(), $withItems(collect(['NORTHLINE', 'VELOCITY', 'PRO SPECS', 'URBAN ARC', 'MOTION', 'E800', 'MAX CO.'])->map(fn ($title) => ['title' => $title])->all())), 'en' => $heading()]],
            ['block_type' => 'e800_promotions', 'label' => 'Bản tin khuyến mãi', 'description' => 'Tin tức và chiến dịch khuyến mãi mới nhất.', 'preview_image' => $preview, 'anchor_id' => 'khuyen-mai', 'dynamic' => true, 'settings' => ['source' => 'cms_posts', 'limit' => 5], 'settings_schema' => $source(5, ['cms_posts' => 'Tin tức', 'custom' => 'Tự nhập']), 'data' => ['vi' => array_merge($heading('Bản tin', 'khuyến mãi'), $withItems(collect(range(1, 5))->map(fn ($index) => ['title' => "Ưu đãi hiệu suất {$index}: Deal thể thao bùng nổ", 'summary' => 'Khám phá sản phẩm tuyển chọn cùng ưu đãi giới hạn dành riêng cho thành viên.', 'image' => $categoryImages[($index - 1) % 5], 'published_at' => now()->format('d/m/Y'), 'url' => '#'])->all())), 'en' => $heading('Promotion', 'news')]],
            ['block_type' => 'e800_benefits', 'label' => 'Cam kết mua hàng', 'description' => 'Bốn cam kết dịch vụ hiển thị tại footer.', 'preview_image' => $preview, 'anchor_id' => 'cam-ket', 'settings' => [], 'data' => ['vi' => array_merge($heading(), $withItems([])), 'en' => $heading()]],
        ];
    }
    ...
    private function foot403DefaultBlocks(): array
    {
        ...
        return [
            ...
            ['block_type' => 'featured_categories', 'label' => 'Danh mục nổi bật', 'description' => 'Danh mục sản phẩm lấy từ dữ liệu cửa hàng.', 'preview_image' => $preview, 'anchor_id' => 'danh-muc', 'dynamic' => true, 'settings' => ['source' => 'catalog_categories', 'limit' => 8, 'order' => 'sort_order'], 'settings_schema' => $schema, 'data' => ['vi' => ['title' => 'Danh mục nổi bật', 'subtitle' => 'Khám phá hương vị', 'content' => ['items' => []]]]],
            ...
            ['block_type' => 'bizmax_latest_posts', 'label' => 'Tin tức ẩm thực', 'description' => 'Bài viết mới nhất từ bảng tin tức.', 'preview_image' => $preview, 'anchor_id' => 'tin-tuc', 'dynamic' => true, 'settings' => ['source' => 'cms_posts', 'limit' => 3, 'featured_only' => false], 'settings_schema' => $schema, 'data' => ['vi' => ['title' => 'Tin tức', 'subtitle' => 'Chuyện từ căn bếp', 'button_label' => 'Xem thêm', 'content' => ['items' => []]]]],
    ...
    private function e807DefaultBlocks(): array
    {
        ...
        $withItems = fn (array $items): array => ['content' => ['items' => $items]];
        $source = fn (int $limit, array $options = ['cms_products' => 'Sản phẩm', 'custom' => 'Tự nhập']): array => ['source' => ['type' => 'select', 'label' => 'Nguồn dữ liệu', 'options' => $options], 'limit' => ['type' => 'number', 'label' => 'Số mục hiển thị', 'default' => $limit], 'category_id' => ['type' => 'select', 'label' => 'Danh mục lọc'], 'featured_only' => ['type' => 'boolean', 'label' => 'Chỉ lấy mục nổi bật']];
        $products = collect(range(0, 11))->map(fn (int $i): array => ['title' => ['Máy khoan pin Impact Pro', 'Máy cưa đĩa Circular 185', 'Máy mài góc GrindMax', 'Máy xịt rửa Pressure 150', 'Máy hàn điện tử Arc Mini', 'Bộ dụng cụ sửa chữa 108 chi tiết'][$i % 6].' '.($i + 1), 'summary' => 'E807 PROFESSIONAL', 'image' => $i % 2 ? $hero : $collection, 'price' => 859000 + $i * 215000, 'original_price' => 1210000 + $i * 260000, 'url' => '#san-pham'])->all();
        $categories = collect(['Máy rửa áp lực', 'Công cụ không dây', 'Dụng cụ khí', 'Lưỡi dao', 'Súng bắn đinh', 'Súng bấm ghim', 'Dụng cụ cắt', 'Đầu bits', 'Dụng cụ an toàn', 'Dụng cụ mài', 'Bảo hộ trên cao', 'Dụng cụ điện'])->map(fn ($title, $i) => ['title' => $title, 'icon' => ['fa-solid fa-water', 'fa-solid fa-screwdriver', 'fa-solid fa-wind', 'fa-solid fa-scissors', 'fa-solid fa-hammer', 'fa-solid fa-toolbox'][$i % 6], 'url' => '#san-pham'])->all();
        $promos = [['title' => 'Khuyến mại máy đầm cóc', 'summary' => 'Thiết bị thi công bền bỉ', 'price_label' => 'Từ 14.750.000đ', 'image' => $collection], ['title' => 'Khuyến mại máy khoan', 'summary' => 'Hiệu suất cao cho mọi công trình', 'price_label' => 'Từ 3.155.000đ', 'image' => $hero], ['title' => 'Khuyến mại balo đồ nghề', 'summary' => 'Sắp xếp dụng cụ chuyên nghiệp', 'price_label' => 'Từ 1.120.000đ', 'image' => $collection]];
        $posts = collect(['Khám phá công cụ giúp tăng năng suất công việc', 'Top 5 máy mài pin nhỏ đáng đầu tư', 'Cách chọn máy khoan đúng nhu cầu', 'An toàn trước mọi dự án cơ khí'])->map(fn ($title, $i) => ['title' => $title, 'summary' => 'Kinh nghiệm thực tế giúp lựa chọn và sử dụng dụng cụ an toàn, hiệu quả hơn.', 'image' => $i % 2 ? $hero : $collection, 'url' => '#'])->all();
        return [
            ['block_type' => 'hero_tools', 'label' => 'Hero dụng cụ chuyên nghiệp', 'description' => 'Banner công trường đầu trang.', 'preview_image' => $preview, 'anchor_id' => 'top', 'media' => ['image' => $hero], 'settings' => [], 'data' => ['vi' => $heading('Sức mạnh cho mọi công trình', 'GIẢI PHÁP DỤNG CỤ CHUYÊN NGHIỆP', 'Dụng cụ bền bỉ · hiệu suất vượt trội · hỗ trợ tận tâm', 'Khám phá ngay'), 'en' => $heading('Power for every project', 'PROFESSIONAL TOOL SOLUTIONS', 'Reliable performance for every job', 'Explore now')]],
            ['block_type' => 'e807_categories', 'label' => 'Danh mục nổi bật', 'description' => 'Mười hai nhóm dụng cụ chính.', 'preview_image' => $preview, 'anchor_id' => 'danh-muc', 'settings' => [], 'data' => ['vi' => array_merge($heading('Danh mục nổi bật'), $withItems($categories)), 'en' => $heading('Featured categories')]],
            ['block_type' => 'e807_deals', 'label' => 'Deal cực hấp dẫn', 'description' => 'Sản phẩm khuyến mại trên nền công nghiệp.', 'preview_image' => $preview, 'anchor_id' => 'deal', 'dynamic' => true, 'settings' => ['source' => 'cms_products', 'limit' => 5, 'featured_only' => true], 'settings_schema' => $source(5), 'data' => ['vi' => array_merge($heading('Deal cực hấp dẫn'), $withItems(array_slice($products, 0, 5))), 'en' => $heading('Hot deals')]],
            ['block_type' => 'e807_new', 'label' => 'Sản phẩm mới', 'description' => 'Năm sản phẩm mới nhất.', 'preview_image' => $preview, 'anchor_id' => 'san-pham', 'dynamic' => true, 'settings' => ['source' => 'cms_products', 'limit' => 5], 'settings_schema' => $source(5), 'data' => ['vi' => array_merge($heading('Sản phẩm mới'), $withItems(array_slice($products, 1, 5))), 'en' => $heading('New products')]],
            ['block_type' => 'e807_promo_trio', 'label' => 'Ba banner khuyến mại', 'description' => 'Ba banner theo nhóm thiết bị.', 'preview_image' => $preview, 'anchor_id' => 'khuyen-mai', 'settings' => [], 'data' => ['vi' => array_merge($heading(), $withItems($promos)), 'en' => $heading()]],
            ['block_type' => 'e807_best', 'label' => 'Sản phẩm bán chạy', 'description' => 'Lưới mười sản phẩm nổi bật.', 'preview_image' => $preview, 'anchor_id' => 'ban-chay', 'dynamic' => true, 'settings' => ['source' => 'cms_products', 'limit' => 10, 'featured_only' => true], 'settings_schema' => $source(10), 'data' => ['vi' => array_merge($heading('Sản phẩm bán chạy'), $withItems(array_slice($products, 2, 10))), 'en' => $heading('Best sellers')]],
            ['block_type' => 'e807_promo_duo', 'label' => 'Khuyến mại máy khoan và máy cưa', 'description' => 'Hai banner ngang sản phẩm chủ lực.', 'preview_image' => $preview, 'anchor_id' => 'promo-duo', 'settings' => [], 'data' => ['vi' => array_merge($heading(), $withItems(array_slice($promos, 0, 2))), 'en' => $heading()]],
            ['block_type' => 'e807_about', 'label' => 'Giới thiệu giải pháp', 'description' => 'Giới thiệu cửa hàng và lợi thế dịch vụ.', 'preview_image' => $preview, 'anchor_id' => 'gioi-thieu', 'settings' => [], 'data' => ['vi' => $heading('Giải pháp toàn diện', null, 'Hệ sinh thái dụng cụ đáng tin cậy cho nhà thầu, kỹ sư và người yêu thích DIY.'), 'en' => $heading('Complete solutions', null, 'A dependable tool ecosystem for professionals and DIY makers.')]],
            ['block_type' => 'e807_news', 'label' => 'Tin tức mới nhất', 'description' => 'Tin tức ngành dụng cụ.', 'preview_image' => $preview, 'anchor_id' => 'tin-tuc', 'dynamic' => true, 'settings' => ['source' => 'cms_posts', 'limit' => 4], 'settings_schema' => $source(4, ['cms_posts' => 'Tin tức', 'custom' => 'Tự nhập']), 'data' => ['vi' => array_merge($heading('Tin tức mới nhất'), $withItems($posts)), 'en' => $heading('Latest news')]],
            ['block_type' => 'e807_testimonials', 'label' => 'Feedback khách hàng', 'description' => 'Ba phản hồi trên nền công nghiệp.', 'preview_image' => $preview, 'anchor_id' => 'feedback', 'settings' => [], 'data' => ['vi' => array_merge($heading('Feedback từ khách hàng'), $withItems([['title' => 'Hoàng Dung', 'role' => 'Kỹ sư công trình', 'summary' => 'Sản phẩm chắc chắn, tư vấn đúng nhu cầu và hỗ trợ sau bán hàng rất nhanh.'], ['title' => 'Hồng Liêm', 'role' => 'Thiết kế nội thất', 'summary' => 'Danh mục đa dạng, thông số rõ ràng và giao hàng đúng hẹn.'], ['title' => 'Ngọc Tuyền', 'role' => 'Chủ xưởng', 'summary' => 'Thiết bị vận hành ổn định, chính sách bảo hành minh bạch.']])), 'en' => $heading('Customer feedback')]],
            ['block_type' => 'e807_knowledge', 'label' => 'Chia sẻ kiến thức', 'description' => 'Ba bài viết cẩm nang kỹ thuật.', 'preview_image' => $preview, 'anchor_id' => 'kien-thuc', 'settings' => [], 'data' => ['vi' => array_merge($heading('Chia sẻ kiến thức'), $withItems(array_slice($posts, 1, 3))), 'en' => $heading('Knowledge hub')]],
            ['block_type' => 'e807_brands', 'label' => 'Thương hiệu nổi bật', 'description' => 'Dải nhãn hàng dụng cụ.', 'preview_image' => $preview, 'anchor_id' => 'thuong-hieu', 'settings' => [], 'data' => ['vi' => array_merge($heading('Thương hiệu nổi bật'), $withItems(collect(['PROCORE', 'IRONMAX', 'LUXTER', 'PIGEON', 'WESCO', 'TOLSEN'])->map(fn ($title) => ['title' => $title])->all())), 'en' => $heading('Featured brands')]],
            ['block_type' => 'e807_policies', 'label' => 'Chính sách dịch vụ', 'description' => 'Bốn chính sách cuối trang.', 'preview_image' => $preview, 'anchor_id' => 'chinh-sach', 'settings' => [], 'data' => ['vi' => array_merge($heading(), $withItems([['title' => 'Giao hàng miễn phí', 'summary' => 'Nội thành Hồ Chí Minh', 'icon' => 'fa-solid fa-truck-fast'], ['title' => 'Bảo hành', 'summary' => 'Bảo hành lên đến 2 năm', 'icon' => 'fa-solid fa-shield-halved'], ['title' => 'Hỗ trợ nhanh', 'summary' => 'Hotline 1900 6750', 'icon' => 'fa-solid fa-headset'], ['title' => 'Đổi trả', 'summary' => 'Chính sách đổi trả dễ dàng', 'icon' => 'fa-solid fa-rotate']])) , 'en' => $heading()]],
        ];
    }
}
