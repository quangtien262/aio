...
class ThemeDemoContentGenerator
{
    ...
    private function buildProductSku(array $preset, string $departmentName, int $parentIndex, int $childIndex, int $productIndex): string
    {
        if ($this->isRealEstatePreset($preset)) {
            ...
            return sprintf('AIORE-%s-%02d%02d%02d', $typeCode, $parentIndex + 1, $childIndex + 1, $productIndex);
        }
    ...
    private function buildProductHighlights(array $preset, string $departmentName, string $childName): string
    {
        ...
        if ($this->isRealEstatePreset($preset)) {
            return implode(PHP_EOL, [
                'Sản phẩm '.$childName.' thuộc phân khu '.$departmentName.', dùng để test card mở bán, banner ưu đãi và trang chi tiết listing cho AIO Real Estate.',
    ...
    private function buildProductDetailContent(array $preset, string $departmentName, string $childName, string $productName): string
    {
        ...
        if ($this->isRealEstatePreset($preset)) {
            return implode(PHP_EOL.PHP_EOL, [
                $productName.' là dữ liệu demo được sinh cho preset '.$preset['label'].', giúp kiểm thử homepage dự án, trang bảng hàng và trang chi tiết listing của AIO Real Estate.',
        ...
        return implode(PHP_EOL.PHP_EOL, [
            ...
            'Sản phẩm thuộc nhóm '.$childName.' trong ngành '.$departmentName.', vì vậy phần nội dung dài được thiết kế để hiển thị đẹp ở các block mô tả, điều kiện sử dụng và vị trí áp dụng trên theme AIO Commerce.',
    ...
    private function presetDefinitions(): array
    {
        return [
            [
                ...
                'description' => 'Preset cho AIO Real Estate theo hướng landing dự án mở bán, bảng hàng listing, CTA nhận brochure và đặt lịch xem nhà mẫu.',
                'company_name' => 'AIO Real Estate Project Landing',
                'domain' => 'aio-real-estatelanding.demo',
                ...
                'hero_subtitle' => 'Dùng để test AIO Real Estate với hero landing, listing bất động sản, pricing ribbon, timeline mở bán và CTA nhận bảng giá.',
