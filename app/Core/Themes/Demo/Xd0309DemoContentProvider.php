...
class Xd0309DemoContentProvider implements ThemeDemoContentProvider
{
    private const THEME_KEY = 'XD0309';
    private const PRESET_KEY = 'xd0309-industrial-safety';
    ...
    public function preset(): array { return ['key' => self::PRESET_KEY, 'label' => 'Antek industrial safety', 'description' => 'Default banner, services, company content, quotation form, news and fixed footer for XD0309.']; }

    public function generate(string $presetKey): array
    {
        if ($presetKey !== self::PRESET_KEY) {
            throw new InvalidArgumentException('Preset demo khÃƒÂ´ng hÃ¡Â»Â£p lÃ¡Â»â€¡ cho XD0309.');
        }
        ...
        return DB::transaction(function (): array {
            ...
            foreach ([
                ...
                $service = CmsService::query()->create(['title' => $title, 'slug' => Str::slug('xd0309-logistics-'.$title), 'status' => 'published', 'summary' => $summary, 'content' => '<p>'.$summary.'</p>', 'button_label' => 'Xem chi tiÃ¡ÂºÂ¿t', 'link_url' => '#hotline', 'is_featured' => true, 'is_highlight' => true, 'sort_order' => $index, 'publish_at' => $now]);
            ...
            foreach ([
                ...
                $partner = CmsPartner::query()->create(['title' => $title, 'slug' => Str::slug('xd0309-logistics-'.$title), 'description' => $description, 'image_url' => $image($photo, 420), 'image_alt' => $title, 'link_url' => '#top', 'status' => 'published', 'publish_at' => $now, 'is_featured' => true, 'sort_order' => $index]);
                $this->record($partner);
            }
            ...
            foreach ([
                ...
                $banner = SiteBanner::query()->create(['theme_key' => self::THEME_KEY, 'placement' => 'xd0309-hero-slider', 'title' => $title, 'subtitle' => $summary, 'image_url' => $image($photo, 1920), 'link_url' => '#lien-he', 'badge' => 'TÃ†Â° vÃ¡ÂºÂ¥n doanh nghiÃ¡Â»â€¡p', 'metadata' => ['kicker' => 'TÃ†Â° vÃ¡ÂºÂ¥n doanh nghiÃ¡Â»â€¡p', 'summary' => $summary, 'button_label' => 'NhÃ¡ÂºÂ­n bÃƒÂ¡o giÃƒÂ¡'], 'sort_order' => $index, 'is_active' => true]);
                $this->record($banner);
            }

            $menu = CmsMenu::query()->create(['name' => 'XD0309 Main Menu', 'location' => 'primary-navigation', 'items' => [['label' => 'Trang chÃ¡Â»Â§', 'url' => '#top'], ['label' => 'DÃ¡Â»â€¹ch vÃ¡Â»Â¥', 'url' => '#dich-vu'], ['label' => 'GiÃ¡ÂºÂ£i phÃƒÂ¡p', 'url' => '#giai-phap'], ['label' => 'ThÃ†Â° viÃ¡Â»â€¡n', 'url' => '#thu-vien'], ['label' => 'Ã„ÂÃ¡Â»â€˜i tÃƒÂ¡c', 'url' => '#doi-tac'], ['label' => 'LiÃƒÂªn hÃ¡Â»â€¡', 'url' => '#footer']]]);
            $this->record($menu);
