...
class Xd0309DemoContentProvider implements ThemeDemoContentProvider
{
    ...
    public function preset(): array
    {
        return [
            'key' => self::PRESET_KEY,
            'label' => 'Antek industrial safety',
            'description' => 'Default banner, services, company content, quotation form, news and fixed footer for XD0309.',
        ];
    }
    ...
    public function generate(string $presetKey): array
    {
        if ($presetKey !== self::PRESET_KEY) {
            throw new InvalidArgumentException('Preset demo không hợp lệ cho XD0309.');
        }
        ...
        return DB::transaction(function (): array {
            ...
            foreach ([
                ['Vận tải nội địa', 'Điều phối phương tiện phù hợp, theo dõi lộ trình rõ ràng và đúng hạn.', 'photo-1586191582151-f73872dfd183'],
                ...
                $service = CmsService::query()->create([
                    'title' => $title,
                    'slug' => Str::slug('xd0309-logistics-'.$title),
                    'status' => 'published',
                    'summary' => $summary,
                    'content' => '<p>'.$summary.'</p>',
            ...
            $profile->forceFill([
                'site_name' => 'Logistics Việt',
                'website_type' => 'service',
                'active_theme_key' => self::THEME_KEY,
                'branding' => array_merge((array) $profile->branding, [
                    'company_name' => 'Logistics Việt',
                    'company_description' => 'Giải pháp vận tải và hậu cần linh hoạt, kết nối doanh nghiệp với mọi hành trình.',
                    'support_hotline' => '1900 9477',
                    'support_email' => 'hello@logisticsviet.vn',
                    'support_location' => '344 Huỳnh Tấn Phát, Quận 7, TP.HCM',
                ]),
            ])->save();
            ...
            $existingPage = LandingPage::query()
                ...
                ->where('is_home', true)
                ->first();
            $page = $this->landingPageBuilder->resolveHome(
                app(\App\Support\SiteContext::class)->websiteKey(),
                self::THEME_KEY,
                true
            );
