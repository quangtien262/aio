...
class Xd0304DemoContentProvider implements ThemeDemoContentProvider
{
    private const THEME_KEY = 'XD0304';
    private const PRESET_KEY = 'xd0304-logistics';
    ...
    public function preset(): array { return ['key' => self::PRESET_KEY, 'label' => 'Logistics và vận tải', 'description' => 'Banner, dịch vụ logistics, đối tác, menu và landingpage mẫu cho XD0304.']; }

    public function generate(string $presetKey): array
    {
        if ($presetKey !== self::PRESET_KEY) {
            throw new InvalidArgumentException('Preset demo không hợp lệ cho XD0304.');
        }
        ...
        return DB::transaction(function (): array {
            ...
            foreach ([
                ...
                $service = CmsService::query()->create(['title' => $title, 'slug' => Str::slug('xd0304-'.$title), 'status' => 'published', 'summary' => $summary, 'content' => '<p>'.$summary.'</p>', 'button_label' => 'Xem chi tiết', 'link_url' => '#hotline', 'is_featured' => true, 'is_highlight' => true, 'sort_order' => $index, 'publish_at' => $now]);
            ...
            foreach ([
                ...
                $partner = CmsPartner::query()->create(['title' => $title, 'slug' => Str::slug('xd0304-'.$title), 'description' => $description, 'image_url' => $image($photo, 420), 'image_alt' => $title, 'link_url' => '#top', 'status' => 'published', 'publish_at' => $now, 'is_featured' => true, 'sort_order' => $index]);
                $this->record($partner);
            }
            ...
            foreach ([
                ...
                $banner = SiteBanner::query()->create(['theme_key' => self::THEME_KEY, 'placement' => 'xd0304-hero-slider', 'title' => $title, 'subtitle' => $summary, 'image_url' => $image($photo, 1920), 'link_url' => '#footer', 'badge' => 'Logistics và vận tải', 'metadata' => ['kicker' => 'Logistics và vận tải', 'summary' => $summary, 'button_label' => 'Nhận báo giá'], 'sort_order' => $index, 'is_active' => true]);
                $this->record($banner);
            }

            $menu = CmsMenu::query()->create(['name' => 'XD0304 Main Menu', 'location' => 'primary-navigation', 'items' => [['label' => 'Trang chủ', 'url' => '#top'], ['label' => 'Dịch vụ', 'url' => '#dich-vu'], ['label' => 'Giải pháp', 'url' => '#giai-phap'], ['label' => 'Thư viện', 'url' => '#thu-vien'], ['label' => 'Đối tác', 'url' => '#doi-tac'], ['label' => 'Liên hệ', 'url' => '#footer']]]);
            $this->record($menu);
