...
class Xd0305DemoContentProvider implements ThemeDemoContentProvider
{
    private const THEME_KEY = 'XD0305';
    private const PRESET_KEY = 'xd0305-business-consulting';
    ...
    public function preset(): array { return ['key' => self::PRESET_KEY, 'label' => 'Tư vấn doanh nghiệp', 'description' => 'Banner, dịch vụ, đối tác, menu và landingpage mẫu cho XD0305.']; }

    public function generate(string $presetKey): array
    {
        if ($presetKey !== self::PRESET_KEY) {
            throw new InvalidArgumentException('Preset demo không hợp lệ cho XD0305.');
        }
        ...
        return DB::transaction(function (): array {
            ...
            foreach ([
                ...
                $service = CmsService::query()->create(['title' => $title, 'slug' => Str::slug('xd0305-logistics-'.$title), 'status' => 'published', 'summary' => $summary, 'content' => '<p>'.$summary.'</p>', 'button_label' => 'Xem chi tiết', 'link_url' => '#hotline', 'is_featured' => true, 'is_highlight' => true, 'sort_order' => $index, 'publish_at' => $now]);
            ...
            foreach ([
                ...
                $partner = CmsPartner::query()->create(['title' => $title, 'slug' => Str::slug('xd0305-logistics-'.$title), 'description' => $description, 'image_url' => $image($photo, 420), 'image_alt' => $title, 'link_url' => '#top', 'status' => 'published', 'publish_at' => $now, 'is_featured' => true, 'sort_order' => $index]);
                $this->record($partner);
            }
            ...
            foreach ([
                ...
                $banner = SiteBanner::query()->create(['theme_key' => self::THEME_KEY, 'placement' => 'xd0305-hero-slider', 'title' => $title, 'subtitle' => $summary, 'image_url' => $image($photo, 1920), 'link_url' => '#lien-he', 'badge' => 'Tư vấn doanh nghiệp', 'metadata' => ['kicker' => 'Tư vấn doanh nghiệp', 'summary' => $summary, 'button_label' => 'Nhận báo giá'], 'sort_order' => $index, 'is_active' => true]);
                $this->record($banner);
            }

            $menu = CmsMenu::query()->create(['name' => 'XD0305 Main Menu', 'location' => 'primary-navigation', 'items' => [['label' => 'Trang chủ', 'url' => '#top'], ['label' => 'Dịch vụ', 'url' => '#dich-vu'], ['label' => 'Giải pháp', 'url' => '#giai-phap'], ['label' => 'Thư viện', 'url' => '#thu-vien'], ['label' => 'Đối tác', 'url' => '#doi-tac'], ['label' => 'Liên hệ', 'url' => '#footer']]]);
            $this->record($menu);
