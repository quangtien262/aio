...
class IndustryDemoContentProvider implements ThemeDemoContentProvider
{
    ...
    {
        if (! empty($this->brief['images'])) {
            return $this->brief['images'][$index % count($this->brief['images'])];
        }
    ...
    public function generate(string $presetKey): array
    {
        ...
        return DB::transaction(function () use ($presetKey): array {
            ...
            $media = [];
            for ($i = 0; $i < 3; $i++) {
                $media[] = $this->create(CmsMedia::class, ['title' => $brand.' — ảnh minh họa '.($i + 1), 'alt_text' => 'Minh họa '.$sector, 'file_path' => '', 'file_url' => $this->image($i), 'mime_type' => 'image/'.(pathinfo($this->image($i), PATHINFO_EXTENSION) === 'jpg' ? 'jpeg' : pathinfo($this->image($i), PATHINFO_EXTENSION))]);
            }
