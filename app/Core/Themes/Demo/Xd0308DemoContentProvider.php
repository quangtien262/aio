...
class Xd0308DemoContentProvider implements ThemeDemoContentProvider
{
    ...
    public function preset(): array { return ['key' => self::PRESET_KEY, 'label' => 'Logistics vÃ  váº­n táº£i', 'description' => 'Banner, dá»‹ch vá»¥ logistics, Ä‘á»‘i tÃ¡c, menu vÃ  landingpage máº«u cho XD0308.']; }

    public function generate(string $presetKey): array
    {
        if ($presetKey !== self::PRESET_KEY) {
            throw new InvalidArgumentException('Preset demo khÃ´ng há»£p lá»‡ cho XD0308.');
        }
        ...
        return DB::transaction(function (): array {
            ...
            foreach ([
                ...
                $service = CmsService::query()->create(['title' => $title, 'slug' => Str::slug('xd0308-'.$title), 'status' => 'published', 'summary' => $summary, 'content' => '<p>'.$summary.'</p>', 'button_label' => 'Xem chi tiáº¿t', 'link_url' => '#hotline', 'is_featured' => true, 'is_highlight' => true, 'sort_order' => $index, 'publish_at' => $now]);
            ...
            foreach ([
                ...
                $banner = SiteBanner::query()->create(['theme_key' => self::THEME_KEY, 'placement' => 'xd0308-hero-slider', 'title' => $title, 'subtitle' => $summary, 'image_url' => $image($photo, 1920), 'link_url' => '#footer', 'badge' => 'Logistics vÃ  váº­n táº£i', 'metadata' => ['kicker' => 'Logistics vÃ  váº­n táº£i', 'summary' => $summary, 'button_label' => 'Nháº­n bÃ¡o giÃ¡'], 'sort_order' => $index, 'is_active' => true]);
                $this->record($banner);
            }

            $menu = CmsMenu::query()->create(['name' => 'XD0308 Main Menu', 'location' => 'primary-navigation', 'items' => [['label' => 'Trang chá»§', 'url' => '#top'], ['label' => 'Dá»‹ch vá»¥', 'url' => '#dich-vu'], ['label' => 'Giáº£i phÃ¡p', 'url' => '#giai-phap'], ['label' => 'ThÆ° viá»‡n', 'url' => '#thu-vien'], ['label' => 'Äá»‘i tÃ¡c', 'url' => '#doi-tac'], ['label' => 'LiÃªn há»‡', 'url' => '#footer']]]);
            $this->record($menu);
