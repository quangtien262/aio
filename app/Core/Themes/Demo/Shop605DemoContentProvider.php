...
class Shop605DemoContentProvider implements ThemeDemoContentProvider
{
    private const THEME_KEY = 'SHOP605';
    private const PRESET_KEY = 'shop605-oh-under';
    ...
    public function preset(): array { return ['key' => self::PRESET_KEY, 'label' => 'SHOP605 OH!Under', 'description' => 'Shop nội y và đồ bơi nữ với sản phẩm, tin tức, đối tác, đánh giá và homepage block động.']; }

    public function generate(string $presetKey): array
    {
        if ($presetKey !== self::PRESET_KEY) {
            throw new InvalidArgumentException('Preset demo không hợp lệ cho SHOP605.');
        }
        ...
        return DB::transaction(function (): array {
            ...
            $catalogImages = [
                '/theme-demo/shop605/product-women-knit.png',
                '/theme-demo/shop605/product-women-rose.png',
                '/theme-demo/shop605/product-men-green.png',
            ];
            $image = fn (string $id, int $width = 1000): string => str_contains($id, '152913') || str_contains($id, '148398')
                ? '/theme-demo/shop605/hero-fashion.png'
            ...
            foreach ([['Áo ngực', 'Thiết kế tôn dáng, mềm mại và nâng đỡ tự nhiên.'], ['Quần lót', 'Thoải mái trong từng khoảnh khắc thường ngày.'], ['Bikini', 'Tự tin khoe trọn vẻ đẹp quyến rũ và năng động.']] as $index => [$name, $description]) {
                $category = CatalogCategory::query()->create([
                    'name' => $name, 'slug' => Str::slug('shop605-'.$name), 'description' => $description,
            ...
            foreach ($products as $index => [$name, $price, $originalPrice, $categoryIndex, $photo]) {
                $product = CatalogProduct::query()->create([
                    'catalog_category_id' => $categories[$categoryIndex]->id, 'name' => $name, 'slug' => Str::slug('shop605-'.$name),
                    'sku' => 'SHOP605-'.str_pad((string) ($index + 1), 2, '0', STR_PAD_LEFT), 'price' => $price, 'original_price' => $originalPrice,
                    'stock' => 50, 'short_description' => 'OH!UNDER', 'detail_content' => '<p>Chất liệu mềm mại, đường may tinh tế và phom dáng thoải mái cho mọi chuyển động.</p>',
            ...
            foreach ([
                ['Đồ lót nữ thanh lịch', 'Thiết kế hoàn hảo tôn lên vẻ đẹp nữ tính đầy tinh tế.', '/theme-demo/shop605/product-women-knit.png'],
                ['Đồ bơi mùa hè', 'Tự tin tận hưởng những chuyến đi với phom dáng hiện đại.', '/theme-demo/shop605/product-women-rose.png'],
                ['Đồ mặc nhà mềm mại', 'Chất liệu cao cấp, êm ái và vừa vặn suốt ngày dài.', '/theme-demo/shop605/product-men-green.png'],
            ] as $index => [$title, $summary, $photo]) {
                $banner = SiteBanner::query()->create([
                    'theme_key' => self::THEME_KEY, 'placement' => 'shop605-hero-slider', 'title' => $title, 'subtitle' => $summary,
                    'image_url' => $photo, 'link_url' => '#san-pham-moi', 'badge' => 'OH!UNDER',
            ...
            $postCategory = CmsCategory::query()->create(['name' => 'Cẩm nang Bean', 'slug' => 'shop605-cam-nang-bean', 'description' => 'Xu hướng nội y, đồ bơi và bí quyết chọn sản phẩm.']);
            ...
            foreach ([['Gợi ý những mẫu bikini dành riêng cho cô nàng ngại ngùng', 'Chọn bikini phù hợp giúp bạn tự tin hơn trong mọi chuyến đi biển.'], ['Những điều tối kỵ khi diện bikini mà nàng nên biết', 'Các lưu ý quan trọng để chọn đúng kiểu dáng và chất liệu.'], ['Bốn chiêu tạo dáng với bikini để thỏa sức tung hoành', 'Những bí quyết đơn giản cho khung hình mùa hè tự nhiên.'], ['Bảy xu hướng đồ bơi hứa hẹn gây sốt hè', 'Các phong cách được yêu thích trong mùa du lịch năm nay.']] as $index => [$title, $excerpt]) {
                $post = CmsPost::query()->create(['category_id' => $postCategory->id, 'title' => $title, 'slug' => Str::slug('shop605-'.$title), 'status' => 'published', 'excerpt' => $excerpt, 'body' => '<p>'.$excerpt.'</p>', 'publish_at' => now()->subDays($index + 1), 'is_highlight' => true]);
                $this->record($post);
            }

            foreach (['HUNKEMÖLLER', 'LA SENZA', 'TRIUMPH', 'WACOAL', "VICTORIA'S SECRET", 'CALVIN KLEIN'] as $index => $name) {
                $partner = CmsPartner::query()->create(['title' => $name, 'slug' => Str::slug('shop605-'.$name), 'description' => 'Đối tác thời trang OH!Under.', 'image_url' => null, 'image_alt' => $name, 'link_url' => '#top', 'status' => 'published', 'publish_at' => now(), 'is_featured' => true, 'sort_order' => $index]);
                $this->record($partner);
            }

            $home = route('site.home');
            $menu = CmsMenu::query()->create(['name' => 'SHOP605 Main Menu', 'location' => 'primary-navigation', 'items' => [
            ...
            $this->record($menu);
            $page = CmsPage::query()->firstOrCreate(['slug' => 'contact'], ['title' => 'Liên hệ', 'status' => 'published', 'excerpt' => 'Liên hệ OH!Under Fashion.', 'body' => '<p>Đội ngũ OH!Under luôn sẵn sàng hỗ trợ bạn.</p>', 'publish_at' => now()]);
            if ($page->wasRecentlyCreated) $this->record($page);

            $profile = SiteProfile::query()->firstOrNew();
            $profile->forceFill(['site_name' => 'OH!Under', 'website_type' => 'ecommerce', 'active_theme_key' => self::THEME_KEY, 'branding' => array_merge((array) $profile->branding, ['logo_url' => null, 'company_name' => 'OH!Under', 'company_description' => 'Thiết kế nội y và đồ bơi tinh tế, thoải mái và tôn vinh đường cong.', 'support_hotline' => '1800.6750', 'support_email' => 'support@ohunder.vn', 'support_location' => '70 Lữ Gia, Phường 15, Quận 11, TP. Hồ Chí Minh'])])->save();
            ...
            return ['preset' => $this->preset(), 'counts' => ['categories' => 3, 'products' => 12, 'banners' => 3, 'post_categories' => 1, 'posts' => 4, 'partners' => 6, 'pages' => $page->wasRecentlyCreated ? 1 : 0, 'menus' => 1, 'landing_pages' => ! $existing && $landing ? 1 : 0], 'purged' => $purged];
        });
    }
