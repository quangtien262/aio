...
class HrmOrganizationController
{
    ...
    {
        $rules = ['code' => ['required', 'string', 'max:50', Rule::unique($type === 'departments' ? 'hrm_departments' : 'hrm_positions', 'code')->ignore($record?->id)], 'name' => ['required', 'string', 'max:255'], 'description' => ['nullable', 'string'], 'is_active' => ['required', 'boolean']];
        if ($type === 'departments') {
