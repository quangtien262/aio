...
class RepairXd0310EncodingCommand extends Command
{
    ...
    protected $description = 'Sửa tiếng Việt mã hóa sai trong dữ liệu XD0310; mặc định chỉ xem trước.';

    public function handle(LegacyTextEncoding $repair): int
    {
        $site = Site::query()->where('domain', $this->argument('domain'))->first();
        if (! $site || $site->theme_key !== 'XD0310') {
            $this->error('Domain không tồn tại hoặc không dùng theme XD0310.');
            return self::FAILURE;
        }
        ...
        $fields = [
            ...
            'cms_posts' => ['title', 'summary', 'content'],
        ...
        DB::transaction(function () use ($site, $fields, $jsonFields, $repair, &$changes): void {
            ...
            if ($this->option('write') && $changes !== []) {
                ...
                $path = $directory.'/xd0310-'.now()->format('Ymd-His').'-'.bin2hex(random_bytes(4)).'.json';
